...
public class ComponentRenderer extends AbstractBlockRenderer {
    ...
    private void renderBubble(Tessellator tess, double r1, double r2) {
        ...
        bindTexture(flatTex);
        // Disable lighting for the flat overlay quad so smooth lighting
        // doesn't cause brightness popping when the view angle changes.
    ...
    public void renderBlock(Tessellator tess, TileEntity te, int x, int y, int z) {
        ...
        if (te instanceof ComponentTileEntity) {
            if (!(te instanceof LandingPadTileEntity)) renderBubble(tess);
            ComponentTileEntity tcte = ((ComponentTileEntity) te);
            int count = 0;
            for (TardisTEComponent comp : TardisTEComponent.values()) if (tcte.hasComponent(comp)) count++;
            if (count > 0) {
                double ang = -360 / count;
                int i = 0;
                for (TardisTEComponent comp : TardisTEComponent.values())
                    if (tcte.hasComponent(comp)) renderStick(comp.tex, 0.1, 0, (i++) * ang);
            }
        }
    }
