...
public class ConsoleRenderer extends AbstractBlockRenderer {
    ...
    private void renderSchemaChooser(Tessellator tess, ConsoleTileEntity tce) {
        // Ensure schema selector text is full-bright white
        compRender
            .renderTextScreen(tess, tce, tce.schemaCategoryString, 52, 1.05, -0.42, -0.1, -49, -90, 0, 0.4, 0.4, 0.2);
        compRender
            .renderTextScreen(tess, tce, tce.schemaChooserString, 52, 1.26, -0.22, -0.1, -49, -90, 0, 0.4, 0.4, 0.2);
    ...
    private void renderFrontPanel(Tessellator tess, ConsoleTileEntity tce, CoreTileEntity core) {
        ...
        compRender.renderScrewdriverHolder(tess, tce, -1, -0.55, -0.45, 0, 0, -45, 0.5, 0.5, 0.5); // ID 6 Screwdriver
                                                                                                   // holder
    ...
    private void renderRightPanel(Tessellator tess, ConsoleTileEntity tce, CoreTileEntity core) {
        ...
        compRender.renderScreen(tess, tce, 100, "main", -0.7, -0.325, -1.20, -45, 180, 0, 0.6, 0.6, 0.6); // ID 100
                                                                                                          // Coord
                                                                                                          // guesser
    ...
    private void renderLeftPanel(Tessellator tess, ConsoleTileEntity tce, CoreTileEntity core) {
        ...
        compRender.renderPushSwitch(tess, tce, 900, -0.95, -0.2, 1.37, -39, 0, 0, 0.5, 0.5, 0.5); // ID 900 save/load
                                                                                                  // switch
    ...
    public void renderBlock(Tessellator tess, TileEntity te, int x, int y, int z) {
        if (compRender == null) compRender = new ControlRenderer(func_147498_b(), field_147501_a.field_147553_e);
    ...
    public void renderTileEntityAt(TileEntity tileEntity, double d0, double d1, double d2, float f) {
        ...
        if (handleLighting() && (w != null)) {
            // Apply a small boost to the environment lightmap so the console glows white but still responds
