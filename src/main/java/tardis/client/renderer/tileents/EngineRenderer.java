...
public class EngineRenderer extends AbstractObjRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileEntity, double d0, double d1, double d2, float f) {
        ...
        if (handleLighting() && (w != null)) {
            // Apply a small boost to the environment lightmap so the engine glows white but still responds
        ...
        /*
         * Note that true tile entity coordinates (tileEntity.xCoord, etc) do not match to render coordinates (d, etc)
         * that are calculated as [true coordinates] - [player coordinates (camera coordinates)]
         */
        GL11.glPushMatrix();
    ...
    private void renderBack(Tessellator tess, EngineTileEntity eng, CoreTileEntity core, TardisDataStore ds) {
        ...
        if ((eng.visibility < 1) && (ds != null)) renderUnderPanel(tess, eng, core, ds);
        GL11.glPushMatrix();
        comps.renderButton(tess, eng, 100, -0.01, 0.5, 0.04, 0, 0, 270, 0.3, 0.3, 0.3);
        GL11.glPopMatrix();
    }

    private void renderBreakable(Tessellator tess, EngineTileEntity eng, CoreTileEntity core, TardisDataStore ds,
        int comp) {
        double x = 0.5;
        double y = 0.6;
        double z = -0.05;
        ...
        if (ds.damage.isComponentBroken(comp)) bindTexture(bubbleDown);
        else bindTexture(bubbleUp);
        bubble.renderAll();
        GL11.glPopMatrix();
    }
    ...
    private void renderUnderPanel(Tessellator tess, EngineTileEntity eng, CoreTileEntity core, TardisDataStore ds) {
        ...
        GL11.glPopMatrix();
        for (int i = 0; i < TardisDamageSystem.numBreakables; i++) renderBreakable(tess, eng, core, ds, i);
    }

    @Override
    public void renderBlock(Tessellator tess, TileEntity te, int x, int y, int z) {
        if (comps == null) comps = new ControlRenderer(func_147498_b(), field_147501_a.field_147553_e);
