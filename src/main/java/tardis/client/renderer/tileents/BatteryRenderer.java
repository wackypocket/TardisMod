...
public class BatteryRenderer extends AbstractObjRenderer implements IItemRenderer {
...

    {
        if (box == null) {
            ...
            for (int i = 0; i < 360; i++) offsets[i] = Math.sin(Math.PI * (((double) i) / 180));
        }
    }
    ...
    private void magicFunct(double angle, int mode, int ringNum) {
        ...
        if (mode == 0) {
            for (int i = 0; i <= ringNum; i++) {
                GL11.glPushMatrix();
                if ((i % 2) == 0) GL11.glRotated(angle * pr(i), 1, 0, 0);
                else GL11.glRotated(angle * pr(i), 0, 0, 1);
            }
        } else if (mode == 1) {
            GL11.glPushMatrix();
            int thing = (int) Math.round(angle - (15 * ringNum));
            if (thing < 0) thing += 360;
        ...
        ring.renderAll();
        if (mode == 0) {
            for (int i = ringNum; i >= 0; i--) GL11.glPopMatrix();
        } else if ((mode == 1) || (mode == 2)) GL11.glPopMatrix();
    }

    @Override
    public void renderBlock(Tessellator tess, TileEntity te, int x, int y, int z) {
        if (!(te instanceof BatteryTileEntity)) return;
        BatteryTileEntity bat = (BatteryTileEntity) te;
        int lev = bat.getLevel();
        GL11.glPushMatrix();
        if (lev == 3) bindTexture(boxTexThr);
        else if (lev == 2) bindTexture(boxTexTwo);
        else bindTexture(boxTexOne);
        box.renderAll();
        GL11.glPopMatrix();
        GL11.glPushMatrix();
        bindTexture(ringTex);
        ...
        if (lev > 1) magicFunct(angle, mode, 2);
        if (lev > 2) magicFunct(angle, mode, 3);
        GL11.glPopMatrix();
    }
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        ...
        if (lev == 0) Minecraft.getMinecraft().renderEngine.bindTexture(boxTexOne);
        else if (lev == 1) Minecraft.getMinecraft().renderEngine.bindTexture(boxTexTwo);
        else Minecraft.getMinecraft().renderEngine.bindTexture(boxTexThr);
        ...
        magicFunct(0, 0, 0);
        magicFunct(0, 0, 1);
        if (lev > 1) magicFunct(0, 1, 2);
        if (lev > 2) magicFunct(0, 1, 3);
        GL11.glPopMatrix();
        GL11.glPopMatrix();
    }
