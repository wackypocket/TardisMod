...
public class AdvancedLabRenderer extends AbstractObjRenderer {
    ...
    private void renderRecipes(AdvancedLab al) {
        ...
        fr.drawString(
            String.format("%d/%d", al.page + 1, MathHelper.ceil(recipes.size() / 6.0)),
            40,
            74,
            Colour.white.asInt,
            false);
    ...
    private void renderIS(ItemStack is, int x, int y, int count, boolean drawCount) {
        ...
        if ((is.stackSize > 1) || drawCount) {
            ...
            if (drawCount) c = count >= is.stackSize ? gre : red;
    ...
    private void renderRecipe(AdvancedLab al, LabRecipe lr) {
        ...
        for (int i = 0; i < lr.dest.length; i++) renderIS(lr.dest[i], 16 * i, 16, 0, false);
    ...
    private void renderScreen(AdvancedLab al) {
        ...
        if (al.selectedRecipe == null) renderRecipes(al);
        else renderRecipe(al, al.selectedRecipe);
    }
    ...
    public void renderBlock(Tessellator tess, TileEntity te, int x, int y, int z) {
        ...
        if (model == null)
            model = AdvancedModelLoader.loadModel(new ResourceLocation("tardismod", "models/newlab.obj"));
        RenderHelper.bindTexture(tex);
        int meta = 0;
        if (te != null) meta = te.getWorldObj()
