...
public class Configs {
    ...
    private static void refreshLevelConfig() {
        ...
        xpInc = levelConfig
            .getInt("xp increase", 20, "The amount that is added on to the xp cost every time the TARDIS levels up");

        maxEnergy = levelConfig.getInt("Max energy", 1000, "The base maximum energy");
        maxEnergyInc = levelConfig
            .getInt("Max energy increase", 1000, "How much a level of energy increases the max amount of energy");
        maxNumRooms = Math.max(1, levelConfig.getInt("Max rooms", 6, "The base maximum number of rooms"));
        maxNumRoomsInc = Math.max(
            1,
            levelConfig.getInt(
                "Max rooms increase",
                6,
                "How much a level of max rooms increases the maximum number of rooms"));
        energyPerSecond = levelConfig
            .getInt("Energy rate", 1, "The base amount of energy the TARDIS generates per second");
        energyPerSecondInc = levelConfig.getInt(
            "Energy rate increase",
            1,
            "How much a level of energy rate increases the amount of energy per second");

        levelLocate = levelConfig
            .getInt("Function - Locate", 3, "When the locate functionality is unlocked, -1 disables");
        levelSensors = levelConfig
            .getInt("Function - Sensors", 5, "When the sensors functionality is unlocked, -1 disables");
        levelStable = levelConfig
            .getInt("Function - Stabilizers", 7, "When the stabilizers functionality is unlocked, -1 disables");
        levelTransmat = levelConfig
            .getInt("Function - Transmat", 9, "When the transmat functionality is unlocked, -1 disables");
        levelRecall = levelConfig
            .getInt("Function - Recall", 11, "When the recall functionality is unlocked, -1 disables");
        levelTranq = levelConfig
            .getInt("Function - Tranquility", 15, "When the tranquility functionality is unlocked, -1 disables");
        levelClarity = levelConfig
            .getInt("Function - Clarity", 13, "When the clarity functionality is unlocked, -1 disables");
        levelSpawnProt = levelConfig
            .getInt("Function - Spawn Prev", 15, "When the spawn prevention functionality is unlocked, -1 disables");
    }
    ...
    private static void refreshRoundelConfig() {
        ...
        maxComponents = rfBase = roundelConfig
            .getInt("RF storage base", 50000, "The amount of RF that can be stored when a TARDIS is level 0");
        euRatio = roundelConfig.getInt("RF->EU Conversion ratio", 4, "How many RF make up a single unit of EU");
        rfInc = roundelConfig.getInt(
            "RF storage increase per level",
            50000,
            "The extra amount of storage which is added every time the TARDIS levels up");
        rfPerT = roundelConfig
            .getInt("RF output per tick", 4098, "The amount of RF which the TARDIS can output per tick");
        maxFlu = roundelConfig.getInt(
            "Internal tanks - Max mb",
            16000,
            "The amount of millibuckets of fluid that can be stored for each internal tank");
        numTanks = roundelConfig
            .getInt("Internal tanks - Number", 6, "The number of internal tanks that the TARDIS has");
        numInvs = roundelConfig
            .getInt("Internal inventory - Slots", 30, "The number of item inventory slots that the TARDIS has");
        numAspects = roundelConfig.getInt(
            "Internal TC - Num aspects",
            32,
            "The number of thaumcraft aspects which can be stored in the TARDIS's cabling");
        maxEachAspect = roundelConfig
            .getInt("Internal TC - Max aspect", 32, "The maximum amount of each thaumcraft aspect that can be stored");
        maxEachAspectInc = roundelConfig
            .getInt("Internal TC - Max aspect inc", 16, "The amount of aspect storage gained per level");

        nanogeneRange = Math
            .pow(roundelConfig.getDouble("Nanogene - Range", 6, "The range at which nanogenes can heal"), 2);
        nanogeneTimer = roundelConfig
            .getInt("Nanogene - Timer", 10, "The number of ticks between each nanogene healing pulse");
        nanogeneCost = roundelConfig
            .getInt("Nanogene - Cost", 1, "The amount of Artron energy used up each time a nanogene heals");
        nanogeneHealAmount = roundelConfig
            .getInt("Nanogene - Heal amount", 2, "The amount of health a nanogene can restore per pulse");
        nanogeneFeed = roundelConfig
            .getBoolean("Nanogene - Feeds", true, "Whether nanogenes should also feed players as well as heal");

        restorationFieldTimer = roundelConfig
            .getInt("Restoration Field - Timer", 120, "The number of ticks between each restoration field pulse");
        restorationFieldPercentage = roundelConfig.getInt(
            "Restoration Field - Repair percentage",
            70,
            "What percentage of the tool will be repaired (1 - 100)");
        restorationFieldCost = roundelConfig.getInt(
            "Restoration Field - Cost",
            5,
            "The amount of Artron energy used up each time a tool gets healed by 1 point");
        restorationFieldRange = Math.pow(
            roundelConfig
                .getDouble("Restoration Field - Range", 4, "The range at which the restoration field has effect"),
            2);

        maxComponents = roundelConfig
            .getInt("Maximum components", 6, "The number of cable interfaces/components per roundel/landing pad");
    }
    ...
    private static void refreshCraftingConfig() {
        ...
        maxLabSpeed = craftConfig
            .getInt("Max lab speed", 5, "The maximum speed which the lab can operate at (artron/tick)");
        kontronCraftable = craftConfig.getBoolean(
            "Craftable - Kontron",
            false,
            "If true, a standard crafting recipe is added for the kontron crystal");
        keyCraftable = craftConfig
            .getBoolean("Craftable - Key", true, "True if the key is craftable.", "False if they can only be spawned");
        keyReqKontron = craftConfig
            .getBoolean("Key requires kontron", true, "True if the key requires a Kontron crystal to craft");
        numDirtRecipe = MathHelper.clamp(
            craftConfig
                .getInt("Temporal dirt production", 2, "Number of temporal dirt per recipe instance", "Min 1, max 64"),
            1,
            64);
    }
    ...
    private static void refreshToolsConfig() {
        ...
        batMaxEnergyPerLevel = toolConfig
            .getInt("Battery - Max energy per level", 100, "The amount of max energy that is gained per level");
        batEnergyPerLevel = toolConfig
            .getInt("Battery - Energy per level", 1, "The amount of energy per pulse that is gained per level");
        batTicksPerEnergy = toolConfig
            .getInt("Battery - Ticks per energy", 20, "The number of ticks between each energy pulse");
        batNeedsJumpStart = toolConfig.getBoolean(
            "Battery - Needs jump start",
            true,
        ...
        dirtTickMult = toolConfig.getDouble(
            "Dirt block - Tick mult",
            0.5,
        ...
        tempAccTickMult = toolConfig.getDouble(
            "Temporal Accelerator - Tick mult",
            0.2,
        ...
        dirtBoneChance = toolConfig.getDouble(
            "Dirt block - Bonemeal chance",
            0.25,
        ...
        visibleSchema = toolConfig
            ...
            .getBoolean();
        visibleForceField = toolConfig
            .getBoolean("Visibility - forcefields", false, "Should the forcefields be visible or not");
    ...
    private static void refreshMechanicsConfig() {
        ...
        keyOnFirstJoin = mechConfig
            .getBoolean("Key - On first join", false, "If true, all players get a new key when they first join");
        kontronRarity = mechConfig.getInt(
            "Dungeon Loot - Kontron Rarity",
            20,
            "The higher this value, the more likely you are to find kontron crystals in chests");
        deleteDisconnected = mechConfig.getBoolean(
            "Rooms - Delete disconnected",
            true,
            "Delete rooms which aren't connected to the console room when the connecting room is deleted");
        deathTransmatLive = mechConfig
            .getBoolean("Transmat - Death - Live after", true, "Whether you should live after being saved from death");
        deathTransmat = mechConfig.getBoolean(
            "Transmat - Death",
            true,
            "If true, when you die within range of your TARDIS you will be transmatted");
        transmatExitDist = mechConfig.getDouble(
            "Transmat - Exit distance",
            2,
            "The distance from the transmat point within which you will be transmatted out of the TARDIS");
        keyInHand = mechConfig
            .getConfigItem(
                new ConfigItem(
                    "Key - In hand",
                    CType.BOOLEAN,
            ...
            .getBoolean();
        lockSoundDelay = mechConfig.getInt(
            "Lock sound delay",
            40,
        ...
        enableLinking = mechConfig.getBoolean(
            "Enable screwdriver linking mode",
            true,
            "Enable the linking mode on the sonic",
            "Note: WIP. Some linking features are still buggy");
        deleteAllOwnerOnly = mechConfig.getBoolean(
            "Delete all rooms owner only",
            false,
            "Whether or not the delete all rooms button should be owner only");
        enableTardisMobSpawning = mechConfig
            .getBoolean("Enable mob spawning", false, "Whether or not mobs are able to spawn in the TARDIS dimension");

        String dimIds = mechConfig.getString(
            "Dimension ID's locked by upgrades",
            "",
    ...
    public static TardisOutput.Priority priorityLevel = TardisOutput.Priority.WARNING;
    ...
    private static void refreshModConfig() {
        ...
        priorityLevel = TardisOutput.getPriority(
            modConfig.getInt("Debug level", TardisOutput.Priority.WARNING.ordinal(), "Sets the level of debug output"));
        providerID = modConfig.getInt("Dimension provider ID", 54, "The id of the dimension provider");
        tardisLoaded = modConfig
            .getBoolean("Dimension always loaded", true, "Should the TARDIS dimensions always be loaded");
        tardisVol = modConfig
            .getDouble("Volume", 1, "How loud should Tardis Mod sounds be (1.0 = full volume, 0.0 = no volume)");
        exteriorGenChunksRad = MathHelper.clamp(
            modConfig
                .getInt("exterior chunk gen radius", 8, "Radius in chunks for the exterior to generate while landing"),
            -1,
            10);
        exteriorGenChunksPT = MathHelper.clamp(
            modConfig
                .getInt("exterior chunk gen per pulse", 1, "Number of chunks for the exterior to generate per pulse"),
            1,
            20);
        exteriorGenChunksTR = MathHelper.clamp(
            modConfig
                .getInt("exterior chunk gen ticks per pulse", 4, "Number of ticks between chunk generation pulses"),
            1,
            20);
        consoleBiomeID = modConfig
            .getInt("Console Room Dimension ID", 42, "The id of the biome which is used in the console room");
