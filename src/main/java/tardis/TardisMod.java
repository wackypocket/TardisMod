...
import tardis.common.core.TardisLogging;
...
import tardis.common.core.events.internal.RuntimeLifecycleEventHandler;
...
import tardis.common.integration.waila.WailaIntegration;
...
@Mod(
    modid = "TardisMod",
    name = "Tardis Mod Legacy",
    version = TardisMod.VERSION,
    dependencies = "required-after:FML; required-after:darkcore; after:Thaumcraft; after:CoFHCore; after:appliedenergistics2; after:Waila; before:DragonAPI")
public class TardisMod implements IConfigHandlerMod {
    ...
    public static ChameleonRegistry<AbstractTardisChameleon> tardisChameleonReg = new ChameleonRegistry<>(
        new DefaultTardisCham());

    @EventHandler
    public void preInit(FMLPreInitializationEvent event) throws IOException {
        i = this; // Ensure TardisMod.i is assigned to an instance of TardisMod
        io.darkcraft.darkcore.mod.logging.TMLLog.alwaysInfo("Geronimo!");
        configHandler = ConfigHandlerFactory.getConfigHandler(this);
        refreshConfigs();
        TardisLogging.configureLogging();
        schemaHandler = new SchemaHandler(configHandler);
        schemaHandler.getSchemas();
        AEIntegration.initialize();
        ThaumcraftIntegration.initialize();
        WailaIntegration.initialize();
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(ScrewdriverHelperFactory.i);
        FMLCommonHandler.instance()
            .bus()
            .register(RuntimeLifecycleEventHandler.i);
    }

    @EventHandler
    public void serverAboutToStart(FMLServerAboutToStartEvent event) {
        if (otherDims != null) {
            otherDims.stopScanning();
            MinecraftForge.EVENT_BUS.unregister(otherDims);
        }
        TardisRuntimeCleanup.clearRuntimeState();
    ...
    public void serverStarting(FMLServerStartingEvent event) {
        ...
        teleporter = new DarkcoreTeleporter(
            event.getServer()
    ...
    public void serverStopped(FMLServerStoppedEvent event) {
        ...
        otherDims = null;
        // During shutdown, only clear registry/cache objects. Unregistering dimensions
        // here can race vanilla save/stop logic in integrated server.
