...
import tardis.common.items.SecretUpgradeItem;
...
public class TMRegistry {
    ...
    public static ColorableRoundelBlock colorableRoundelBlock;
    ...
    public static SecretUpgradeItem secretUpgradeItem;
    ...
    public static void init() {
        ...
        unbreakableBlocks = Sets.newHashSet(
            tardisBlock,
            tardisTopBlock,
            tardisCoreBlock,
            tardisConsoleBlock,
            ...
            colorableBrickBlock,
            colorablePlankBlock,
            colorableRoundelBlock,
            colorableOpenRoundelBlock,
            manualBlock,
            manualHelperBlock,
            shieldBlock,
            temporalAccelerator);

        if (!Configs.dimUpgradesIds[0].isEmpty()) for (int i = 0; i < Configs.dimUpgradesIds.length; i++) {
            try {
                dimensionUpgradeItems.put(
                    Integer.parseInt(Configs.dimUpgradesIds[i]),
                    new DimensionUpgradeItem(Integer.parseInt(Configs.dimUpgradesIds[i])).register());
            } catch (Throwable e) {
                e.printStackTrace();
            }
        }
