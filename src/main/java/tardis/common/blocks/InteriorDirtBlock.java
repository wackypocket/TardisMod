...
public class InteriorDirtBlock extends AbstractBlock implements IScrewablePrecise {
    ...
    public void initRecipes() {
        if (Configs.numDirtRecipe > 0) LabRecipeRegistry.addRecipe(
            new LabRecipe(
                "tm.magicdirt",
                new ItemStack[] { new ItemStack(Blocks.dirt, 64), CraftingComponentType.KONTRON.getIS(1),
                    CraftingComponentType.CHRONOSTEEL.getIS(1), new ItemStack(Items.dye, 32, 15) },
    ...
    public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction,
        ...
        if (Helper.isTardisWorld(world)) return true;
        return false;
    }
    ...
    public int getNewTickRate(int old) {
        return Math.max(1, MathHelper.ceil(old * Configs.dirtTickMult));
    }
    ...
    public void updateTick(World w, int x, int y, int z, Random rand) {
        ...
        if (b != null) {
            if (b instanceof InteriorDirtBlock) return;
            ...
            if ((core != null) && ((core.tt % getNewTickRate(b.tickRate(w))) == 0)) {
                if (w instanceof WorldServer) {
                    ...
                    if (rand.nextDouble() <= Configs.dirtBoneChance) ItemDye.applyBonemeal(is, w, x, y + 1, z, pl);
                    int i = 1;
