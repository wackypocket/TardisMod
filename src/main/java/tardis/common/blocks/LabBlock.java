...
public class LabBlock extends AbstractBlockContainer {
    ...
    public void initRecipes() {
        GameRegistry.addRecipe(
            new ShapedOreRecipe(
                new ItemStack(TMRegistry.labBlock, 1, 0),
                true,
                "igi",
                "ioi",
                "iri",
    ...
    public void breakBlock(World w, int x, int y, int z, Block b, int m) {
        ...
        if (te instanceof LabTileEntity) ((LabTileEntity) te).dropEverything();
    ...
    public int getComparatorInputOverride(World world, int x, int y, int z, int par5) {
        int power = 0;
        for (ItemStack is : ((LabTileEntity) world.getTileEntity(x, y, z)).getInputSlots()) if (is != null) power++;
