...
public class CoreBlock extends AbstractBlockContainer {
    ...
    public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int meta) {
        if (Helper.isTardisWorld(world) && (x == Helper.tardisCoreX)
            && (y == Helper.tardisCoreY)
            && (z == Helper.tardisCoreZ)) {
            TileEntity te = world.getTileEntity(x, y, z);
            NBTTagCompound nbt = new NBTTagCompound();
            if (te != null) te.writeToNBT(nbt);
            world.setBlock(x, y, z, this);
            TileEntity newTE = world.getTileEntity(x, y, z);
            if ((te != null) && (newTE != null)) newTE.readFromNBT(nbt);
        }
    }
    ...
    public boolean canPlaceBlockAt(World w, int x, int y, int z) {
        if (!Helper.isTardisWorld(w)) return false;
            ...
            if ((x != Helper.tardisCoreX) || (y != Helper.tardisCoreY) || (z != Helper.tardisCoreZ)) return false;
            return true;
        }
    }
}
