...
public class SchemaComponentBlock extends AbstractBlock {
    ...
    private static boolean isOpaqueDoorMeta(int meta) {
        return (meta == 0) || (meta == 4) || (meta == 5);
    }
    ...
    public int getLightOpacity(IBlockAccess w, int x, int y, int z) {
        int meta = w.getBlockMetadata(x, y, z);
        return isOpaqueDoorMeta(meta) ? 255 : 0;
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess w, int x, int y, int z, int s) {
        ...
        if ((md == 1) || (md == 3) || (md == 6) || (md == 7)) return false;
        if (isOpaqueDoorMeta(md)) return super.shouldSideBeRendered(w, x, y, z, s);
        return true;
    }

    public static boolean isDoorConnector(World w, int x, int y, int z) {
        if ((w.getBlock(x, y, z) == TMRegistry.schemaComponentBlock) && (w.getBlockMetadata(x, y, z) <= 1)) return true;
        return false;
    }
    ...
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer pl, int s, float t, float j, float k) {
        ...
        if (((meta == 6) && (j > 0.4) && ser) || (meta == 8)) {
            ...
            if (core != null) core.activate(pl, s);
            return true;
        }
        if (meta == 2) {
            boolean found = false;
            for (int i = 1; (i < 10) && (found == false); i++) {
                if (w.getBlock(x, y - i, z) == TMRegistry.schemaCoreBlock) {
                    if (TMRegistry.screwItem.handleBlock(new SimpleCoordStore(w, x, y - i, z), pl)) {
                        ...
                        if (te instanceof SchemaCoreTileEntity) ((SchemaCoreTileEntity) te).activate(pl, s);
                        return true;
                    }
                }
            }
        ...
        if ((meta == 3) || (meta == 6)) {
            if (!ser) {
                ...
                if (console != null) return console.activate(pl, x, y, z, t, j, k);
            }
            return true;
        }
        if (((meta == 4) || (meta == 5)) && ser) {
            ...
            if (te != null) {
                if ((ServerHelper.isServer()) && (!te.changeLock(pl, true))) te.leaveTardis(pl, false);
            }
            return true;
        }
        if (meta == 7) {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess w, int x, int y, int z) {
        if (w.getBlockMetadata(x, y, z) == 3) setBlockBounds(0, 0.5F, 0, 1, 1, 1);
        else if (w.getBlockMetadata(x, y, z) == 1) setBlockBounds(0, 0, 0, 0, 0, 0);
    ...
    public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int meta) {
        ...
        if (ServerHelper.isServer() && (x >= -1) && (x <= 1) && (z >= -1) && (z <= 1)) Helper.repairConsole(world);
    }
