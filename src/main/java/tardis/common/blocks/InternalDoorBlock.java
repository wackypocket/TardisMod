...
public class InternalDoorBlock extends AbstractBlock implements ILinkable {
    ...
    public String getSubName(int num) {
        if ((num % 8) < 4) return super.getSubName(0);
        else return super.getSubName(1);
    }

    @Override
    public void getSubBlocks(Item itemID, CreativeTabs tab, List<ItemStack> itemList) {
    ...
    public static int dx(int myFace) {
        if ((myFace % 4) == 0) return -1;
        if ((myFace % 4) == 2) return 1;
        return 0;
    }

    public static int dz(int myFace) {
        if ((myFace % 4) == 1) return -1;
        if ((myFace % 4) == 3) return 1;
        return 0;
    }

    @Override
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer player, int side, float i, float j,
        float k) {
        if (player != null) {
                            ...
                            if ((ds == null) || (ds.hasPermission(player, TardisPermission.ROOMS))) {
                                if (help.getSchemaName() != null) {
                                    ...
                                    if (pb.roomFor(w, nX, nY, nZ, opposingFace(facing))) {
                                        ...
                                        if ((te == null) || te.addRoom(false, null)) // pass null as arg for schemacore
                                                                                     // since it adds itself
                                        {
                                            pb.reconstitute(w, nX, nY, nZ, opposingFace(facing));
                                            ServerHelper.sendString(
                                                player,
                                                "Room counter after creation: " + (te.getNumRooms() + 1)
                                                    + "/"
                                                    + te.getMaxNumRooms());
                                        } else player
                                            .addChatMessage(new ChatComponentText("Too many rooms in this TARDIS"));
                                    } else {
                                        TardisOutput.print(
                                            "TIDB",
                                            "NoRoom:" + nX + "," + nY + "," + nZ,
                                ...
                                } else player.addChatMessage(new ChatComponentText("No schematic loaded"));
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World w, int x, int y, int z) {
        ...
        } else return super.getCollisionBoundingBoxFromPool(w, x, y, z);
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess w, int x, int y, int z, int s) {
        ...
        if (w.getBlockMetadata(x, y, z) >= 8) return false;
        return true;
    }
    ...
    public static boolean hasConnector(World w, int x, int y, int z) {
        if (w.getBlock(x, y, z) == TMRegistry.internalDoorBlock) {
            ...
            if (w.getBlock(connectedDoorX, y, connectedDoorZ) == TMRegistry.internalDoorBlock)
                if ((w.getBlockMetadata(connectedDoorX, y, connectedDoorZ) % 4) == opposingFace(facing)) return true;
        }
        return false;
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess w, int x, int y, int z) {
        if (w.getBlockMetadata(x, y, z) >= 8) setBlockBounds(0, 0, 0, 0, 0, 0);
    ...
    public void addCollisionBoxesToList(World w, int x, int y, int z, AxisAlignedBB par5AxisAlignedBB, List par6List,
        Entity par7Entity) {
    ...
    public boolean link(EntityPlayer pl, SimpleCoordStore link, SimpleCoordStore other) {
        ...
        // Do not allow linking if either door is the "Primary Door" variant (metadata % 8 >= 4)
        int linkMeta = link.getMetadata() % 8;
        int otherMeta = other.getMetadata() % 8;
        if ((linkMeta >= 4) || (otherMeta >= 4)) return false;

        // Only allow linking within the same world
        if (link.world != other.world) {
            if (ServerHelper.isServer())
                ServerHelper.sendString(pl, "Linking failed", "You must be inside your own TARDIS to link");
            return false;
        }

        // Require that the player is inside their own TARDIS dimension
        if (TardisMod.plReg == null) return false;
        Integer ownerDim = TardisMod.plReg.getDimension(pl);
        if (ownerDim == null) {
            if (ServerHelper.isServer()) ServerHelper.sendString(pl, "Linking failed", "You do not own a TARDIS");
            return false;
        }
        int playerDim = WorldHelper.getWorldID(pl.worldObj);
        if (ownerDim.intValue() != playerDim) {
            if (ServerHelper.isServer())
                ServerHelper.sendString(pl, "Linking failed", "You must be inside your own TARDIS to link");
            return false;
        }

        // Permissions: require permission in both data stores (if present)
        TardisDataStore dsLink = Helper.getDataStore(link.getWorldObj());
        TardisDataStore dsOther = Helper.getDataStore(other.getWorldObj());
        if (!((dsLink == null) || dsLink.hasPermission(pl, TardisPermission.ROOMS))) return false;
        if (!((dsOther == null) || dsOther.hasPermission(pl, TardisPermission.ROOMS))) return false;
        ...
        if (linkMap.get(other) == link) {
            CoreTileEntity c = Helper.getTardisCore(link.getWorldObj());
            if (c != null) c.refreshDoors(false);
        }
        return true;
    }
    ...
    public boolean isLinkable(SimpleCoordStore link) {
        ...
        // Doors with metadata % 8 >= 4 are the rear (red) variant — not linkable
        int meta = link.getMetadata() % 8;
        return meta < 4;
    }
