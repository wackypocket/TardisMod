...
public class TardisBlock extends AbstractBlockContainer {
    ...
    public void addCollisionBoxesToList(World w, int x, int y, int z, AxisAlignedBB aabb, List l, Entity e) {
        ...
        if (te instanceof TardisTileEntity) if (((TardisTileEntity) te).isLanding()) return;
        super.addCollisionBoxesToList(w, x, y, z, aabb, l, e);
    }

    // OTHER THINGS
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int i, float j, float k,
        float l) {
        super.onBlockActivated(world, x, y, z, player, i, j, k, l);
        int myMeta = world.getBlockMetadata(x, y, z);
        if (((myMeta == 0) && (i == 2)) || ((myMeta == 1) && (i == 5))
            || ((myMeta == 2) && (i == 3))
            || ((myMeta == 3) && (i == 4))) {
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        if (world.isAirBlock(x, y, z) && world.isAirBlock(x, y + 1, z)) return true;
        return false;
    }
    ...
    public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int meta) {
        ...
        if (world.getBlock(x, y + 1, z) == TMRegistry.tardisTopBlock) world.setBlockToAir(x, y + 1, z);
    }
