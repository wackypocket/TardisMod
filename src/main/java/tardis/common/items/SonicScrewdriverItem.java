...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "buildcraft.api.tools.IToolWrench", modid = "BuildCraftAPI|core"),
...
public class SonicScrewdriverItem extends AbstractItem implements IToolHammer, IToolWrench, IScrewdriver {
    ...
    private void addModeInfo(ScrewdriverMode mode, ScrewdriverHelper helper, List<String> infoList) {
        infoList.add("Mode: " + mode.toString());
        if (mode == ScrewdriverMode.Schematic) infoList.add("Schematic: " + helper.getSchemaDisplay());
        if (mode == ScrewdriverMode.Link) {
            SimpleCoordStore link = helper.getLinkSCS();
            if (link == null) infoList.add("Link: Target not set");
            else infoList.add("Link: " + link.toString());
        }
    }
    ...
    public static boolean isPlayerHoldingScrewdriver(EntityPlayer pl) {
        if (pl != null) return isScrewdriver(pl.getHeldItem());
        return false;
    }
    ...
    public void notifyMode(ScrewdriverHelper helper, EntityPlayer player, boolean override) {
        ...
        if (!(override || helper.isModeValid(player, mode))) helper.switchMode(player);
        ArrayList<String> list = new ArrayList<String>();
        addModeInfo(mode, helper, list);
        for (String s : list) {
            ServerHelper.sendString(player, screwName, s, EnumChatFormatting.AQUA);
        }
    }
    ...
    private boolean dismantle(Object o, SimpleCoordStore pos, EntityPlayer player) {
        ...
        if (dis.canDismantle(player, pos.getWorldObj(), pos.x, pos.y, pos.z)) {
            ...
            for (ItemStack tis : s) if (tis != null) WorldHelper.giveItemStack(player, tis);
    ...
    private boolean tardisDismantle(ITDismantleable dis, SimpleCoordStore scs, EntityPlayer pl) {
        if (dis.canDismantle(scs, pl)) {
            ...
            for (ItemStack tis : s) if (tis != null) WorldHelper.giveItemStack(pl, tis);
    ...
    private boolean screwScrewable(Object screw, ScrewdriverHelper helper, ScrewdriverMode mode, EntityPlayer player,
        SimpleCoordStore pos) {
    ...
    public boolean handleBlock(SimpleCoordStore pos, EntityPlayer pl) {
        ...
        if (mode == ScrewdriverMode.Dismantle) {
            if (te instanceof ITDismantleable) if (tardisDismantle((ITDismantleable) te, pos, pl)) return true;
            if (Loader.isModLoaded("CoFHCore")) if (dismantle(te, pos, pl) || dismantle(b, pos, pl)) return true;
            ...
            if (b == TMRegistry.colorableRoundelBlock) {
                ...
                if ((ds == null) || ds.hasPermission(pl, TardisPermission.ROUNDEL)) {
                    pos.getWorldObj()
                        .setBlock(pos.x, pos.y, pos.z, TMRegistry.colorableOpenRoundelBlock, m, 3);
                    toolUsed(is, pl, pos.x, pos.y, pos.z);
                    return true;
                } else pl.addChatMessage(CoreTileEntity.cannotModifyRoundel);
    ...
    private boolean rightClickBlock(ItemStack is, ScrewdriverMode mode, EntityPlayer player, World w) {
        if (ServerHelper.isServer()) {
            ...
            if (r) toolUsed(is, player, pos.x, pos.y, pos.z);
            return r;
        }
        return false;
    }
    ...
    @Override
    public boolean onItemUse(ItemStack is, EntityPlayer pl, World w, int x, int y, int z, int s, float i, float j,
        float k) {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        ...
        if (ServerHelper.isServer() && !player.isSneaking()) {
            ...
            if (mode.equals(ScrewdriverMode.Locate) && ServerHelper.isServer()) {
                            ...
                            if (ext != null) {
                                if (WorldHelper.getWorldID(ext) != player.worldObj.provider.dimensionId)
                                    ServerHelper.sendString(
                                        player,
                                        screwName,
                                        "The TARDIS does not appear to be in this dimension");
                                else ServerHelper.sendString(
                                    player,
                                    screwName,
                                    "The TARDIS is at [" + ext.xCoord + "," + ext.yCoord + "," + ext.zCoord + "]");
                            }
                        }
                    }
                } else ServerHelper.sendString(player, screwName, "The TARDIS could not be located");
                ...
                if (core.inAbortableFlight() && !core.inCoordinatedFlight()) {
                    if (core.attemptToLand()) ServerHelper.sendString(player, screwName, "TARDIS landing");
                    else ServerHelper.sendString(player, screwName, "TARDIS recall failed");
                } else if ((con != null) && !core.inFlight()) {
                    if (con.setControls(
                        WorldHelper.getWorldID(player.worldObj),
                        (int) Math.floor(player.posX + 1),
            ...
            if (mode.equals(ScrewdriverMode.Dismantle) || mode.equals(ScrewdriverMode.Reconfigure)) {
                if (!rightClickBlock(is, mode, player, world)) helper.switchMode(player);
            } else helper.switchMode(player);
        }
        if ((mode != helper.getMode()) && ServerHelper.isServer()) notifyMode(helper, player, true);
        if (helper.isDirty()) player.inventory.markDirty();
    ...
    public boolean onLeftClickEntity(ItemStack is, EntityPlayer hitter, Entity hit) {
        ...
        if ((helper.getMode() == ScrewdriverMode.Transmat) && !(hit instanceof EntityPlayer)
            && (hit instanceof EntityLivingBase)) {
            TardisDataStore ds = helper.getLinkedDS();
    ...
    public void toolUsed(ItemStack is, EntityLivingBase player, int x, int y, int z) {
        if (ServerHelper.isClient()) return;
        float speed = (float) (player.getRNG()
            .nextDouble() * 0.5) + 0.75f;
    ...
    public boolean canUse(EntityPlayer pl, ItemStack is) {
        if (getHelper(is).getMode() == ScrewdriverMode.Reconfigure) return true;
        return false;
    }
    ...
    public static ScrewdriverMode getMode(ItemStack is) {
        ScrewdriverHelper helper = getHelper(is);
        if (helper != null) return helper.getMode();
        return null;
    }
