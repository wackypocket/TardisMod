...
public enum DecoratorToolTypes {
    ...
    public void set(SimpleCoordStore pos) {
        ...
        if (c == null) return;
        if (meta == -1) {
            if (c.meta == -1) pos.setBlock(b, pos.getMetadata(), 3);
            else pos.setBlock(b, b instanceof IColorableBlock ? 15 : 0, 3);
        } else pos.setBlock(b, meta, 3);
    }
    ...
    public static DecoratorToolTypes getMatching(SimpleCoordStore pos) {
        for (DecoratorToolTypes t : values()) if (t.match(pos)) return t;
        return null;
    }
