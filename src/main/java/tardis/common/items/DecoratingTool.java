...
public class DecoratingTool extends AbstractItem {
    ...
    private static HashMap<Integer, Long> lastChangeMap = new HashMap<Integer, Long>();
    ...
    public SimpleCoordStore getPlayerLookingAt(EntityPlayer pl) {
        ...
        if ((hitPos != null) && (hitPos.typeOfHit == MovingObjectType.BLOCK)) return new SimpleCoordStore(w, hitPos);
        return null;
    }

    private void playSound(EntityPlayer player) {
        if (ServerHelper.isServer()) SoundHelper.playSound(player, "tardismod:decorate", 1, 1);
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World w, EntityPlayer pl) {
        if (pl.isSneaking()) {
            ...
            if (ServerHelper.isServer()) ServerHelper.sendString(pl, "Decorator", "New Block: " + t.getName());
            return is;
        }
