...
public class KeyItem extends AbstractItem {
    ...
    public static String getOwnerName(ItemStack is) {
        if (is != null) {
            if (is.getItem() instanceof KeyItem) {
                ...
                return io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(data, "keyOwner", null);
            }
        }
        return null;
    }
    ...
    public void addInfo(ItemStack is, EntityPlayer player, List<String> infoList) {
        if (is != null) {
            ...
            if (owner != null) infoList.add("Owner: " + owner);
            else {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        if ((is != null) && (getOwnerName(is) == null) && !(player instanceof FakePlayer))
            setOwnerName(is, ServerHelper.getUsername(player));

        if (ServerHelper.isServer()) {
            if (!(player instanceof FakePlayer) && !TardisMod.plReg.hasTardis(getOwnerName(is))
                && ServerHelper.getUsername(player)
                    .equals(getOwnerName(is))) {
                ...
                if ((on != null) && !on.isEmpty()) {
                    ...
                    if ((core != null) && (ds != null)) {
                        if (!Helper.isTardisWorld(world)) {
                            if (ServerHelper.getUsername(player)
                                .equals(on) && !ds.hasValidExterior()
                                && !core.inFlight()) {
                            ...
                            if (ds.hasPermission(player, TardisPermission.FLY) && player.isSneaking()
                                && core.hasFunction(TardisFunction.STABILISE)) {
                                SimpleDoubleCoordStore extPos = core.getPosition()
                                    .getCenter();
                                if (extPos.distance(player) < 8) {
                                    if (!core.inFlight()) core.takeOffUncoordinated(player);
    ...
    public void initRecipes() {
        if (Configs.keyCraftable) {
            if (Configs.keyReqKontron) GameRegistry.addRecipe(
                new ShapedOreRecipe(
                    new ItemStack(TMRegistry.keyItem, 1),
                    true,
                    " i ",
                    " ik",
                    " ii",
