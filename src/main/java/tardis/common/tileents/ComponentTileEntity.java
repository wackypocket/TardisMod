...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "cofh.api.energy.IEnergyHandler", modid = CofHCore.modname),
        @Optional.Interface(iface = "appeng.api.networking.IGridHost", modid = AEIntegration.MOD_ID),
        @Optional.Interface(iface = "thaumcraft.api.aspects.IEssentiaTransport", modid = ThaumcraftIntegration.MOD_ID),
...
public class ComponentTileEntity extends AbstractTileEntity implements IActivatablePrecise, IBlockUpdateDetector,
    ...
    public boolean isComponentValid(TardisTEComponent comp) {
        if (inside == null) inside = Helper.isTardisWorld(worldObj);
    ...
    public int getNumComponents() {
        return comps == null ? 0 : comps.size();
    }

    public boolean hasComponent(TardisTEComponent comp) {
        if (comps != null) return comps.containsKey(comp.ordinal());
        return false;
    }

    public ITardisComponent getComponent(TardisTEComponent comp) {
        if (hasComponent(comp)) return comps.get(comp.ordinal());
        return null;
    }
    ...
    public void updateEntity() {
        ...
        if ((comps != null) && !comps.isEmpty()) {
            for (ITardisComponent comp : comps.values()) comp.updateTick();
        }
    }
    ...
    protected void dismantle(EntityPlayer pl) {
        ...
        if (contained.length > 0) {
            for (ItemStack is : contained) WorldHelper.giveItemStack(pl, is);
        }
    ...
    public boolean screw(ScrewdriverMode mode, EntityPlayer player) {
        ...
        if (mode == ScrewdriverMode.Dismantle) {
            ...
            if ((ds == null) || ds.hasPermission(player, TardisPermission.ROUNDEL)) {
                dismantle(player);
                return true;
            } else player.addChatMessage(CoreTileEntity.cannotModifyMessage);
        }
    ...
    private ITardisComponent getComponent(int slot) {
        {
            int i = 0;
            for (ITardisComponent c : comps.values()) if (i++ == slot) return c;
        }
        return null;
    }
                    ...
                    if ((dam >= 0) && (dam < possComps.length)) {
                        ...
                        if (isComponentValid(rel)) {
                            if (addComponent(rel)) {
                                ...
                                return true;
                            } else ServerHelper.sendString(pl, "That component has already been fitted");
                        } else ServerHelper.sendString(
                            pl,
                            "That component is not valid " + (inside ? "inside" : "outside of") + " a TARDIS");
                    }
                } else ServerHelper.sendString(pl, "You do not have permission to modify this");
            }
        }
    ...
    public void blockUpdated(Block neighbourBlock) {
        if ((comps != null) && (comps.size() > 0)) {
            for (ITardisComponent te : comps.values()) {
                if (te instanceof IBlockUpdateDetector) ((IBlockUpdateDetector) te).blockUpdated(neighbourBlock);
            }
        }
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (comps == null) comps = new HashMap<Integer, ITardisComponent>();
        TardisTEComponent[] possibleComps = TardisTEComponent.values();
        for (TardisTEComponent comp : possibleComps) {
            if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, comp.componentName)) {
                addComponent(comp);
                ITardisComponent te = getComponent(comp);
                if (te != null) {
                    te.readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, comp.componentName));
                }
            }
        }
    }
    ...
    public void writeTransmittable(NBTTagCompound nbt) {
        String c = "";
        if (comps != null) for (Integer i : comps.keySet()) c += i + ",";
        nbt.setString("c", c);
    }
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        if (comps == null) comps = new HashMap<Integer, ITardisComponent>();
        String c = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "c", "");
        String[] d = c.split(",");
        if (d.length > 0) {
    ...
    public boolean canConnectEnergy(ForgeDirection from) {
        if (!valid || compAdded) return true;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergyHandler) return ((IEnergyHandler) comp).canConnectEnergy(from);
        }
        return false;
    }
    ...
    public int getEnergyStored(ForgeDirection from) {
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergyHandler) return ((IEnergyHandler) comp).getEnergyStored(from);
        }
        return 0;
    }
    ...
    public int getMaxEnergyStored(ForgeDirection from) {
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergyHandler) return ((IEnergyHandler) comp).getMaxEnergyStored(from);
        }
        return 0;
    }
    ...
    public int getSizeInventory() {
        if (!valid || compAdded) return 0;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) return ((IInventory) comp).getSizeInventory();
        }
        return 0;
    }
    ...
    public ItemStack getStackInSlot(int i) {
        if (!valid || compAdded) return null;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) return ((IInventory) comp).getStackInSlot(i);
        }
        return null;
    }
    ...
    public ItemStack decrStackSize(int i, int j) {
        if (!valid || compAdded) return null;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) return ((IInventory) comp).decrStackSize(i, j);
        }
        return null;
    }
    ...
    public void setInventorySlotContents(int i, ItemStack itemstack) {
        if (!valid || compAdded) return;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) ((IInventory) comp).setInventorySlotContents(i, itemstack);
        }
    }
    ...
    public String getInventoryName() {
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) return ((IInventory) comp).getInventoryName();
        }
        return null;
    }
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (!valid || compAdded) return false;
        for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IInventory) return ((IInventory) comp).isItemValidForSlot(i, itemstack);
        }
        return false;
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).fill(from, resource, doFill);
        }
        return 0;
    }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).drain(from, resource, doDrain);
        }
        return null;
    }
    ...
    public FluidStack drain(ForgeDirection from, int maxDrain, boolean doDrain) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).drain(from, maxDrain, doDrain);
        }
        return null;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).canFill(from, fluid);
        }
        return false;
    }
    ...
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).canDrain(from, fluid);
        }
        return false;
    }
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IFluidHandler) return ((IFluidHandler) comp).getTankInfo(from);
        }
        return null;
    }
    ...
    public boolean shouldChunkload() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IChunkLoader) return ((IChunkLoader) comp).shouldChunkload();
        }
        return false;
    }
    ...
    public ChunkCoordIntPair[] loadable() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IChunkLoader) return ((IChunkLoader) comp).loadable();
        }
        return null;
    }
    ...
    public IGridNode getGridNode(ForgeDirection dir) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IGridHost) return ((IGridHost) comp).getGridNode(dir);
        }
        return null;
    }
    ...
    public AECableType getCableConnectionType(ForgeDirection dir) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IGridHost) return ((IGridHost) comp).getCableConnectionType(dir);
        }
    ...
    public void securityBreak() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IGridHost) ((IGridHost) comp).securityBreak();
        }
    }

    @Optional.Method(modid = ThaumcraftIntegration.MOD_ID)
    public AspectList getAspects() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).getAspects();
        }
        return null;
    }
    ...
    public void setAspects(AspectList aspects) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) ((IAspectSource) comp).setAspects(aspects);
        }
    }
    ...
    public boolean doesContainerAccept(Aspect tag) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).doesContainerAccept(tag);
        }
        return false;
    }
    ...
    public int addToContainer(Aspect tag, int amount) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).addToContainer(tag, amount);
        }
        return amount;
    }
    ...
    public boolean takeFromContainer(Aspect tag, int amount) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).takeFromContainer(tag, amount);
        }
        return false;
    }
    ...
    public boolean takeFromContainer(AspectList ot) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).takeFromContainer(ot);
        }
        return false;
    }
    ...
    public boolean doesContainerContainAmount(Aspect tag, int amount) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).doesContainerContainAmount(tag, amount);
        }
        return false;
    }
    ...
    public boolean doesContainerContain(AspectList ot) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).doesContainerContain(ot);
        }
        return false;
    }
    ...
    public int containerContains(Aspect tag) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return ((IAspectSource) comp).containerContains(tag);
        }
        return 0;
    }
    ...
    public boolean isConnectable(ForgeDirection face) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return true;
        }
        return false;
    }
    ...
    public Aspect getSuctionType(ForgeDirection face) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof ComponentAspect) return ((ComponentAspect) comp).getSuctionAspect();
        }
        return null;
    }
    ...
    public int getSuctionAmount(ForgeDirection face) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof ComponentAspect) return ((ComponentAspect) comp).getSuction();
        }
        return 0;
    }
    ...
    public int addEssentia(Aspect aspect, int amount, ForgeDirection face) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IAspectSource) return amount - ((IAspectSource) comp).addToContainer(aspect, amount);
        }
        return 0;
    }
    ...
    public boolean acceptsEnergyFrom(TileEntity emitter, ForgeDirection direction) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySink) return ((IEnergySink) comp).acceptsEnergyFrom(emitter, direction);
        }
        return false;
    }
    ...
    public double getDemandedEnergy() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySink) return ((IEnergySink) comp).getDemandedEnergy();
        }
        return 0;
    }
    ...
    public int getSinkTier() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySink) return ((IEnergySink) comp).getSinkTier();
        }
        return 0;
    }
    ...
    public double injectEnergy(ForgeDirection directionFrom, double amount, double voltage) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySink) return ((IEnergySink) comp).injectEnergy(directionFrom, amount, voltage);
        }
        return amount;
    }
    ...
    public boolean emitsEnergyTo(TileEntity receiver, ForgeDirection direction) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySource) return ((IEnergySource) comp).emitsEnergyTo(receiver, direction);
        }
        return false;
    }
    ...
    public double getOfferedEnergy() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySource) return ((IEnergySource) comp).getOfferedEnergy();
        }
        return 0;
    }
    ...
    public void drawEnergy(double amount) {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySource) ((IEnergySource) comp).drawEnergy(amount);
        }
    }
    ...
    public int getSourceTier() {
        if (valid && !compAdded) for (ITardisComponent comp : comps.values()) {
            if (comp instanceof IEnergySource) return ((IEnergySource) comp).getSourceTier();
        }
        return 0;
    }
