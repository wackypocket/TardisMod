...
public class LabRecipe {
    ...
    public LabRecipe(String _id, ItemStack[] _source, ItemStack[] _dest, EnumSet<LabFlag> _flags, int _enCost) {
        ...
        for (ItemStack is : dest) if (is != null) output = is;
    ...
    public boolean isValid() {
        ...
        if ((source == null) || (dest == null) || (flags == null)) return false;
        if ((source.length == 0) || (dest.length == 0) || (source.length > 5) || (dest.length > 5)) return false;
        for (int i = 0; i < source.length; i++) if (source[i] == null) return false;
        for (int i = 0; i < dest.length; i++) if (dest[i] == null) return false;
        if (flags.contains(LabFlag.NOTINFLIGHT)
            && (flags.contains(LabFlag.INFLIGHT) || flags.contains(LabFlag.INCOORDINATEDFLIGHT)
                || flags.contains(LabFlag.INUNCOORDINATEDFLIGHT)))
            return false;
        if (flags.contains(LabFlag.INFLIGHT)
            && (flags.contains(LabFlag.INCOORDINATEDFLIGHT) || flags.contains(LabFlag.INUNCOORDINATEDFLIGHT)))
            return false;
        if (flags.contains(LabFlag.INCOORDINATEDFLIGHT) && flags.contains(LabFlag.INUNCOORDINATEDFLIGHT)) return false;
        return true;
    }
    ...
    public boolean isSatisfied(ItemStack[] items) {
        ...
        for (ItemStack is : source) {
            ...
            for (ItemStack compIS : items) {
                if (compIS == null) continue;
                if (!WorldHelper.sameItem(compIS, is)) continue;
    ...
    public boolean flagsSatisfied(IArtronEnergyProvider core) {
        ...
        boolean sat = true;
        for (LabFlag flag : flags) sat = sat && core.doesSatisfyFlag(flag);
        return sat;
    }
}
