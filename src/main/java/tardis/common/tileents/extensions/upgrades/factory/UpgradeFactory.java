...
import tardis.common.tileents.extensions.upgrades.SecretUpgrade;
...
public class UpgradeFactory implements IUpgradeFactory {
    ...
    public AbstractUpgrade create(NBTTagCompound nbt) {
        ...
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "id")) {
            String id = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "id", "");
            if (id.equals("protExpl")) return new DamReduceExplosion(nbt);
            if (id.equals("protComb")) return new DamReduceCombat(nbt);
            if (id.equals("protMiss")) return new DamReduceMissedControl(nbt);
            if (id.equals("levelUp")) return new LevelUpgrade(
                TardisUpgradeMode.getUpgradeMode(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "tum", 0)));
            if (id.equals("cham"))
                return new ChameleonUpgrade(TardisMod.tardisChameleonReg.get(nbt, AbstractTardisChameleon.nbtKey));
            if (id.equals("dim"))
                return new DimensionUpgrade(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "dimID", 0));
            if (id.equals("secret")) return new SecretUpgrade();
        }
        return null;
    }
    ...
    public AbstractUpgrade create(ItemStack is) {
        ...
        if (i == TMRegistry.secretUpgradeItem) {
            return new SecretUpgrade();
        }
