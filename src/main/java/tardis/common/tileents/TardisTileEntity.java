...
public class TardisTileEntity extends AbstractTileEntity implements IChunkLoader, IBlockUpdateDetector, IExplodable {
    ...
    private boolean exitLandingSoundTriggered = false;
    ...
    private Runnable chunkGenerated = new Runnable() {
    ...
    private void genChunks() {
        ...
        if (icp instanceof ChunkProviderServer) {
            ...
            if (cgR > Configs.exteriorGenChunksRad) {
                tardis.common.core.TardisOutput.print("TTE", "Generated " + totalGen + " chunks");
                gened = true;
            }
        }
    }

    private void absorbEntitiesDuringMaterialization() {
        if (ServerHelper.isClient()) return;
        if (!(isTakingOff() || (isLanding() && !landed))) return;
        CoreTileEntity core = getCore();
        if (core == null) return;

        AxisAlignedBB absorbZone = AxisAlignedBB
            .getBoundingBox(xCoord - 0.15, yCoord, zCoord - 0.15, xCoord + 1.15, yCoord + 2.1, zCoord + 1.15);
        List<Entity> entities = worldObj.getEntitiesWithinAABBExcludingEntity(null, absorbZone);
        if (entities == null) return;
        for (Entity e : entities) {
            if (!(e instanceof EntityLivingBase)) continue;
            EntityLivingBase living = (EntityLivingBase) e;
            if (living.isDead) continue;
            core.forceEnterTardis(living);
        }
    }
    ...
    public void updateEntity() {
        super.updateEntity();
        if (baseURL == null)
            baseURL = Configs.modConfig.getString("Skin URL", "https://tardismod.wackypocket.com/skins/");

        if (ServerHelper.isServer() && (Configs.exteriorGenChunksRad > 0)
            && ((tt % Configs.exteriorGenChunksTR) == 0)
            && !gened) genChunks();
        ...
        if (inFlight()) {
            if (isLanding() && !landingSoundPlayed) playLandSound();
            else if (isTakingOff() && !takingOffSoundPlayed) playTakeoffSound();
            absorbEntitiesDuringMaterialization();
            // sendUpdate();
            if (++fadeTimer > (!landFast && isLanding() ? 21.5 * 20 : 3.5 * 20)) {
                if (isLanding()) {
                    landed = true;
                    if (ServerHelper.isServer() && exitLandingSoundTriggered) {
                        SoundHelper.stopSoundsNear(this, 48);
                        SoundHelper.playSound(this, "tardismod:engineDrum", 0.75F);
                    }
                    exitLandingSoundTriggered = false;
                } else if (isTakingOff()) {
                    worldObj.setBlockToAir(xCoord, yCoord + 1, zCoord);
                    worldObj.setBlockToAir(xCoord, yCoord, zCoord);
                }
    ...
    private void playLandSound() {
        ...
        if (!landFast) SoundHelper.playSound(this, "tardismod:landing", 1);
        else SoundHelper.playSound(this, "tardismod:landingInt", 1);
        landingSoundPlayed = true;
    }

    public void playLandingSoundForExit() {
        if (ServerHelper.isClient() || !isLanding()) return;
        exitLandingSoundTriggered = true;
        if (!landFast) SoundHelper.playSound(this, "tardismod:landing", 1);
        else SoundHelper.playSound(this, "tardismod:landingInt", 1);
    }
    ...
    public synchronized void land(boolean fast) {
        fadeTimer = 0;
        if (!landing) playLandSound();
        landing = true;
        landFast = fast;
        exitLandingSoundTriggered = false;
        // TardisOutput.print("TTE", "LANDING!!!! " + (fast ? "FAST " : "SLOW ") + (ServerHelper.isClient()?"REM":"SER")
        // + ":" + (landed?"LAN":"UNL"));
        sendUpdate();
    }
    ...
    public AbstractTardisChameleon getChameleon() {
        if (chameleon == null) return TardisMod.tardisChameleonReg.getDefault();
        return chameleon;
    }

    public float getTransparency() {
        double multiplier = 1;
        if (!landed && !isLanding()) return 0.0F;
        else if (isTakingOff() || isLanding()) {
            if (isLanding()) multiplier = (fadeTimer / (landFast ? 50 : 80 * 5.5));
            else if (isTakingOff()) multiplier = 1 - (fadeTimer / (80 * 5.5));
    ...
    public void doorActivated(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!inFlight()) {
            if ((linkedDimension == null) || !TardisMod.dimReg.hasDimension(linkedDimension)) {
                if (TardisMod.plReg.getDimension(player) == null)
                    linkedDimension = Helper.generateTardisInterior(player, this);
                else player.addChatMessage(new ChatComponentText("You already own a TARDIS"));
            } else {
                CoreTileEntity core = Helper.getTardisCore(linkedDimension);
                TardisDataStore ds = Helper.getDataStore(linkedDimension);
                if ((core != null) && (ds != null)) {
                    ds.linkToExterior(this);
                    if (!core.changeLock(player, false)) core.enterTardis(player, false);
                }
            }
        }
    }
    ...
    public TardisDataStore getDataStore() {
        if ((linkedDimension != null) && (linkedDimension != 0)) return Helper.getDataStore(linkedDimension);
        return null;
    }
    ...
    public void writeTransmittable(NBTTagCompound nbt) {
        ...
        if (linkedDimension != null) nbt.setInteger("linkedDimension", linkedDimension);
        if (owner != null) nbt.setString("owner", owner);
        if ((chameleon != TardisMod.tardisChameleonReg.getDefault()) && (chameleon != null))
            if (ServerHelper.isServer()) chameleon.writeToNBT(nbt);
    }

    @Override
    public void readTransmittable(NBTTagCompound nbt) {
        takingOff = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "takingOff", takingOff);
        landing = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "landing", landing);
        landFast = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "landFast", landFast);
        landed = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "landed", landed);
        fadeTimer = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "fadeTimer", fadeTimer);
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "linkedDimension"))
            linkedDimension = io.darkcraft.darkcore.mod.nbt.NBTUtils
                .getInt(nbt, "linkedDimension", linkedDimension == null ? 0 : linkedDimension);
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "owner"))
            owner = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "owner", owner);
        chameleon = TardisMod.tardisChameleonReg.get(nbt, AbstractTardisChameleon.nbtKey);
        gened = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "gened", gened);
    }

    public List<Entity> getEntitiesInside() {
        ArrayList<Entity> list = new ArrayList<Entity>();
        List<Entity> genList = worldObj.getEntitiesWithinAABBExcludingEntity(
            null,
            AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 2, zCoord + 1));
        if (genList != null) {
            for (Object o : genList) {
                if (o instanceof EntityLivingBase) list.add((EntityLivingBase) o);
            }
        }
        return list;
    }
    ...
    public void explode(SimpleCoordStore pos, Explosion explosion) {
        ...
        if (ds != null) ExplosionDamageHelper.damage(ds.damage, pos, explosion, 1);
    }
}
