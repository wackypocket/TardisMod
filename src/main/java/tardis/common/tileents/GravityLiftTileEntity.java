...
public class GravityLiftTileEntity extends AbstractTileEntity implements IScrewable {
    ...
    private double getStopPointBelow(double y) {
        int b = getBlock(y, false);
        if (b == -1) return -1;
        if (b == 0) return yCoord + amountAboveToStopDown;
    ...
    private void scanForCeiling() {
        ...
        for (int distance = 2; distance < (Configs.gravMaxDistance + 3); distance++) {
            if (!WorldHelper.softBlock(worldObj, xCoord, yCoord + distance, zCoord)) {
                if ((yCoord + distance) > maxDist) maxDist = distance;
    ...
    private void scanForPlayers() {
        List<?> baseList = this.worldObj.getEntitiesWithinAABBExcludingEntity(
            null,
        ...
        for (Object o : baseList) {
            ...
            if (pl.capabilities.isFlying) continue;
            if (goingUp.keySet()
                .contains(pl)) continue;
    ...
    private void validatePlayers() {
        Iterator<EntityPlayer> iter = goingUp.keySet()
            .iterator();
        while (iter.hasNext()) {
            EntityPlayer pl = iter.next();
            if (pl.isDead) iter.remove();
            else if (pl.worldObj != worldObj) iter.remove();
            else if (!worldObj.playerEntities.contains(pl)) iter.remove();
            else if ((pl.posX < ((xCoord - 1) + bevel)) || (pl.posX > ((xCoord + 2) - bevel))) iter.remove();
            else if ((pl.posZ < ((zCoord - 1) + bevel)) || (pl.posZ > ((zCoord + 2) - bevel))) iter.remove();
            else if (pl.capabilities.isFlying) iter.remove();
            else if (getBlock(posY(pl), false) == -1) iter.remove();
        }
    }

    private void movePlayer(EntityPlayer pl, boolean up) {
        double dir = 0;
        if (up) dir = Math.min(Configs.gravMovePerTick, getStopPointAbove(posY(pl)) - posY(pl));
        else dir = Math.max(-Configs.gravMovePerTick, getStopPointBelow(posY(pl)) - posY(pl));
        pl.fallDistance = 0;
        pl.motionY = dir / 3;
    ...
    public void updateEntity() {
        super.updateEntity();
        if ((tt % Configs.gravScanCeilingInterval) == 0) scanForCeiling();
        if ((tt % Configs.gravScanPlayerInterval) == 0) scanForPlayers();
        validatePlayers();
        movePlayers();
    }
