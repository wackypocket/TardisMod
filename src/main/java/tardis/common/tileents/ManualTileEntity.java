...
public class ManualTileEntity extends AbstractTileEntity implements IControlMatrix {
    ...
    public List<String> getPageList() {
        if (pageListCache == null) return pageListCache = ManualPageTree.topTree.getString(getPage(), 0);
    ...
    public boolean activate(int x, int y, int z, EntityPlayer pl, int s, float t, float j, float k) {
        if ((dX() ? z : x) == (dX() ? zCoord : xCoord)) {
            if (isRightSide(s, meta())) {
                ...
                boolean flip = meta() < 2;
                double oX;
                if (!flip) oX = ((dX() ? (x - xCoord) + t : (z - zCoord) + k) + 2);
                else oX = ((dX() ? (xCoord - x) - t : (zCoord - z) - k) + 3);
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        selected = ManualPage.get(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "page", selected.ordinal()));
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        else return AxisAlignedBB.getBoundingBox(xCoord, yCoord - 1, zCoord - 2, xCoord + 1, yCoord + 1, zCoord + 3);
    }
