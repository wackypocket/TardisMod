...
public class AdvancedLab extends AbstractTileEntitySer implements IActivatablePrecise, ISidedInventory {
    ...
    public int getStackCount(ItemStack is) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (WorldHelper.sameItem(is, toCheck)) count += toCheck.stackSize;
        }
        return count;
    }
    ...
    public String getInventoryName() {
        return "TardisMod.AdvLabInventory";
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        if (inv[slot].is() == null) return true;
        ItemStack old = inv[slot].is();
        if (WorldHelper.sameItem(old, is)) if (old.stackSize < old.getMaxStackSize()) return true;
        return false;
    }
    ...
    public void addDrops(List<ItemStack> drops) {
        for (ItemStackContainer isc : inv) if (isc.is() != null) drops.add(isc.is());
    }

    @NBTProperty
    public int page = 0;
    @NBTProperty
    public LabRecipe selectedRecipe;
    @NBTProperty
    public boolean active;
    @NBTProperty
    public int energyBuildup;
    ...
    public boolean activate(EntityPlayer ent, int side, float x, float y, float z) {
        ...
        if (side == ForgeDirection.UP.ordinal()) {
            ...
            if (inHand == null) {
                for (int i = 0; i < 5; i++) if (inv[i].is() != null) {
            ...
            ItemStack inHand = ent.getCurrentEquippedItem();
            if (inHand == null) {
                for (int i = 5; i < 10; i++) if (inv[i].is() != null) {
                    WorldHelper.giveItemStack(ent, inv[i].is());
                    inv[i].setIS(null);
                    queueUpdate();
                    return true;
        ...
        if (hp.within(as, r, r, 1 - r, 1 - r)) {
            ...
            if (selectedRecipe == null) {
                ...
                if (yPress == 0) {
                    if (xPress == 0) page = Math.max(page - 1, 0);
                    if (xPress == 2) page = Math.min(
                        page + 1,
                        (LabRecipeRegistry.getRecipes()
                            .size() - 1) / 6);
                } else {
                    int slot = (page * 6) + xPress + ((2 - yPress) * 3);
                    List<LabRecipe> recs = LabRecipeRegistry.getRecipes();
                    if ((slot >= 0) && (slot < recs.size())) selectedRecipe = recs.get(slot);
                }
            } else {
                if (hp.posY < 0.2) {
                    if (hp.posZ < 0.4) {
    ...
    private boolean hasItems() {
        for (ItemStack is : selectedRecipe.source) if (getStackCount(is) < is.stackSize) return false;
        return true;
    }

    private void takeItems() {
        recLoop: for (ItemStack neededIS : selectedRecipe.source) {
    ...
    private boolean addResults() {
        ...
        for (int i = 0; i < 5; i++) {
            output[i] = inv[5 + i].is();
            if (output[i] != null) output[i] = output[i].copy();
        } // Generate array to test

        itemLoop: for (int i = 0; i < results.length; i++) {
            if (results[i] == null) continue;
            for (int j = 0; j < 5; j++) {
                if (output[j] == null) {
                    output[j] = results[i];
                    continue itemLoop;
                } else if ((output[j].stackSize < output[j].getMaxStackSize())
                    && WorldHelper.sameItem(results[i], output[j])) {
        ...
        for (int i = 0; i < 5; i++) inv[5 + i].setIS(output[i]);
        return true;
    }
    ...
    private void processRecipe() {
        ...
        if (energyBuildup < selectedRecipe.energyCost) {
            ...
            if ((iaep != null) && hasItems()) {
                ...
                if (iaep.takeArtronEnergy(take, false)) energyBuildup += take;
            }
    ...
    private void emptyDown() {
        TileEntity te = coords().translate(0, -1, 0)
            .getTileEntity();
        if (te instanceof IInventory) {
            for (int i = 5; i < 10; i++) {
                ...
                inv[i].setIS(is);
                if ((is == null) || (is.stackSize != oss)) queueUpdate();
            }
        }
    }
    ...
    public void tick() {
        ...
        if ((tt % 20) == 0) {
            if ((selectedRecipe != null) && active) processRecipe();
            emptyDown();
        }
    }
