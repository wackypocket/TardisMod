...
public class EngineTileEntity extends AbstractTileEntity implements IControlMatrix, IExplodable {
    ...
    public void updateEntity() {
        ...
        if (ServerHelper.isClient()) {
            if (!isEngineOpen && (visibility < 1)) visibility += 0.1;
            else if (isEngineOpen && (visibility > 0)) visibility -= 0.1;
        }
        if (((tt % 40) == 1) && ServerHelper.isServer()) {
            if (availableConsoleRooms == null) refreshAvailableConsoleRooms();
            verifyEngineBlocks();
            getUsernames();
        }
        ...
        if ((preparingToUpgrade != null) && (tt > (preparingToUpgradeTT + 80))) {
            ...
            if (ServerHelper.isServer()) sendUpdate();
        }
    }

    private void getUsernames() {
        currentUsers = MinecraftServer.getServer()
            .getAllUsernames();
    ...
    public int getControlFromHit(HitPosition hit) {
        ...
        if (hit.within(2, 2.318, 0.170, 2.432, 0.830)) return 0;
        else if (hit.within(2, 0.558, 0.685, 0.660, 0.768)) return 4;
        else if (hit.within(2, 0.488, 0.685, 0.564, 0.768)) return 5;
        else if (hit.within(5, 0.393, 0.725, 0.545, 0.876)) return 10;
        else if (hit.within(5, 0.393, 0.523, 0.545, 0.679)) return 11;
        else if (hit.within(5, 0.393, 0.326, 0.545, 0.477)) return 12;
        else if (hit.within(5, 0.393, 0.122, 0.545, 0.276)) return 13;
        else if (hit.within(5, 0.551, 0.711, 0.704, 0.898)) return 20;
        else if (hit.within(5, 0.551, 0.510, 0.704, 0.696)) return 21;
        else if (hit.within(5, 0.551, 0.311, 0.704, 0.492)) return 22;
        else if (hit.within(5, 0.551, 0.108, 0.704, 0.294)) return 23;
        else if (hit.within(5, 0.729, 0.711, 0.888, 0.898)) return 30;
        else if (hit.within(3, 0.42, 0.532, 0.59, 0.664)) return 39;
        else if (hit.within(3, 0.360, 0.274, 0.440, 0.346)) return 41;
        else if (hit.within(3, 0.460, 0.274, 0.540, 0.346)) return 44;
        else if (hit.within(3, 0.560, 0.274, 0.640, 0.346)) return 45;
        else if (hit.within(3, 0.360, 0.360, 0.440, 0.438)) return 51;
        else if (hit.within(3, 0.460, 0.360, 0.540, 0.438)) return 54;
        else if (hit.within(3, 0.560, 0.360, 0.640, 0.438)) return 55;
        else if (hit.within(3, 0.743, 0.254, 0.876, 0.375)) return 60;
        else if (hit.within(2, 0.716, 0.036, 0.832, 0.674)) return 70;
        else if (hit.within(2, 0.783, 0.687, 0.859, 0.769)) return 71;
        else if (hit.within(2, 0.691, 0.687, 0.768, 0.769)) return 72;
        else if (hit.within(2, 0.735, 0.782, 0.813, 0.859)) return 73;
        else if (hit.side == 2) {
            for (TardisPermission p : TardisPermission.values()) {
                ...
                double x = 0.68 - (o * d);
                double X = 0.68 - ((o + 1) * d);
                if (hit.within(2, 0.33, X, 0.43, x)) return 80 + o;
                if (hit.within(2, 0.43, X, 0.53, x)) return 90 + o;
            }
        } else if (hit.within(4, 0.450, 0.01, 0.525, 0.09)) return 100;
        else if ((ds != null) && ds.hasFunction(TardisFunction.SPAWNPROT) && hit.within(3, 0.675, 0.450, 0.925, 0.540))
            return 130;
        else if (hit.within(3, 0.450, 0.72, 0.55, 0.82)) return 131;
        else if (hit.within(3, 0.76, 0.61, 0.87, 0.72)) return 132;
        return -1;
    }
    ...
    private int getControlFromEngineHit(HitPosition hit) {
        if (hit.side == 4) {
            ...
            for (int i = 0; i < 8; i++) {
                double x = 0.135 + (d * i);
                double X = x + 0.07;
                if (hit.within(4, 0.950, x, 1.083, X)) return 100 + i + 1;
            }
            if (hit.within(4, 0.495, 0.400, 0.700, 0.600)) return 110;
            else if (hit.within(4, 0.75, 0.65, 0.85, 0.75)) return 111;
            else if (hit.within(4, 0.55, 0.70, 0.65, 0.80)) return 112;
            else if (hit.within(4, 0.35, 0.64, 0.45, 0.75)) return 113;
            else if (hit.within(4, 0.75, 0.25, 0.85, 0.35)) return 114;
            else if (hit.within(4, 0.55, 0.20, 0.66, 0.30)) return 115;
            else if (hit.within(4, 0.35, 0.25, 0.45, 0.35)) return 116;
            else if (hit.within(4, 0.15, 0.15, 0.30, 0.30)) return 117;
            else if (hit.within(4, 0.15, 0.40, 0.32, 0.58)) return 119;
            else if (hit.within(4, 0.21, 0.61, 0.29, 0.69)) return 118;
        }
        return -1;
    }
    ...
    public boolean activate(EntityPlayer pl, int side, int blockY, float x, float y, float z) {
        ...
        } else control = getControlFromHit(hit);
        if (control != -1) Helper.activateControl(this, pl, control);
        else TardisOutput.print("TETE", hit.toString());
        return true;
    }
    ...
    public void activateControl(EntityPlayer pl, int control) {
        ...
        if ((core != null) && (ds != null)) {
            if ((control == 4) || (control == 5)) {
            ...
            } else if (control == 30) pl.addChatMessage(
                new ChatComponentText(
                    "[ENGINE] Unspent level points: " + ds.unspentLevelPoints() + "/" + ds.maxUnspentLevelPoints()));
            else if (control == 39) {
                if (screwHelper != null) {
            ...
            } else if (control == 60) internalOnly = !internalOnly;
            else if ((control == 71) || (control == 72)) {
                if ((availableConsoleRooms == null) || (availableConsoleRooms.length == 1)) updateConsoleRooms();
                consoleSettingControl = MathHelper
                    .cycle(consoleSettingControl + (control == 71 ? -1 : 1), 0, availableConsoleRooms.length - 1);
                consoleSettingString = availableConsoleRooms[consoleSettingControl];
            } else if (control == 73) {
                if (ds.hasPermission(pl, TardisPermission.ROOMS)) {
                    if ((prevLastButton != 73) && pl.isSneaking()) lastButton = -1;
                    else if ((prevLastButton == 73) && !pl.isSneaking()) lastButton = -1;
                    else if (prevLastButton != 73) {
                        ServerHelper.sendString(
                            pl,
                            "ENGINE",
                ...
                if (ds.togglePermission(pl, currentPerson, p)) core.sendUpdate();
                else ServerHelper.sendString(pl, CoreTileEntity.cannotModifyPermissions);
            } else if ((control >= 90) && (control < 100)) {
                TardisPermission p = TardisPermission.get(control - 90);
                ServerHelper.sendString(
                    pl,
                    currentPerson + (ds.hasPermission(currentPerson, p) ? hasPerm : hasNoPerm) + p.name);
            } else if (control == 100) {
                if (ds.hasPermission(pl, TardisPermission.POINTS)) isEngineOpen = !isEngineOpen;
                ...
                if ((control >= 101) && (control <= 108)) {
                    if (ds.hasPermission(pl, TardisPermission.POINTS)) {
                        ...
                        if (!addUpgrade(slot, pl, ds)) removeUpgrade(slot, pl, ds);
                        core.sendUpdate();
                    } else ServerHelper.sendString(pl, CoreTileEntity.cannotModifyMessage);
                }
            ...
            if (up != null) {
                if (ds.upgrades[slot] == null) {
                    if (up.isValid(ds.upgrades)) {
                        ...
                        tardis.common.core.TardisOutput
                            .print("ENG", "Added upgrade:" + up, tardis.common.core.TardisOutput.Priority.INFO);
                        ds.markDirty();
                        return true;
                    }
                }
    ...
    public double getControlState(int cID) {
        if (ServerHelper.isServer()) return 0;
        CoreTileEntity core = Helper.getTardisCore(worldObj);
        TardisDataStore ds = Helper.getDataStore(worldObj);
        if ((core != null) && (ds != null)) {
            ...
            if ((cID >= 40) && (cID < 60)) {
                if (screwHelper == null) return 0;
                int mID = cID >= 50 ? cID - 50 : cID - 40;
                ScrewdriverMode m = ScrewdriverMode.get(mID);
                if (cID < 50) return screwHelper.hasPermission(m) ? 1 : 0;
                else {
                    if ((m.requiredFunction == null) || core.hasFunction(m.requiredFunction)) {
            ...
            if (cID == 60) return internalOnly ? 1 : 0;
            if ((cID >= 80) && (cID < 90)) {
            ...
            if (cID == 100) return 1 - visibility;
            if (cID == 130) return protectedRadius / (double) maxProtectedRadius;
            if (cID == 132) {
    ...
    public double[] getColorRatio(int controlID) {
        double[] retVal = { 0, 0, 0 };
        if (controlID == 6) retVal = new double[] { 0.2, 0.3, 0.9 };
        if ((controlID >= 50) && (controlID < 60)) {
            ScrewdriverMode m = ScrewdriverMode.get(controlID - 50);
            return m.c;
        }
        if ((controlID >= 90) && (controlID < 100)) retVal = colors;
        return retVal;
    }
    ...
    public double getControlHighlight(int controlID) {
        if ((controlID >= 10) && (controlID < 20)) {
            ...
            if (mode != null) return preparingToUpgrade == mode ? 1 : -1;
        }
        return -1;
    }
    ...
    public void explode(SimpleCoordStore pos, Explosion explosion) {
        TardisDataStore ds = Helper.getDataStore(this);
        if (ds != null) ExplosionDamageHelper.damage(ds.damage, pos, explosion, 0.9);
    }
    ...
    public void writeTransmittable(NBTTagCompound nbt) {
        ...
        if (currentPerson != null) nbt.setString("cP", currentPerson);
        if (preparingToUpgrade != null) nbt.setInteger("ptU", preparingToUpgrade.ordinal());
        else nbt.setBoolean("ptUN", false);
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        ...
        currentPerson = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "cP", currentPerson);
        lastButton = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "lB", lastButton);
        lastButtonTT = tt;
        preparingToUpgradeTT = tt;
        internalOnly = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "io", internalOnly);
        isEngineOpen = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "eO", isEngineOpen);
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "ptU")) preparingToUpgrade = TardisUpgradeMode
            .getUpgradeMode(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "ptU", -1));
        else if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "ptUN")) preparingToUpgrade = null;
        protectedRadius = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "sp", protectedRadius);
    }

    @Override
    public void readTransmittableOnly(NBTTagCompound nbt) {
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "css"))
            consoleSettingString = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "css", "Main");
        else consoleSettingString = "Main";
    }
    ...
    public String[] getExtraInfo(int control) {
        ...
        switch (control) {
            case 60:
                return new String[] {
                    "Current mode: " + (internalOnly ? "Only interfaces inside the TARDIS can interact"
                        : "All interfaces can interact") };
        }
        if ((control >= 101) && (control <= 108)) {
            if (ds != null) {
                int slot = control - 101;
                AbstractUpgrade up = ds.upgrades[slot];
                if (up != null) return up.getExtraInfo();
            }
        }
        if ((control >= 20) && (control < 30) && (ds != null)) {
            ...
            return new String[] {
                String.format("Repairing will take %s", TardisDamageSystem.repairCompNames[control - 110]) };
        } else if (control == 130)
            return new String[] { String.format("Protection radius: %d blocks", getProtectedSpawnRadius()) };
        else if (control == 132) return new String[] {
            String.format("Current style: " + (ds.getSpaceProjection() ? "Space" : "Overworld")) };
        return null;
    }

    public int getProtectedSpawnRadius() {
        TardisDataStore ds = Helper.getDataStore(this);
        if ((ds != null) && ds.hasFunction(TardisFunction.SPAWNPROT)) return protectedRadius;
        return 0;
    }
}
