...
public class ConsoleTileEntity extends AbstractTileEntity implements IControlMatrix, IExplodable {
...

    {
        for (int i = 0; i < 7; i++) {
            xControls[i] = 0;
            zControls[i] = 0;
            if (i < 4) yControls[i] = 0;
        }
        clampControls();
        if (ServerHelper.isServer()) refillUnstableQueue();
    }
    ...
    public void updateEntity() {
        super.updateEntity();
        if (((++tickTimer % cycleLength) == 0) && (tickTimer != 0)) tickTimer = 0;
        ...
        if ((lastButtonTT != -1)
            && (tt > (lastButtonTT + (importantButton(lastButton) ? FlightConfiguration.shiftPressTime : 20)))) {
            lastButton = -1;
            lastButtonTT = -1;
            sendUpdate();
        }
        ...
        if (ServerHelper.isServer()) {
            if (rdpCounter > 0) rdpCounter--;

            if (roomDeletePrepare && (rdpCounter <= 0)) roomDeletePrepare = false;
    ...
    private HitPosition activateSide(EntityPlayer pl, int blockX, int blockY, int blockZ, float i, float j, float k,
        int side) {
        float distanceAway = ((side == 0) || (side == 2)) ? (float) (Math.abs(pl.posX - 0.5) - 0.5)
            : (float) (Math.abs(pl.posZ - 0.5) - 0.5);
        float distanceSide = ((side == 0) || (side == 2)) ? (float) (pl.posZ + 1) : (float) (pl.posX + 1);
        float hitAway;
        if ((blockX != 0) || (blockZ != 0)) {
            if ((side == 0) && (blockX < 1)) return null;
            if ((side == 1) && (blockZ < 1)) return null;
            if ((side == 2) && (blockX > -1)) return null;
            if ((side == 3) && (blockZ > -1)) return null;
            hitAway = (side == 0 ? i : (side == 2 ? 1 - i : (side == 1 ? k : 1 - k)));
        } else {
            if ((side == 0) && (i < 0.9)) return null;
            if ((side == 2) && (i > 0.1)) return null;
            if ((side == 3) && (k > 0.1)) return null;
            if ((side == 1) && (k < 0.9)) return null;
            j = j + 1;
            hitAway = (side == 0 ? i : (side == 2 ? 1 - i : (side == 1 ? k : 1 - k))) - 1;
        }
        float hitSide;
        if ((side == 0) || (side == 2)) hitSide = blockZ + 1 + k;
        else hitSide = blockX + 1 + i;
        ...
        if (((hitZ < 1) && ((1 - hitX) >= hitZ)) || ((hitZ > 2) && ((1 - hitX) > (3 - hitZ)))) return null;
    ...
    public int getControlFromHit(HitPosition hit) {
        ...
        if (hit.within(0, 0.985, 0.420, 1.124, 0.521)) // Screwdriver
            return 6;
        if (hit.within(2, 1.214, 0.657, 1.338, 0.760)) return 7;
        if (hit.within(0, 0.779, 0.431, 0.901, 0.525)) return 5;
        if (hit.within(0, 1.651, 0.271, 1.883, 0.400))// Gauge1
            return 0;
        if (hit.within(0, 1.375, 0.271, 1.615, 0.400)) return 1;
        if (hit.within(0, 1.517, 0.138, 1.750, 0.265)) return 8;
        if (hit.within(0, 1.210, 0.138, 1.441, 0.265)) return 9;
        if (hit.within(0, 1.10, 0.271, 1.335, 0.400)) return 2;
        if (hit.within(0, 0.865, 0.55, 1.327, 0.868)) return 3;
        if (hit.within(0, 1.725, 0.585, 2.05, 0.846)) return 4;
        if (hit.within(3, 2.156, 0.654, 2.540, 0.924)) return 100;
        if (hit.within(3, 1.727, 0.626, 2.019, 0.826)) return 10;
        if (hit.within(3, 1.361, 0.626, 1.634, 0.826)) return 11;
        if (hit.within(3, 0.981, 0.626, 1.210, 0.826)) return 16;
        if (hit.within(3, 1.513, 0.178, 1.681, 0.461)) return 12;
        if (hit.within(3, 1.303, 0.178, 1.486, 0.461)) return 13;
        if (hit.within(3, 1.694, 0.227, 1.951, 0.408)) return 14;
        if (hit.within(3, 1.039, 0.257, 1.238, 0.417)) return 15;
        // if(hit.within(1, 1.145, 0.207, 1.259, 0.443))
        if (hit.within(1, 1.026, 0.207, 1.169, 0.443)) return 20;
        if (hit.within(1, 1.221, 0.207, 1.379, 0.443)) return 21;
        if (hit.within(1, 1.426, 0.207, 1.564, 0.443)) return 26;
        if (hit.within(1, 1.638, 0.207, 1.769, 0.443)) return 22;
        if (hit.within(1, 1.827, 0.207, 1.969, 0.443)) return 23;
        if (hit.within(1, 0.650, 0.664, 0.943, 0.856)) // Z Wheel 1
            return 24;
        if (hit.within(1, 1.264, 0.664, 1.545, 0.856)) // Z Wheel 2
            return 25;
        if (hit.within(2, 1.730, 0.147, 1.859, 0.470)) return 30;
        if (hit.within(2, 1.581, 0.147, 1.710, 0.470)) return 31;
        if (hit.within(2, 1.430, 0.147, 1.558, 0.470)) return 32;
        if (hit.within(2, 1.284, 0.147, 1.415, 0.470)) return 33;
        if (hit.within(2, 1.116, 0.266, 1.248, 0.360)) return 34;
        ...
        if (hit.within(0, 2.110, 0.562, 2.441, 0.872)) // Flight Takeoff
            return 42;
        if (hit.within(0, 0.523, 0.687, 0.651, 0.783)) return 43;
        if (hit.within(2, 2.251, 0.728, 2.371, 0.816)) return 50;
        if (hit.within(2, 2.251, 0.822, 2.371, 0.902)) return 51;
        if (hit.within(2, 0.290, 0.825, 0.441, 0.915)) return 52;
        if (hit.within(0, 1.760, 0.172, 1.914, 0.265)) return 53;
        if (hit.within(1, 1.013, 0.493, 1.194, 0.624) && core.hasFunction(TardisFunction.SENSORS)) return 54;
        if (hit.within(3, 0.701, 0.703, 0.823, 0.792)) return 55;
        if (hit.within(3, 0.701, 0.807, 0.823, 0.903) && core.hasFunction(TardisFunction.STABILISE)) return 56;
        if (hit.within(2, 2.249, 0.515, 2.377, 0.608)) return 57;
        if (hit.within(2, 2.249, 0.622, 2.377, 0.710)) return 58;
        if (hit.within(2, 0.971, 0.598, 1.138, 0.941)) return 60;
        if (hit.within(3, 2.557, 0.806, 2.683, 0.896)) // Delete rooms button
            return 901;
        if (hit.within(1, 2.369, 0.801, 2.491, 0.894)) // Load/Save Switch
            return 900;
        if (hit.within(1, 2.377, 0.703, 2.498, 0.800)) return 902;
        if (hit.within(1, 2.375, 0.610, 2.497, 0.701)) return 903;
        if (hit.within(2, 0.967, 0.266, 1.100, 0.360)) return 904;
        ...
        if (hit.within(3, 0.533, 0.706, 0.669, 0.909)) return 1020;
        if (hit.within(3, 0.370, 0.706, 0.492, 0.909)) return 1021;
        if (hit.within(3, 0.592, 0.478, 0.927, 0.595)) return 1022;
        if (hit.within(3, 2.103, 0.478, 2.408, 0.595)) return 1023;
        if (hit.within(2, 0.771, 0.439, 1.049, 0.546)) return 1024;
        if (hit.within(2, 1.037, 0.499, 1.311, 0.595)) return 1025;
        if (hit.within(2, 1.116, 0.367, 1.245, 0.471)) return 1026;
        if (hit.within(1, 1.275, 0.542, 1.537, 0.619)) return 1027;
        if (hit.within(1, 0.669, 0.542, 0.937, 0.619)) return 1028;
        if (hit.within(1, 0.669, 0.435, 0.937, 0.520)) return 1029;
        if (hit.within(0, 1.421, 0.521, 1.550, 0.615)) return 1030;
        if (hit.within(0, 1.421, 0.623, 1.550, 0.718)) return 1031;
        if (hit.within(0, 1.421, 0.730, 1.550, 0.822)) return 1032;
        return -1;
    }
    ...
    public boolean activate(EntityPlayer pl, int blockX, int blockY, int blockZ, float i, float j, float k) {
        ...
        if (hit != null) {
            ...
            if (controlHit >= 0) Helper.activateControl(this, pl, controlHit);
            else TardisOutput.print("TConTE", "H:" + hit.toString(), TardisOutput.Priority.INFO);
        } else TardisOutput.print("TConTE", "No hit");
        return true;
    }

    public boolean isMovementControl(int controlID) {
        if ((controlID >= 10) && (controlID < 40)) return true;
        switch (controlID) {
            case 3:
            case 53:
            case 55:
    ...
    public void activateControl(EntityPlayer pl, int controlID) {
        ...
        if ((core == null) || (ds == null)) return;
        if ((isMovementControl(controlID) || requiresFlightPermission(controlID))
            && !ds.hasPermission(pl, TardisPermission.FLY)) {
            pl.addChatMessage(CoreTileEntity.cannotModifyFly);
            return;
        }
        TardisOutput.print("TConTE", "Control:" + controlID, TardisOutput.Priority.DEBUG);
        if (controlID == 0) pl.addChatMessage(
            new ChatComponentText("Energy: " + core.getArtronEnergy() + "/" + core.getMaxArtronEnergy()));
        else if (controlID == 1)
            pl.addChatMessage(new ChatComponentText("Rooms: " + core.getNumRooms() + "/" + core.getMaxNumRooms()));
        else if (controlID == 2) pl.addChatMessage(
            new ChatComponentText(String.format("Speed: %.1f/%.1f", core.getSpeed(true), core.getMaxSpeed())));
        else if (controlID == 8) {
        ...
        } else if (controlID == 9) pl.addChatMessage(
            new ChatComponentText("Shields: " + ds.damage.getShields() + "/" + ds.damage.getMaxShields()));
        else if (controlID == 4) // Speed lever
        {
            int d = 1;
            if (pl.isSneaking()) d = -1;
            core.addSpeed(d);
        } else if (controlID == 100) // Scanner
            core.sendDestinationStrings(pl);
        else if (controlID == 904) // Land on pad
            landOnPad = !landOnPad;
        else if (core.inAbortableFlight() && (controlID == 42)) attemptToLand = core.attemptToLand();
        else if (controlID == 55) uncoordinated = !uncoordinated;
            ...
            if (isMovementControl(controlID) && (!core.inFlight() || !core.inCoordinatedFlight())) {
                ...
                if (controlID == 3) facing = MathHelper.cycle(facing + (pl.isSneaking() ? -1 : 1), 0, 3);
                else if (((controlID >= 10) && (controlID < 14)) || (controlID == 16)) {
                    if (pl.isSneaking()) xControls[controlID - 10]--;
                    else xControls[controlID - 10]++;
                    ...
                    else xControls[controlID - 10] = MathHelper.cycle(xControls[controlID - 10] + 1, 0, 7);
                    clampControls(xControls);
                } else if (((controlID >= 20) && (controlID < 24)) || (controlID == 26)) {
                    if (pl.isSneaking()) zControls[controlID - 20]--;
                    else zControls[controlID - 20]++;
                    ...
                    else zControls[controlID - 20] = MathHelper.cycle(zControls[controlID - 20] + 1, 0, 7);
                    clampControls(zControls);
                } else if ((controlID >= 30) && (controlID < 34)) {
                    if (pl.isSneaking()) yControls[controlID - 30]--;
                    else yControls[controlID - 30]++;
                    clampControls(yControls);
                } else if (controlID == 34) landGroundControl = !landGroundControl;
                else if (controlID == 53) relativeCoords = !relativeCoords;
                ...
                } else if (controlID == 43) randomiseControls(core);
            } else if (!core.inFlight()) {
                if (controlID == 40) primed = true;
                else if ((controlID == 41) && primed) regulated = true;
                else if ((controlID == 42) && primed && regulated) core.takeOff(pl);
            }
        }
        ...
        if (controlID == 5) {
            ...
            } else ServerHelper.sendString(pl, CoreTileEntity.cannotModifyMessage);
        ...
        } else if ((controlID == 50) || (controlID == 51)) // Schema change
        {
            ...
            refreshSchemas();
        } else if (controlID == 52) ds.setDaytimeSetting(((ds.getDaytimeSetting()) + 1) % 3);
        else if ((controlID == 54) && core.hasFunction(TardisFunction.SENSORS)) core.sendScannerStrings(pl);
        else if ((controlID == 56) && core.hasFunction(TardisFunction.STABILISE)) {
            stable = !stable;
            if (stable) clearUnstableControl();
        } else if (controlID == 900) saveCoords = !saveCoords;
        ...
        } else if (controlID >= 1000) // Flight instabilitiers
        {
            ...
            if ((controlID >= 1000) && (controlID < 1020)) {
                ...
                if ((is != null) && ((is.getItem() instanceof ItemNameTag) || (is.getItem() instanceof NameTagItem))
                    && (ssnds != null)) {
                    if (is.hasDisplayName()) {
                        ...
                        if (ssnds.setName(n, controlID - 1000)) {
                            ...
                            run = false;
                            if (!pl.capabilities.isCreativeMode) is.stackSize--;
                        }
                    }
                }
                if (run) ssnds.sendUpdate();
            ...
            if (run && core.inFlight()) {
                if (unstableControl == controlID) pressedUnstable();
                else TardisOutput.print("TConTE", "Stable button pressed:" + controlID + ":" + unstableControl);
            }
        }

        if ((controlID == 901)
            && ((Configs.deleteAllOwnerOnly && core.isOwner(pl)) || ds.hasPermission(pl, TardisPermission.ROOMS))) {
            if (!roomDeletePrepare) {
                ...
                if (pl.isSneaking()) core.removeAllRooms();
            ...
            if (roomDeletePrepare) roomDeletePrepare = false;
        }
        lastButton = controlID;
        lastButtonTT = tt;
        markDirty();
    ...
    public boolean setControls(TardisTileEntity ext, boolean allowNearest) {
        ...
        TardisOutput.print(
            "TConTE",
            "Attempting to set controls to :" + dim + "," + xC + "," + yC + "," + zC,
    ...
    private static int[] getControlsFromDest(int dest) {
        ...
        c += temp[3];
        if (c == dest) TardisOutput
            .print("TConTE", "Found coords for " + dest + ": " + Arrays.toString(temp), TardisOutput.Priority.DEBUG);
        else TardisOutput.print(
            "TConTE",
            "Found nearest coords for " + dest + ": " + c + ": " + Arrays.toString(temp),
            TardisOutput.Priority.DEBUG);
        return temp;
    }
    ...
    public int getControlFromDim(int dim) {
        TardisDataStore ds = Helper.getDataStore(this);
        if (ds != null) return TardisMod.otherDims.getControlFromDim(dim, ds.getLevel(), ds);
        return 0;
    }

    public int getNumDims() {
        TardisDataStore ds = Helper.getDataStore(this);
        if (ds == null) return TardisMod.otherDims.numDims();
        else return TardisMod.otherDims.numDims(ds.getLevel(), ds);
    ...
    public int getYFromControls(int[] cont) {
        ...
        TardisOutput.print(
            "TConTE",
            "YCont:" + cont[0] + "," + cont[1] + "," + cont[2] + "," + cont[3] + "=" + count,
    ...
    private void loadControls(int num) {
        ...
        if (states.containsKey(num)) loadControls(states.get(num));
    }
    ...
    private void clampControls() {
        if ((xControls == null) || (xControls.length != 7)) xControls = ControlStateStore.fixControls(xControls);
        if ((yControls == null) || (yControls.length != 4)) yControls = new int[4];
        if ((zControls == null) || (zControls.length != 7)) zControls = ControlStateStore.fixControls(zControls);
        clampControls(xControls);
        clampControls(yControls);
        clampControls(zControls);
    }
    ...
    public double getControlState(int controlID) {
        ...
        if ((core != null) && (ds != null)) {
            ...
            if ((controlID >= 14) && (controlID < 16)) return xControls[controlID - 10] / 8.0;
            if (((controlID >= 20) && (controlID < 24)) || (controlID == 26))
                return ((double) (zControls[controlID - 20] + 6) / 12);
            if ((controlID >= 24) && (controlID < 26)) return zControls[controlID - 20] / 8.0;
            if ((controlID >= 30) && (controlID < 34)) return yControls[controlID - 30] / 3.0;
            if (controlID == 34) return (landGroundControl ? 1 : 0);
            if (controlID == 40) return (primed ? 1 : 0);
            if (controlID == 41) return (regulated ? 1 : 0);
            if (controlID == 42) return ((!attemptToLand) && core.inFlight()) ? 1 : 0;
            if ((controlID == 50) || (controlID == 51)
                ...
                || (controlID == 57)
                || (controlID == 58)
                || (controlID == 43)) return lastButton == controlID ? 1 : 0;
            if (controlID == 52) return ds.getDaytimeSetting() / 2.0;
            if (controlID == 53) return relativeCoords ? 1 : 0;
            if (controlID == 55) return uncoordinated ? 1 : 0;
            if (controlID == 56) return stable ? 1 : 0;
            if (controlID == 60) return dimControlState;
            if (controlID == 900) return saveCoords ? 1 : 0;
            if (controlID == 901) return roomDeletePrepare ? 1 : 0;
            if (controlID == 904) return landOnPad ? 1 : 0;
            if ((controlID >= 1000) && (controlID < 1033)) return lastButton == controlID ? 1 : 0;
    ...
    public String[] getExtraInfo(int controlID) {
        ...
        if ((core != null) && (ds != null)) {
            ...
            if ((controlID >= 10) && (controlID < 17)) return new String[] { "Set to " + xControls[controlID - 10] };
            if ((controlID >= 20) && (controlID < 27)) return new String[] { "Set to " + zControls[controlID - 20] };
            if ((controlID >= 30) && (controlID < 34)) return new String[] { "Set to " + yControls[controlID - 30] };

            if (controlID == 900) return new String[] { "Current: " + (saveCoords ? "Save" : "Load") };
            if (controlID == 904)
            ...
            if (controlID == 56) return new String[] { "Current: " + (stable ? "Stable flight" : "Unstable flight") };
            if (controlID == 52) {
            ...
            if (controlID == 55) return new String[] {
                "Current: " + (uncoordinated ? "Uncoordinated flight (Drifting)" : "Coordinated flight") };
            if (controlID == 53) return new String[] {
                "Current: " + (relativeCoords ? "Relative coordinates" : "Absolute coordinates") };
            if ((controlID >= 1000) && (controlID < 1020) && (ssnds != null)) {
                int slot = controlID - 1000;
                String name = ssnds.getName(slot);
                if (name != null) return new String[] { "Name: " + name };
            }
        }
        return null;
    }
    ...
    public double[] getColorRatio(int controlID) {
        if (controlID >= 1020) return flightColors;
        if ((controlID >= 1000) && ((controlID % 2) == 0)) return flightColors;
    ...
    public double getControlHighlight(int controlID) {
        ...
        if ((controlID == 901) && roomDeletePrepare) return highlightAmount;
        return -1;
    }
    ...
    private void refillUnstableQueue() {
        ...
        while (size < 8) {
            int ran = 0;
            if (minUnstable != maxUnstable) ran = rand.nextInt((1 + maxUnstable) - minUnstable);
    ...
    public void getNextUnstableControl() {
        if (ServerHelper.isServer()) if (unstableQueue.size() < 3) refillUnstableQueue();
        if (ServerHelper.isClient()) unstableControl = unstableQueue.size() > 0 ? unstableQueue.remove() : minUnstable;
        else unstableControl = unstableQueue.remove();
        unstablePressed = false;
        sendUpdate();
        tardis.common.core.TardisOutput.print("CON", String.valueOf(unstableControl));
    }
    ...
    public void refreshSchemas() {
        if (categoryNum != lastCategoryNum) {
            if ((categoryList == null) || (categoryList.length == 0)) refreshCategories();
    ...
    public ScrewdriverHelper getScrewHelper(int slot) {
        switch (slot) {
            case 0:
                return frontScrewHelper;
            case 1:
                return backScrewHelper;
            default:
                return null;
        }
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        schemaNum = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "schemaNum", schemaNum);
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "dC"))
            dc = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "dC", 0);
        for (int i = 0; i < 20; i++) {
            if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "css" + i))
                states.put(i, ControlStateStore.readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, "css" + i)));
        }
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "lastLandingCSS"))
            lastLanding = ControlStateStore.readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, "lastLandingCSS"));
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "currentLandingCSS"))
            currentLanding = ControlStateStore.readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, "currentLandingCSS"));
        clampControls();
    }
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        stable = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "stable", stable);
        uncoordinated = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "uncoordinated", uncoordinated);
        relativeCoords = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "relativeCoords", relativeCoords);
        roomDeletePrepare = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "rdp", roomDeletePrepare);
        tickTimer = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "tickTimer", tickTimer);
        frontScrewHelper = ScrewdriverHelperFactory.get(nbt, "scNBT");
        backScrewHelper = ScrewdriverHelperFactory.get(nbt, "bscNBT");
        facing = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "facing", facing);
        xControls = io.darkcraft.darkcore.mod.nbt.NBTUtils.getIntArray(nbt, "xControls");
        zControls = io.darkcraft.darkcore.mod.nbt.NBTUtils.getIntArray(nbt, "zControls");
        yControls = io.darkcraft.darkcore.mod.nbt.NBTUtils.getIntArray(nbt, "yControls");
        primed = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "primed", primed);
        regulated = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "regulated", regulated);
        saveCoords = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "saveCoords", saveCoords);
        landGroundControl = io.darkcraft.darkcore.mod.nbt.NBTUtils
            .getBoolean(nbt, "landGroundControl", landGroundControl);
        {
            int newUnstable = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "unstableControl", unstableControl);
            if (newUnstable != unstableControl) {
                if ((unstableQueue.size() > 0) && (newUnstable == unstableQueue.peek())) unstableQueue.remove();
                else System.out
                    .println("NewUnstable:" + newUnstable + " vs OldUnstable:" + unstableControl + " mismatch");
            }
            unstableControl = newUnstable;
        }
        landOnPad = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "lOP", landOnPad);
        clampControls();
    }
    ...
    public void writeTransmittableOnly(NBTTagCompound nbt) {
        ...
        if (ServerHelper.isServer()) {
            ...
            for (Integer unst : unstableQueue) nbt.setInteger("unstableQueue" + (i++), unst);
        }
    }

    @Override
    public void readTransmittableOnly(NBTTagCompound nbt) {
        schemaCategoryString = NBTUtils
            .getString(nbt, "schemaCategoryString", schemaCategoryString == null ? "" : schemaCategoryString);
        schemaChooserString = NBTUtils
            .getString(nbt, "schemaChooserString", schemaChooserString == null ? "" : schemaChooserString);
        dimControlState = NBTUtils.getFloat(nbt, "dCS", dimControlState);
        lastButton = NBTUtils.getInt(nbt, "lastButton", lastButton);
        lastButtonTT = NBTUtils.getInt(nbt, "lastButtonTT", lastButtonTT);
        if (lastButton == unstableControl) pressedUnstable();
        attemptToLand = NBTUtils.getBoolean(nbt, "attemptToLand", attemptToLand);
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "unstableQueue0")) unstableQueue.clear();
        for (int i = 0; io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "unstableQueue" + i); i++)
            unstableQueue.add(NBTUtils.getInt(nbt, "unstableQueue" + i, 0));
    }
    ...
    public void explode(SimpleCoordStore pos, Explosion explosion) {
        TardisDataStore ds = Helper.getDataStore(this);
        if (ds != null) ExplosionDamageHelper.damage(ds.damage, pos, explosion, 0.6);
    }
}
