...
public class LandingPadTileEntity extends ComponentTileEntity {
    ...
    public CoreTileEntity getCore() {
        if (Helper.isTardisWorld(worldObj)) return super.getCore();
        TileEntity te = worldObj.getTileEntity(xCoord, yCoord + 1, zCoord);
        if (!(te instanceof TardisTileEntity)) te = worldObj.getTileEntity(xCoord, yCoord + 2, zCoord);
        if (te instanceof TardisTileEntity) {
            if (!((TardisTileEntity) te).inFlight()) {
                CoreTileEntity core = ((TardisTileEntity) te).getCore();
                if (core != null) {
                    if (core.canBeAccessedExternally()) return core;
                }
            }
        }
        return null;
    ...
    public TardisDataStore getDS() {
        ...
        if (c != null) return Helper.getDataStore(c);
    ...
    public boolean isClear() {
        return worldObj.isAirBlock(xCoord, yCoord + 2, zCoord)
            && (worldObj.isAirBlock(xCoord, yCoord + 1, zCoord) || worldObj.isAirBlock(xCoord, yCoord + 3, zCoord));
    }
