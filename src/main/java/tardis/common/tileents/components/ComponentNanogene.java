...
public class ComponentNanogene extends AbstractComponent implements IScrewable {
        ...
        for (Object o : worldList) {
            if (o instanceof EntityLivingBase) {
                ...
                if (isPlayerInNeedOfHelp(ent)) {
                    ...
                    if (dist <= Configs.nanogeneRange) list.add(ent);
                }
            }
        }
        return list;
    ...
    public void updateTick() {
        super.updateTick();
        if ((parentObj == null) || ServerHelper.isClient()) return;
        if (((tt % Configs.nanogeneTimer) == 0) && ServerHelper.isServer()) {
            IArtronEnergyProvider core = getArtronEnergyProvider();
            if (core == null) return;
            int worldId = WorldHelper.getWorldID(parentObj);
            ArrayList<EntityLivingBase> ents = getNearbyPlayers();
            for (EntityLivingBase ent : ents) {
                if (core.takeArtronEnergy(Configs.nanogeneCost, false)) {
                    Helper.spawnParticle(ParticleType.NANOGENE, worldId, ent.posX, ent.posY + 1, ent.posZ, 20, true);
                    if (ent.isBurning()) ent.extinguish();
    ...
    public boolean screw(ScrewdriverHelper helper, ScrewdriverMode mode, EntityPlayer player) {
        ...
        if ((mode == ScrewdriverMode.Reconfigure)
            && ((ds == null) || ds.hasPermission(player, TardisPermission.ROUNDEL))) {
            state = (state + 1) % 3;
            if (ServerHelper.isServer()) ServerHelper.sendString(player, "New mode: " + getModeString(state));
            return true;
        }
        return false;
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        state = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "state", state);
    }
}
