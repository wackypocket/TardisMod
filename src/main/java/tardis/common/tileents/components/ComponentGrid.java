...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "tardis.common.integration.ae.ITMGrid", modid = "appliedenergistics2"),
        @Optional.Interface(iface = "appeng.api.networking.IGridBlock", modid = "appliedenergistics2") })
public class ComponentGrid extends AbstractComponent
    implements ITMGrid, IGridBlock, IBlockUpdateDetector, IScrewable, IActivatable {
    ...
    public void updateTick() {
        ...
        if (!inited && (parentObj != null)) {
            ...
            if (core != null) core.addGridLink(myCoords);
        }
        if ((!linkedToOther) && (otherGrid != null) && parentObj.isValid()) linkToSCS(otherGrid);
        if (!linkedToCore) linkToCore();
    }
    ...
    public IGridNode getGridNode(ForgeDirection dir) {
        if ((parentObj == null) || !parentObj.isValid()) return null;
        createNode();
        return node;
    }
    ...
    private void createNode() {
        if (!AEIntegration.isAE2Available()) return;
        if ((node == null) && ServerHelper.isServer()) {
            node = (IGridNode) AEIntegration.createGridNode(this);
    ...
    private static boolean doesConnectionExist(IGridNode aNode, IGridNode oNode,
        Iterable<IGridConnection> connections) {
        for (IGridConnection c : connections) if ((c.a()
            .equals(oNode)
            && c.b()
                .equals(aNode))
            || (c.a()
    ...
    private boolean doesConnectionExist(IGridNode aNode, IGridNode oNode) {
        if ((aNode == null) || (oNode == null)) return false;
        ...
        try {
            if ((otherNode != null) && (node != null)) {
                if (!doesConnectionExist(node, otherNode)) {
                    IGridConnection con = (IGridConnection) AEIntegration.createGridConnection(node, otherNode);
                    if (con != null) {
                        errState = null;
                        addConnection(con);
                        if (otherHost instanceof ITMGrid) ((ITMGrid) otherHost).addConnection(con);
            ...
            // Check if it's a connection failure (AE2 FailedConnection exception)
            if (e.getClass()
                .getSimpleName()
                .equals("FailedConnection")) {
    ...
    public void addConnection(IGridConnection con) {
        if ((con.a() == node) || (con.b() == node)) connections.add(con);
    }
    ...
    private void scan() {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (te instanceof IGridHost) {
                ...
                if (!doesConnectionExist(node, otherNode)) upd = true;
            }
        }
        if (upd) parentObj.sendUpdate();
    }
    ...
    public EnumSet<ForgeDirection> getConnectableSides() {
        if (parentObj == null) return validDirs;
        World w = parentObj.getWorldObj();
        if (w == null) return validDirs;
        EnumSet<ForgeDirection> dirs = EnumSet.copyOf(validDirs);
        for (ForgeDirection d : ForgeDirection.VALID_DIRECTIONS) {
            if (w.getTileEntity(
                xCoord + d.offsetX,
                yCoord + d.offsetY,
                zCoord + d.offsetZ) instanceof ComponentTileEntity) dirs.remove(d);
        }
        return dirs;
    }
    ...
    public IGridHost getMachine() {
        if (parentObj instanceof IGridHost) {
            return (IGridHost) parentObj;
        }
        return null;
    }
    ...
    private void breakOGConnection() {
        for (IGridConnection n : connections) n.destroy();
        connections.clear();
        linkToCore();
    }
    ...
    private boolean linkToSCS(SimpleCoordStore scs) {
        ...
        if (!(te instanceof ComponentTileEntity) || (scs.world != WorldHelper.getWorldID(parentObj))) {
            if (otherGrid == scs) otherGrid = null;
            return false;
        }
        ...
        if ((otherGrid != null) && !otherGrid.equals(scs)) breakOGConnection();
        otherGrid = scs;
        if (addConnection(cg, null)) linkedToOther = true;
    ...
    public boolean screw(ScrewdriverHelper helper, ScrewdriverMode mode, EntityPlayer player) {
        ...
        if (helper.getLinkSCS() == null) {
            ...
            if (scs.equals(myCoords) && (otherGrid != null)) {
                ...
                if (ServerHelper.isServer()) ServerHelper
                    .sendString(player, SonicScrewdriverItem.screwName, "Linked " + myCoords + " to " + scs);
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        if (otherGrid != null) otherGrid.writeToNBT(nbt, "og");
    }

    @Override
    public void readFromNBT(NBTTagCompound nbt) {
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "og")) {
            SimpleCoordStore scs = SimpleCoordStore.readFromNBT(nbt, "og");
            if (!scs.equals(otherGrid)) {
                if (inited) linkToSCS(scs);
                else otherGrid = scs;
            }
    ...
    public boolean activate(EntityPlayer ent, int side) {
        if (ServerHelper.isServer()) {
            if (errState != null) ServerHelper.sendString(ent, errState);
            if (otherGrid == null) ServerHelper.sendString(ent, "Direct connection: None");
            else ServerHelper.sendString(ent, "Direct connection: " + otherGrid);
        }
        return true;
    }
