    ...
    public boolean screw(ScrewdriverHelper helper, ScrewdriverMode mode, EntityPlayer player) {
        if (mode.equals(ScrewdriverMode.Reconfigure)) {
            ...
            if ((core != null) && (ds != null)) {
                ...
                if (ds.hasPermission(player, TardisPermission.ROUNDEL)
                    && ds.hasPermission(player, TardisPermission.TRANSMAT)) {
                    ...
                    if (core.isTransmatPoint(transPoint)) {
                        ...
                        if (parentObj.getWorldObj()
                            .isAirBlock((int) player.posX, (int) player.posY, (int) player.posZ)
                            && parentObj.getWorldObj()
                                .isAirBlock((int) player.posX, (int) player.posY + 1, (int) player.posZ)) {
                            core.setTransmatPoint(transPoint);
                            c.appendText(
                                "TARDIS transmat point set to " + core.getTransmatPoint()
                                    .toSimpleString());
                        } else c.appendText("Not enough room for transmat");
                    }
                } else c.appendText("You don't have permission to modify this TARDIS");
