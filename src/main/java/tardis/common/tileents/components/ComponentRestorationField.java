        ...
        if (ent instanceof EntityPlayerMP) {
            ...
            if (pl.inventory.mainInventory != null) {
                for (ItemStack s : pl.inventory.mainInventory) {
                    if (s != null && s.isItemStackDamageable() && s.isItemDamaged()) return true;
                }
            }
        ...
        for (Object o : worldList) {
            if (o instanceof EntityLivingBase) {
                ...
                if (hasBrokenItems(ent)) {
                    ...
                    if (dist <= Configs.restorationFieldRange) list.add(ent);
                }
            }
        }
        return list;
        ...
        if (((tt % Configs.restorationFieldTimer) == 0) && ServerHelper.isServer()) {
            ...
            for (EntityLivingBase ent : ents) {
                if ((ent instanceof EntityPlayerMP)) {
                    ...
                    for (ItemStack s : pl.inventory.mainInventory) {
                        if (s != null) {
                            if (IC2.isItemElectric(s) || CofHCore.isItemElectric(s) || AEIntegration.isItemElectric(s))
                                continue;
                            ...
                            if (s.isItemStackDamageable()) {
                                if (s.isItemDamaged()) {
                                    ...
                                    if (i.getDamage(s)
                                        > (i.getMaxDamage()
                                            * ((double) (100 - Configs.restorationFieldPercentage) / 100))
                                        && core.takeArtronEnergy(Configs.restorationFieldCost, false)) {
                                        i.setDamage(s, i.getDamage(s) - 1);
                                        Helper.spawnParticle(
                                            ParticleType.RESTORATIONFIELD,
                                            worldId,
                                            ent.posX,
                                            ent.posY + 1,
                                            ent.posZ,
