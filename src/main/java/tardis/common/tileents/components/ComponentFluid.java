...
public class ComponentFluid extends AbstractComponent implements IFluidHandler, IScrewable, IActivatable {
    ...
    private FluidStack[] getTanks(boolean limit) {
        if ((parentObj != null) && (parentObj.getWorldObj() != null)) {
            ...
            if (ds != null) {
                ...
                if ((currentTank < 0) || (currentTank >= tanks.length) || !limit) return tanks;
    ...
    private int fill(FluidStack[] tanks, int i, boolean doFill, FluidStack resource) {
        ...
        if (tanks[i] == null) {
            if (doFill) tanks[i] = new FluidStack(resource, Math.min(Configs.maxFlu, resource.amount));
            ...
            if (doFill) tanks[i].amount += maxFill;
            return maxFill;
        }
        return 0;
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        if (tanks != null) {
            if (currentTank == -1)
                for (int i = 0; (i < tanks.length) && (ret == 0); i++) ret = fill(tanks, i, doFill, resource);
            else ret = fill(tanks, currentTank, doFill, resource);
        }
        return ret;
    }
    ...
    private FluidStack drain(FluidStack[] tanks, int i, FluidStack resource, boolean doDrain) {
        if ((tanks[i] != null) && tanks[i].equals(resource)) {
            ...
            if (doDrain) {
                if (toDrain == tanks[i].amount) tanks[i] = null;
                else tanks[i].amount -= toDrain;
            }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        ...
        if (tanks != null) {
            if (currentTank == -1)
                for (int i = 0; (i < tanks.length) && (ret == null); i++) ret = drain(tanks, i, resource, doDrain);
            else ret = drain(tanks, currentTank, resource, doDrain);
        }
        return ret;
    }

    private FluidStack drain(FluidStack[] tanks, ForgeDirection from, IFluidHandler other, int i, int maxDrain,
        boolean doDrain) {
        if (tanks == null) return null;
        if (tanks[i] != null) {
            Fluid f = tanks[i].getFluid();
            if ((other != null) && !other.canFill(from.getOpposite(), f)) return null;
    ...
    public FluidStack drain(ForgeDirection from, int maxDrain, boolean doDrain) {
        ...
        FluidStack ret = null;
        if (tanks != null) {
            SimpleCoordStore n = parentObj.coords()
                .getNearby(from);
            TileEntity te = n.getTileEntity();
            IFluidHandler other = null;
            if (te instanceof IFluidHandler) other = (IFluidHandler) te;
            if (currentTank == -1) for (int i = 0; (i < tanks.length) && (ret == null); i++)
                ret = drain(tanks, from, other, i, maxDrain, doDrain);
            else ret = drain(tanks, from, other, currentTank, maxDrain, doDrain);
        }
        return ret;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        ...
        if (tanks != null) {
            for (int i = 0; i < tanks.length; i++) {
                ...
                if (tanks[i].getFluid()
                    .equals(fluid)) return tanks[i].amount < Configs.maxFlu;
            }
        }
        return foundEmpty;
    }
    ...
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        ...
        if (tanks != null) {
            for (int i = 0; i < tanks.length; i++) {
                ...
                if (tanks[i].getFluid()
                    .equals(fluid)) return tanks[i].amount > 0;
            }
        }
        return false;
    }
    ...
    public boolean activate(EntityPlayer ent, int side) {
        ...
        for (int i = 0; i < tanks.length; i++) {
            ...
            ServerHelper
                .sendString(ent, "Tank " + (i + 1) + ": " + name + " " + ft.amount + "/" + Configs.maxFlu + "mb");
        }
        return true;
    }
    ...
    public boolean screw(ScrewdriverHelper helper, ScrewdriverMode mode, EntityPlayer player) {
        ...
        if ((ds == null) || ds.hasPermission(player, TardisPermission.ROUNDEL)) {
            ...
            if (currentTank == -1) ServerHelper.sendString(player, "Fluid interface configured to all tanks");
            else ServerHelper.sendString(player, "Fluid interface configured to tank " + (currentTank + 1));
            return true;
        } else {
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        currentTank = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "cT", -1);
    }
}
