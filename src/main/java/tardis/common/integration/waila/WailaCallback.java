...
public class WailaCallback {
    ...
    public static AbstractWailaProvider getProvider(Block b, int meta) {
        if (b == TMRegistry.schemaComponentBlock) {
            if ((meta == 3) || (meta == 6)) return consoleProv;
            else if (meta == 7) return engineProv;
        } else if (b == TMRegistry.tardisConsoleBlock) return consoleProv;
        else if (b == TMRegistry.tardisEngineBlock) return engineProv;
        else if (b == TMRegistry.battery) return artronProv;
        else if (b == TMRegistry.tardisBlock) return tardisProv;
        else if (b == TMRegistry.tardisTopBlock) return tardisProv;
        return null;
    }
}
