...
public abstract class AbstractWailaProvider implements IWailaDataProvider {
    ...
    public void addControl(TwoIntStore key, String name, String extra) {
        ...
        // Extra text is no longer used - the manual item fallback system handles this
    }
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (control != -1) {
            boolean f = false;
            for (TwoIntStore store : controlNames.keySet()) if (store.within(control)) {
                f = true;
                currenttip.add("Control: " + controlNames.get(store));
                break;
            }
            String[] extra = extraInfo(accessor, control);
            if (extra != null) for (String extraString : extra) currenttip.add(extraString);
        }
        return currenttip;
    }
