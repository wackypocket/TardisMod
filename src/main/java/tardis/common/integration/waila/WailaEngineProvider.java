...
public class WailaEngineProvider extends AbstractWailaProvider {

    {
        ...
        addControl(
            new TwoIntStore(11),
            "Upgrade energy regeneration level",
            "Upgrades the rate at which Artron Energy is generated");
        ...
        addControl(
            new TwoIntStore(40, 49),
            "Screwdriver permissions buttons",
            "Allows you to toggle permissions on a screwdriver");
        addControl(
            new TwoIntStore(50, 59),
            "Screwdriver permissions lights",
            "Displays which permissions a screwdriver has");
        ...
        addControl(
            new TwoIntStore(73),
            "Switch Console Room",
            "Right click then sneak right click to change console room");
    ...
    private boolean isEngine(IWailaDataAccessor accessor) {
        if ((accessor.getBlock() == TMRegistry.tardisEngineBlock)
            || ((accessor.getBlock() == TMRegistry.schemaComponentBlock) && (accessor.getMetadata() == 7))) return true;
        return false;
    }
    ...
    public String[] extraInfo(IWailaDataAccessor accessor, int control) {
        if (isEngine(accessor)) {
            ...
            if (eng != null) return eng.getExtraInfo(control);
        }
        return null;
    }
