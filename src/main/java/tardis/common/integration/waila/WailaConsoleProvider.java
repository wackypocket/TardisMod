...
public class WailaConsoleProvider extends AbstractWailaProvider {

    {
        ...
        addControl(
            new TwoIntStore(2),
            "Speedometer",
            "Displays the current speed after rooms have been taken into account.");
        ...
        addControl(
            new TwoIntStore(5),
            "Screwdriver Button",
            "Generates a new screwdriver or absorbs an old screwdriver");
        ...
        addControl(
            new TwoIntStore(53),
            "Relative Coords Switch",
            "Switches between using absolute (x=10 means you go to x=10) or relative (x=10 means you move 10 blocks to x) controls");
        ...
        addControl(
            ...
            "Switches between drifting (never landing) and coordinated (going to a destination) flight");
        addControl(
            new TwoIntStore(56),
            "Flight Stabilizer",
            "Prevents buttons from becoming unstable, allowing unsupervised (low xp) flights");
        ...
        addControl(
            new TwoIntStore(900),
            "Coordinate Save/Load Mode Control",
            "Controls whether pressing a save slot will save to or load that slot");
        ...
        addControl(
            new TwoIntStore(904),
            "Landing Pad Control",
            "Sets whether the TARDIS will attempt to find a landing pad to land on or not");
        addControl(new TwoIntStore(1000, 1019), "Save Slots");
        addControl(
            new TwoIntStore(1020, 1032),
            "Flight Controls",
    ...
    public String[] extraInfo(IWailaDataAccessor accessor, int control) {
        if (isConsole(accessor)) {
            World w = accessor.getWorld();
            ConsoleTileEntity con = Helper.getTardisConsole(w);
            if (con != null) return con.getExtraInfo(control);
        }
        return null;
    }
}
