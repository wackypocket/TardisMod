...
public class DimensionEventHandler {
    ...
    public void damageHandler(LivingHurtEvent event) {
        ...
        if (event.isCancelable() && event.source.getDamageType()
            .equals("player") && !event.source.equals(DamageSource.magic)) {
    ...
    private boolean savePlayer(EntityPlayer player, int dim, DamageSource source) {
        ...
        if ((core != null) && core.hasFunction(TardisFunction.TRANSMAT)) {
            if (core.canTransmatEntity(player)) {
                ...
                return true;
            } else SoundHelper.playSound(player, "tardismod:transmatFail", 0.6F, 1);
        }
        return false;
    }
    ...
    public void handleTick(ServerTickEvent event) {
        if (event.side.equals(Side.SERVER) && event.phase.equals(TickEvent.Phase.END)) {
            synchronized (playersToSave) {
                Iterator<String> plNameIter = playersToSave.keySet()
                    .iterator();
                while (plNameIter.hasNext()) {
                    ...
                    if (pl != null) {
                        ...
                        CoreTileEntity core = Helper.getTardisCore(dim);
                        if (core != null) core.transmatEntity(pl);
                        if ((source == DamageSource.starve) || (source == DamageSource.onFire)
                            || (source == DamageSource.wither)
                            || !Configs.deathTransmatLive) pl.attackEntityFrom(source, 200);
                    }
                    plNameIter.remove();
                }
            }
    ...
    public void handleMobSpawn(CheckSpawn event) {
        ...
        if (Helper.isTardisWorld(event.world)) {
            if (!Configs.enableTardisMobSpawning) event.setResult(Result.DENY);
            return;
        }
        ...
        if ((dimIDs == null) || dimIDs.isEmpty()) return;
        for (Integer dim : dimIDs) {
            ...
            if (ds.getEngine() == null) continue;
            SimpleDoubleCoordStore tPos = tardis.coords()
                .getCenter();
            double distance = tPos.distance(spawnPos);
            if (distance == -1) continue;
            double protectedRadius = ds.getEngine()
                .getProtectedSpawnRadius();
    ...
    public void handleLaserBlockBreak(ic2.api.event.LaserEvent.LaserHitsBlockEvent event) {
        ...
        if (bl instanceof AbstractBlock) if (TMRegistry.unbreakableBlocks.contains(bl)) event.setCanceled(true);
    }
