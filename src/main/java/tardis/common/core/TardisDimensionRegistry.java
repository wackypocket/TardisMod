...
public class TardisDimensionRegistry extends AbstractWorldDataStore implements GenericFutureListener {
    ...
    private HashSet<Integer> dimensionIDs = new HashSet<Integer>();

    public void addDimension(int id) {
        if (dimensionIDs.add(id)) {
            if (!DimensionManager.isDimensionRegistered(id)) registerDim(id);
    ...
    public static Set<Integer> getDims() {
        if (TardisMod.dimReg != null) return TardisMod.dimReg.dimensionIDs;
    ...
    public static void loadAll() {
        if (TardisMod.dimReg == null) {
            ...
            FMLCommonHandler.instance()
                .bus()
                .register(TardisMod.dimReg);
            MinecraftForge.EVENT_BUS.register(TardisMod.dimReg);
        }
        // Clear any previously cached dimension IDs when loading for a new server/world
        TardisMod.dimReg.dimensionIDs.clear();
        TardisMod.dimReg.load();
    }

    public static void saveAll() {
        if (ServerHelper.isServer()) TardisMod.dimReg.save();
    }

    public void registerDims() {
        for (Integer i : dimensionIDs) registerDim(i);
    }
    ...
    private boolean registerDim(int id) {
        if (!DimensionManager.isDimensionRegistered(id)) {
            ...
            if (ServerHelper.isServer()) DarkcoreMod.networkChannel.sendToAll(getPacket());
            return true;
        }
        return false;
    }
    ...
    public void unregisterDim(Integer dim) {
        ...
        if (provider == Configs.providerID) DimensionManager.unregisterDimension(dim);
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        // Ensure we start fresh for this world's data
        dimensionIDs.clear();
        int[] dims = io.darkcraft.darkcore.mod.nbt.NBTUtils.getIntArray(nbt, "registeredDimensions");
        if (dims != null) for (int curr : dims) addDimension(curr);
        saveAll();
    }
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        int i = 0;
        for (Integer curr : dimensionIDs) dims[i++] = curr;
    ...
    public void sendPacket(PlayerEvent event) {
        ...
        if (pl instanceof EntityPlayerMP) DarkcoreMod.networkChannel.sendTo(getPacket(), (EntityPlayerMP) pl);
    }
