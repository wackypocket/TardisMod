...
public class SchemaRotationHandler {
    ...
    private static boolean facingOpposite(int a, int b) {
        if (((a == 0) && (b == 2)) || ((b == 0) && (a == 2))) return true;
        if (((a == 1) && (b == 3)) || ((b == 1) && (a == 3))) return true;
        return false;
    }
    ...
    private static int getChestMetadata(int blockMeta, int oldFacing, int newFacing) {
        ...
        TardisOutput
            .print("TSRH", "ChestRot:" + blockMeta + "," + newMeta + " for facing:" + oldFacing + "," + newFacing);
        return newMeta;
    }
    ...
    private static int getStairsMetadata(int blockMeta, int oldFacing, int newFacing) {
        ...
        TardisOutput
            .print("TSRH", "Stair:" + blockMeta + "," + add + "->" + newMeta + "#" + oldFacing + "," + newFacing);
        return newMeta + add;
    }
    ...
    public static int getNewMetadata(Block block, int blockMeta, int oldFacing, int newFacing) {
        if (oldFacing == newFacing) return blockMeta;
        if (block == Blocks.chest) return getChestMetadata(blockMeta, oldFacing, newFacing);
        if (block instanceof BlockStairs) return getStairsMetadata(blockMeta, oldFacing, newFacing);
