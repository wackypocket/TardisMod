...
public class ScrewdriverHelper {
    ...
    protected ScrewdriverHelper(ItemStack is, int _id) {
        ...
        itemstack = is;
        if (is.hasDisplayName()) displayName = is.getDisplayName();
    }
    ...
    protected void setItemStack(ItemStack is) {
        itemstack = is;
        if ((is != null) && (is.stackTagCompound != null)) readFromNBT(is.stackTagCompound);
        markDirty();
    }
    ...
    public ItemStack getItemStack() {
        ...
        writeToNBT(itemstack.stackTagCompound);
        if (displayName != null) itemstack.setStackDisplayName(displayName);
        return itemstack;
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        mode = ScrewdriverMode.get(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "scMo", mode.ordinal()));
        perms = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "perm", perms);
        schemaCat = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "schemaCat", schemaCat);
        schema = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "schemaName", schema);
        linkSCS = io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "linkscs")
            ? SimpleCoordStore.readFromNBT(nbt, "linkscs")
            : null;
        displayName = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "dispName", displayName);
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (linkSCS != null) linkSCS.writeToNBT(nbt, "linkscs");
        else if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "linkscs")) nbt.removeTag("linkscs");
        type.writeToNBT(nbt);
        if (schema != null) {
            nbt.setString("schemaName", schema);
            if (schemaCat != null) nbt.setString("schemaCat", schemaCat);
        }
        if (displayName != null) nbt.setString("dispName", displayName);
    }
    ...
    public void setPermission(ScrewdriverMode setMode, boolean newValue) {
        ...
        if (newValue) perms = perms | value;
        else perms = perms & ~value;
        markDirty();
    }
    ...
    public void setOwner(String newOwner) {
        if (isVoid(owner)) resetPermissions(true);
        else if (!owner.equals(newOwner)) resetPermissions(false);
        owner = newOwner;
        markDirty();
    }
    ...
    public CoreTileEntity getLinkedCore() {
        ...
        if (dim != null) return Helper.getTardisCore(dim);
        return null;
    }

    public TardisDataStore getLinkedDS() {
        Integer dim = getLinkedDimID();
        if (dim != null) return Helper.getDataStore(dim);
        return null;
    }
    ...
    private ILinkable getILinkable(SimpleCoordStore scs) {
        if (scs != null) {
            ...
            if (te instanceof ILinkable) return (ILinkable) te;
            if (b instanceof ILinkable) return (ILinkable) b;
        }
        return null;
    }
    ...
    private boolean clearLinkSCS(EntityPlayer pl, boolean mess) {
        if (mess && ServerHelper.isServer()) ServerHelper
            .sendString(pl, SonicScrewdriverItem.screwName, "Link target has been cleared (no linkable object)");
        setLinkSCS(null);
        return true;
    }
    ...
    public boolean linkUsed(EntityPlayer pl, SimpleCoordStore usedPos) {
        ...
        if ((usedOn == null) || !usedOn.isLinkable(usedPos)) // Used on something that isn't ILinkable, so clear the
                                                             // link
        {
            if (linkSCS != null) return clearLinkSCS(pl, true);
            return false;
        }
            ...
            if (usedOn.link(pl, usedPos, linkSCS)) {
                if (linkLinkable.link(pl, linkSCS, usedPos)) {
                    ...
                    if (ServerHelper.isServer()) ServerHelper
                        .sendString(pl, SonicScrewdriverItem.screwName, "Linked " + linkSCS + " to " + usedPos);
                    return true;
                } else usedOn.unlink(pl, usedPos);
            }
        }
