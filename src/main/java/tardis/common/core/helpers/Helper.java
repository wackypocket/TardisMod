...
public class Helper {
    ...
    public static HashMap<Integer, TardisDataStore> datastoreMap = new HashMap<Integer, TardisDataStore>();
    public static HashMap<Integer, SaveSlotNamesDataStore> ssnDatastoreMap = new HashMap<Integer, SaveSlotNamesDataStore>();
    ...
    public static void teleportEntityToSafety(Entity ent) {
        if ((ent.worldObj.provider) instanceof TardisWorldProvider) {
            if (ent instanceof EntityPlayer) {
                ...
                if (core != null) core.enterTardis((EntityPlayer) ent, true);
                else TeleportHelper.teleportEntity(ent, 0, tardisCoreX - 2, tardisCoreY, tardisCoreZ);
            } else {
    ...
    public static void generateTardisInterior(int dimID, String ownerName, TardisTileEntity exterior) {
        ...
        if ((te != null) && (ds != null)) {
            // Default new interiors to space projection
            ds.setSpaceProjection(true);
            te.setOwner(ownerName);
            if (exterior != null) {
                ds.linkToExterior(exterior);
                ConsoleTileEntity con = te.getConsole();
                if (con != null) con.setControls(exterior, true);
            }
        }
    }
    ...
    private static int getNextFreeDimID() {
        // Prefer the lowest available positive dimension id for a fresh world.
        int startPoint = 2;
        while (!isFree(startPoint)) startPoint++;
        return startPoint;
    }
    ...
    public static boolean summonNewTardis(EntityPlayer player) {
        if (TardisMod.plReg.getDimension(player) != null) return false;
    ...
    public static boolean summonOldTardis(int dim, EntityPlayer player) {
        ...
        if (te != null) {
            ...
            if (con != null) con.setControls(te, true);
            return true;
        } else TardisOutput.print("Helper", "No exterior :(");
        return false;
    }
    ...
    private static TardisTileEntity summonTardisExterior(EntityPlayer player) {
        ...
        int facing = net.minecraft.util.MathHelper.floor_double((double) (player.rotationYaw * 4.0F / 360.0F) + 0.5D)
            & 3;

        // Calculate offset based on facing direction (2-3 blocks in front)
        int frontX = x, frontZ = z;
        switch (facing) {
            case 0: // South
                frontZ += 3;
                break;
        ...
        boolean requireGround = true;

        // First, try to place in front of player
        for (int pass = 0; pass < 2 && (place == null); pass++) {
            requireGround = pass == 0;
            for (int yOf = 0; yOf < 7; yOf++) {
                ...
                for (int xO : validSpotRanges) {
                    ...
                    for (int zO : validSpotRanges) {
                        if ((y > 1) && (y < 253)) {
                            int checkX = frontX + xO;
                            int checkY = y + yO;
                            int checkZ = frontZ + zO;

                            if (canPlaceSummonedExterior(w, checkX, checkY, checkZ, requireGround)) {
                                place = new SimpleCoordStore(w, checkX, checkY, checkZ);
                                break;
                            }
                        }
                    }
        ...
        // If no spot found in front, fall back to original behavior (around player)
        if (place == null) {
            for (int pass = 0; pass < 2 && (place == null); pass++) {
                requireGround = pass == 0;
                for (int yOf = 0; yOf < 7; yOf++) {
        ...
        if (place != null) {
            // Calculate TARDIS facing direction (same as player's facing so the door faces the player)
            int tardisFacing = facing; // Same direction as player

            // Clear soft blocks in the 2-block-tall footprint so summon can land where intended.
            clearSummonSpace(w, place.x, place.y, place.z);

            w.setBlock(place.x, place.y, place.z, TMRegistry.tardisBlock, tardisFacing, 3);
            w.setBlock(place.x, place.y + 1, place.z, TMRegistry.tardisTopBlock, tardisFacing, 3);
    ...
    private static boolean isSummonClearable(World w, int x, int y, int z) {
    ...
    private static boolean canPlaceSummonedExterior(World w, int x, int y, int z, boolean requireGround) {
        if (!isSummonClearable(w, x, y, z)) return false;
        if (!isSummonClearable(w, x, y + 1, z)) return false;
        if (!requireGround) return true;
        if (y <= 0) return false;
        return !WorldHelper.softBlock(w, x, y - 1, z);
    }

    private static void clearSummonSpace(World w, int x, int y, int z) {
        clearSummonBlock(w, x, y, z);
        clearSummonBlock(w, x, y + 1, z);
    }

    private static void clearSummonBlock(World w, int x, int y, int z) {
        if (w.isAirBlock(x, y, z)) return;
        Block b = w.getBlock(x, y, z);
        if (b == null) return;
        if (!isSummonClearable(w, x, y, z)) return;
    ...
    public static void repairConsole(World w) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                if ((x == 0) && (z == 0)) continue;
                if (w.getBlock(x, y, z) != TMRegistry.schemaComponentBlock)
                    w.setBlock(x, y, z, TMRegistry.schemaComponentBlock, 3, 3);
            }
        }
        if (w.getBlock(0, y + 1, 0) != TMRegistry.schemaComponentBlock)
            w.setBlock(0, y + 1, 0, TMRegistry.schemaComponentBlock, 6, 3);
        if (w.getBlock(0, tardisCoreY + 1, 0) != TMRegistry.schemaComponentBlock)
            w.setBlock(0, tardisCoreY + 1, 0, TMRegistry.schemaComponentBlock, 8, 3);
        if (w.getBlock(0, tardisCoreY - 5, 0) != TMRegistry.tardisEngineBlock)
            w.setBlock(0, tardisCoreY - 5, 0, TMRegistry.tardisEngineBlock);
    }

    public static boolean isBlockRemovable(Block blockID) {
        if (blockID == TMRegistry.tardisCoreBlock) return false;
        else if (blockID == TMRegistry.tardisConsoleBlock) return false;
    ...
    public static void loadSchemaDiff(String fromName, String toName, World worldObj, int xCoord, int yCoord,
        int zCoord, int facing) {
    ...
    public static void spawnParticle(ParticleType type, int dim, double x, double y, double z, int count,
        boolean rand) {
    ...
    public static CoreTileEntity getTardisCore(int dimensionID) {
        ...
        if (tardisWorld != null) return getTardisCore(tardisWorld);
        else TardisOutput.print("TH", "No world passed", TardisOutput.Priority.DEBUG);
        return null;
    }
    ...
    public static IArtronEnergyProvider getArtronProvider(TileEntity source, boolean search) {
        ...
        if (search) for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
        ...
        if (iaep == null) return getTardisCore(w);
        return iaep;
    }

    public static TardisDataStore getDataStore(World w) {
        if (Helper.isTardisWorld(w)) return getDataStore(WorldHelper.getWorldID(w));
        else return null;
    }
    ...
    public static Integer getTardisDim(String dimDescriptor) {
        Integer w = 0;
        if (dimDescriptor.startsWith("#")) return TardisMod.plReg.getDimension(dimDescriptor.replaceFirst("#", ""));
        else return Integer.parseInt(dimDescriptor);
    }
}
