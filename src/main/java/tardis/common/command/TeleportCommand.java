            ...
            if (astring.size() >= 1) {
                ...
                for (int i = 0; (i < astring.size()) && !nonPlayerFound; i++) {
                    ...
                    if (plx != null) {
                        pls.add(plx);
                        offset++;
                    } else nonPlayerFound = true;
                }
                if (offset < astring.size()) {
                    if (pls.size() == 0) pls.add(pl);
                    ...
                    if (astring.size() >= (offset + 4)) {
                        int x = MathHelper.toInt(astring.get(1 + offset), Integer.MIN_VALUE);
                        int y = MathHelper.toInt(astring.get(2 + offset), Integer.MIN_VALUE);
                        int z = MathHelper.toInt(astring.get(3 + offset), Integer.MIN_VALUE);
                        if ((x == Integer.MIN_VALUE) || (y == Integer.MIN_VALUE) || (z == Integer.MIN_VALUE)) {
                            sendString(comSen, "Coordinates must be numbers");
                            return true;
                        }
                        for (EntityPlayerMP plx : pls) TeleportHelper.teleportEntity(plx, w, x, y, z);
                    } else if (astring.get(offset)
                        .startsWith("#")) {
                            for (EntityPlayerMP plx : pls)
                                TeleportHelper.teleportEntity(plx, w, 2, Helper.tardisCoreY, 0);
                        } else {
                            for (EntityPlayerMP plx : pls) TeleportHelper.teleportEntity(plx, w);
                        }
                } else if (pls.size() > 0) {
                    EntityPlayerMP dest = pls.get(pls.size() - 1);
                    if (pls.size() == 1) pls.add(pl);

                    for (EntityPlayerMP plx : pls) {
                        if (plx != dest) TeleportHelper
                            .teleportEntity(plx, WorldHelper.getWorldID(dest), dest.posX, dest.posY, dest.posZ);
                    }
                } else {
