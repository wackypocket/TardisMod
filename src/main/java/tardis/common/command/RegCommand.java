...
public class RegCommand extends AbstractCommand {
    ...
    public void commandBody(ICommandSender comsen, String[] astring) {
        if (astring.length == 0) sendString(comsen, getCommandUsage(comsen));
        else {
            if (astring[0].equals("list")) TardisMod.plReg.chatMapping(comsen);
            else if (astring[0].equals("remove")) {
                if (astring.length == 2) {
                    String un = astring[1];
                    if (TardisMod.plReg.removePlayer(un)) sendString(comsen, un + " removed from listing");
                    else sendString(comsen, un + " could not be removed");
                    ...
                    if (astring[1].equals("total")) {
                        ...
                        if (dim != null) {
                            ...
                            if (w != null) {
                                ...
                                for (Object o : l) {
                                    if (o instanceof Entity) TeleportHelper.teleportEntityToOverworldSpawn((Entity) o);
                                }
                                CoreTileEntity core = Helper.getTardisCore(w);
                                if (core != null) core.removeAllRooms(true);
                                ...
                                TileEntity te = w
                                    .getTileEntity(Helper.tardisCoreX, Helper.tardisCoreY - 10, Helper.tardisCoreZ);
                                if (te instanceof SchemaCoreTileEntity) ((SchemaCoreTileEntity) te).remove(false);
                ...
                if (astring.length == 3) {
                    ...
                    if (TardisMod.plReg.addPlayer(un, dim)) sendString(comsen, un + " added to listing");
                    else sendString(comsen, un + " could not be added to listing");
                }
            } else if (astring[0].equals("listdims")) {
                Set<Integer> dims = TardisDimensionRegistry.getDims();
                String s = "Dims: ";
                for (Integer i : dims) s += i + ",";
                sendString(comsen, s);
            }
        }
    }
