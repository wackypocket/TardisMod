...
public class SwitchConsoleCommand extends AbstractCommand {
    ...
    public void commandBody(ICommandSender icommandsender, String[] astring) {
        if (ServerHelper.isClient() || (astring.length == 0)) return;
        World w = null;
        if (astring.length == 2) w = WorldHelper.getWorldServer(MathHelper.toInt(astring[0], 0));
        else if (icommandsender instanceof EntityPlayer) w = ((EntityPlayer) icommandsender).worldObj;
        ...
        if (core != null) core.loadConsoleRoom(name);
    }
