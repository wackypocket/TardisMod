...
public class SetValueCommand extends AbstractCommand {
    ...
    public void commandBody(ICommandSender icommandsender, String[] args) {
        int dim = 0;
        if (icommandsender instanceof EntityPlayer) dim = WorldHelper.getWorldID((EntityPlayer) icommandsender);
        if ((args.length < 2) || (args.length > 3)) return;
        if (args.length == 3) dim = MathHelper.toInt(args[0], dim);
        if (!Helper.isTardisWorld(dim)) return;
        String variable = args[args.length - 2];
        String value = args[args.length - 1];
        int iVal = MathHelper.toInt(value, 0);
        if (variable.equalsIgnoreCase("energy") || variable.equalsIgnoreCase("en")) setEnergy(dim, iVal);
        if (variable.equalsIgnoreCase("shields") || variable.equalsIgnoreCase("sh")) setShields(dim, iVal);
        if (variable.equalsIgnoreCase("hull") || variable.equalsIgnoreCase("hu")) setHull(dim, iVal);
    }
    ...
    private void setEnergy(int dim, int val) {
        ...
        if (en < val) core.addArtronEnergy(val - en, false);
        if (en > val) core.takeArtronEnergy(en - val, false);
    }
