...
public class XpCommand extends AbstractCommand {
    ...
    private void xp(String dest, double amount) {
        ...
        if (dim == 0) dim = TardisMod.plReg.getDimension(dest);
        if ((dim != null) && (dim != 0)) {
    ...
    private void xp(TardisDataStore ds, double amount) {
        if ((ds != null) && (amount != 0)) ds.addXP(amount);
    }

    @Override
    public void commandBody(ICommandSender comsen, String[] astring) {
        if (astring.length == 2) xp(astring[0], MathHelper.toDouble(astring[1], 0));
        else if (astring.length == 1) xp(comsen.getCommandSenderName(), MathHelper.toDouble(astring[0], 0));
        else getCommandUsage(comsen);
    }
