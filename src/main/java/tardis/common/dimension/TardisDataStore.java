...
public class TardisDataStore extends AbstractWorldDataStore {
    ...
    private AspectList aspectList = ThaumcraftIntegration.isThaumcraftAvailable() ? new AspectList() : null;
    public int maxSuck = 16;
    public Aspect maxSuckT = null;
    public int desiredDim = 0;

    private int daytimeSetting = 0;
    private boolean spaceProjection = true;
    ...
    public boolean hasValidExterior() {
        ...
        if (w != null) {
            ...
            if (b == TMRegistry.tardisBlock) return true;
            else System.out
                .println("Ext not tb :" + b.toString() + ":" + exteriorX + "," + exteriorY + "," + exteriorZ);
        } else if (exteriorWorld != 10000) System.err.println("Failed to get exterior world");
        return false;
    }
    ...
    public double addXP(double a) {
        ...
        if (tardisXP >= getXPNeeded()) {
            ...
            SoundHelper
                .playSound(dimID, Helper.tardisCoreX, Helper.tardisCoreY, Helper.tardisCoreZ, "tardismod:levelup", 1);
        }
    ...
    public int spentLevelPoints() {
        int spentPoints = 0;
        for (TardisUpgradeMode mode : upgradeLevels.keySet()) spentPoints += upgradeLevels.get(mode);
        return spentPoints;
    }
    ...
    public ItemStack setIS(ItemStack is, int slot) {
        ...
        if ((is != null) && (is.stackSize > 0)) items[slot] = is;
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        exteriorWorld = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "extW", exteriorWorld);
        exteriorX = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "extX", exteriorX);
        exteriorY = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "extY", exteriorY);
        exteriorZ = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "extZ", exteriorZ);
        pExtW = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "pExtW", pExtW);
        pExtX = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "pExtX", pExtX);
        pExtY = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "pExtY", pExtY);
        pExtZ = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "pExtZ", pExtZ);
        rfStored = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "rS", rfStored);
        desiredDim = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "desDim", desiredDim);
        if (NBTUtils.hasCompound(nbt, "invStore")) {
            NBTTagCompound invTag = NBTUtils.getCompoundOrEmpty(nbt, "invStore");
            for (int i = 0; i < items.length; i++) {
                if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(invTag, "i" + i))
                    items[i] = ItemStack.loadItemStackFromNBT(NBTUtils.getCompoundOrEmpty(invTag, "i" + i));
            }
        }
        if (NBTUtils.hasCompound(nbt, "fS")) {
            NBTTagCompound invTag = NBTUtils.getCompoundOrEmpty(nbt, "fS");
            for (int i = 0; i < fluids.length; i++) {
                if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(invTag, "i" + i))
                    fluids[i] = FluidStack.loadFluidStackFromNBT(NBTUtils.getCompoundOrEmpty(invTag, "i" + i));
                else fluids[i] = null;
            }
        }
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        if (ThaumcraftIntegration.isThaumcraftAvailable()
            && io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "aspectList")) {
            if (aspectList == null) aspectList = new AspectList();
            aspectList.readFromNBT(nbt, "aspectList");
        }
        tardisLevel = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "tL", tardisLevel);
        tardisXP = io.darkcraft.darkcore.mod.nbt.NBTUtils.getDouble(nbt, "txp", tardisXP);
        permissionList.clear();
        {
            int i = 0;
            while (NBTUtils.hasCompound(nbt, "permO" + i)) {
                int h = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "permO" + i, 0);
                int d = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "permD" + i, 0);
                if (d != 0) permissionList.put(h, d);
                i++;
            }
        }
        for (TardisUpgradeMode mode : TardisUpgradeMode.values())
            if (NBTUtils.hasCompound(nbt, "uG" + mode.ordinal())) {
                int am = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "uG" + mode.ordinal(), 0);
                if (am > 0) upgradeLevels.put(mode, am);
            }
        if (NBTUtils.hasCompound(nbt, "damage")) {
            NBTTagCompound damageNBT = NBTUtils.getCompoundOrEmpty(nbt, "damage");
            damage.readFromNBT(damageNBT);
        }

        EngineTileEntity eng = getEngine();
        SimpleCoordStore engPos = null;
        if (eng != null) engPos = eng.coords;
        for (int i = 0; i < upgrades.length; i++) {
            if (NBTUtils.hasCompound(nbt, "upgrade" + i)) {
                if (upgrades[i] != null) upgrades[i].readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, "upgrade" + i));
                else upgrades[i] = UpgradeFactory.createUpgrade(NBTUtils.getCompoundOrEmpty(nbt, "upgrade" + i));
            } else if (upgrades[i] != null) {
                upgrades[i] = null;
            }
            if ((upgrades[i] != null) && (engPos != null)) upgrades[i].setEnginePos(engPos);
        }
        spaceProjection = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "spaceProjection", spaceProjection);
        daytimeSetting = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "dayNightControl", daytimeSetting);
    }
    ...
    private void storeInv(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < items.length; i++) {
            if (items[i] != null) {
                save = true;
                NBTTagCompound iTag = new NBTTagCompound();
                // Preserve the real slot index so reload maps items back to the same slot.
                invTag.setTag("i" + i, items[i].writeToNBT(iTag));
            }
        }
        if (save) nbt.setTag("invStore", invTag);
    }
    ...
    public void writeTransmittable(NBTTagCompound nbt) {
        if (ThaumcraftIntegration.isThaumcraftAvailable() && aspectList != null) {
    ...
    public AspectList getAspectList() {
        if (!ThaumcraftIntegration.isThaumcraftAvailable() || aspectList == null) {
            return new AspectList(); // Return an empty list if Thaumcraft is not installed
        }
        return aspectList;
    }

    public void setAspectList(AspectList al) {
        if (!ThaumcraftIntegration.isThaumcraftAvailable()) return;
        aspectList = al;
        markDirty();
    }
    ...
    public int addAspect(Aspect a, int am) {
        if (!ThaumcraftIntegration.isThaumcraftAvailable() || aspectList == null) return am;
        int cAm = aspectList.getAmount(a);
    ...
    public boolean removeAspect(Aspect a, int am) {
        if (!ThaumcraftIntegration.isThaumcraftAvailable() || aspectList == null) return false;
        int cAm = aspectList.getAmount(a);
    ...
    public String getOwnerName() {
        ...
        if (cte != null) return cte.getOwner();
        return "##NO OWNER##";
    }
    ...
    public void tick() {
        damage.tick();
        if (!ServerHelper.isIntegratedClient()) tt = (tt + 1) % 827026200;
        for (AbstractUpgrade up : upgrades) {
            if ((up != null) && !ServerHelper.isIntegratedClient()) up.tick(tt);
        }
    }

    public AbstractTardisChameleon getChameleon() {
        for (AbstractUpgrade up : upgrades)
            if (up instanceof ChameleonUpgrade) return ((ChameleonUpgrade) up).chameleon;
    ...
    public void setDaytimeSetting(int var) {
        int newVal = ((var % 3) + 3) % 3;
        if (daytimeSetting != newVal) {
            daytimeSetting = newVal;
            markDirty();
        }
    }

    public void setSpaceProjection(boolean var) {
        if (spaceProjection != var) {
            spaceProjection = var;
            markDirty();
        }
    }
