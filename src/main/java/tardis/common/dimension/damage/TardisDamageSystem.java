...
public class TardisDamageSystem {
    ...
    public static void refreshConfigs() {
        ...
        maxShieldsInc = config
            .getInt("Max shields increase", 500, "How much a level of max shields increases the amount of shielding");

        explosionDamageMult = config.getDouble(
            "explosionDamageMult",
            100,
            "Explosion damage is multiplied by this number before being applied to a TARDIS");
        missedDamageMult = config.getDouble(
            "missedDamageMult",
            30,
            "Damage from missing a blue control is multiplied by this number before being applied to a TARDIS");
    ...
    private void fillInRepair() {
        ...
        Pair<Item, Integer> kontronPair = new Pair(
            TMRegistry.craftingComponentItem,
            CraftingComponentType.KONTRON.ordinal());
        Pair<Item, Integer> dalek = new Pair(
            TMRegistry.craftingComponentItem,
            CraftingComponentType.DALEKANIUM.ordinal());
        Pair<Item, Integer> chrono = new Pair(
            TMRegistry.craftingComponentItem,
            CraftingComponentType.CHRONOSTEEL.ordinal());
        repairComps[0] = kontronPair;
        repairComps[1] = dalek;
        repairComps[2] = dalek;
        repairComps[4] = dalek;
        repairComps[5] = dalek;
        for (int i = 1; i < (numBreakables - 2); i++) if (repairComps[i] == null) repairComps[i] = chrono;
    ...
    public TardisDamageSystem(TardisDataStore parent) {
        ...
        hull = getMaxHull();
        prevHull = hull;
        for (int i = 0; i < breakables.length; i++) breakables[i] = false;
    }
    ...
    private void breakComponent() {
        ...
        for (int i = 1; i <= numBreakables; i++) if (!breakables[i % numBreakables]) {
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < numBreakables; i++) nbt.setBoolean("b" + i, breakables[i]);
    }

    public void readFromNBT(NBTTagCompound nbt) {
        if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "shields")) {
            hull = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "hull", hull);
            prevHull = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "prevHull", prevHull);
            shields = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "shields", shields);
            for (int i = 0; i < numBreakables; i++)
                if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "b" + i))
                    breakables[i] = io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "b" + i, breakables[i]);
        }
    }
    ...
    public synchronized void tick() {
        if (prevHull != hull) {
            ...
            while (currentClass < prevHullClass--) breakComponent();
            prevHull = hull;
            ds.sendUpdate();
        }
