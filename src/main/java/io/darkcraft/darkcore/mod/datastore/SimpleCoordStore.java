...
public class SimpleCoordStore {
    ...
    private static final Mapper<SimpleCoordStore> MAPPER = NBTHelper
        .getMapper(SimpleCoordStore.class, SerialisableType.WORLD);
    ...
    public static SimpleCoordStore[] getTouching(SimpleDoubleCoordStore pos, boolean includeCeilHeight) {
        ...
        if (includeCeilHeight) {
            slots[4] = new SimpleCoordStore(
                w,
                MathHelper.floor(pos.x),
                MathHelper.ceil(pos.y),
    ...
    public SimpleDoubleCoordStore travelTo(SimpleCoordStore dest, double amountTravelled, boolean newWorld) {
        if (Double.isNaN(amountTravelled) || Double.isInfinite(amountTravelled)) return null;
    ...
    public <T> T getTileEntity(Class<T> clazz) {
        ...
        if (clazz.isInstance(t)) return (T) t;
        return null;
    }
}
