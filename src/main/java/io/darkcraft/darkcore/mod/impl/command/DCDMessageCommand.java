...
public class DCDMessageCommand extends AbstractCommandNew {
    ...
    public boolean process(ICommandSender sen, List<String> strList) {
        ...
        if (lSize == 1) MessageHelper.sendToAll(s);
        else if (lSize == 2)
            MessageHelper.sendToAll(s, MathHelper.toInt(strList.get(1), MessagePacketHandler.secondsDefault));
        else if (lSize == 3) MessageHelper.sendToAll(new ResourceLocation(strList.get(1), strList.get(2)), s);
        else if (lSize == 4) MessageHelper.sendToAll(
            new ResourceLocation(strList.get(2), strList.get(3)),
            s,
