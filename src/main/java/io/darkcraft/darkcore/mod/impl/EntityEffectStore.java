...
public class EntityEffectStore extends AbstractEntityDataStore {
    ...
    public EntityEffectStore(Entity ent) {
        super(ent, disc);
        if ((ent == null) || (ent.worldObj == null)) client = false;
        else client = ent.worldObj.isRemote;
    }
    ...
    public void tick() {
        ...
        Iterator<Entry<String, AbstractEffect>> iter = effects.entrySet()
            .iterator();
        while (iter.hasNext()) {
    ...
    public boolean shouldBeWatched() {
        ...
        for (AbstractEffect eff : effects.values()) if ((eff.duration != -1) || eff.doesTick) return true;
        return false;
    }
    ...
    public void addEffect(AbstractEffect effect) {
        ...
        if (effect.canStack && (old != null)) {
            if (effect instanceof StackedEffect) throw new RuntimeException("Cannot add two stacked effects!");
            if (old instanceof StackedEffect) effects.put(id, StackedEffect.addEffect((StackedEffect) old, effect));
            else effects.put(id, new StackedEffect(old, effect));
        } else {
            if (old != null) {
        ...
        if (shouldBeWatched()) EffectHandler.addWatchedStore(this);
        queueUpdate();
    }
    ...
    public void writeTransmittable(NBTTagCompound nbt) {
        ...
        if (i == 0) nbt.setBoolean("empty", true);
    }
    ...
    public void readTransmittable(NBTTagCompound nbt) {
        ...
        effects.clear();
        int i = 0;
        while (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "eff" + i)) {
            NBTTagCompound effTag = NBTUtils.getCompoundOrEmpty(nbt, "eff" + i);
            AbstractEffect effect = EffectHandler.getEffect(getEntity(), effTag);
            if (effect != null) addEffect(effect);
            else System.err.println("Effect failed to load");
            i++;
        }
        if (shouldBeWatched()) EffectHandler.addWatchedStore(this);
    }
