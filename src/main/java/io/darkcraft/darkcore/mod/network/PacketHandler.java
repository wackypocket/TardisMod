...
public class PacketHandler {
    ...
    public void handleCustomPacket(CustomPacketEvent event) {
        io.darkcraft.darkcore.mod.logging.TMLLog.debug("PKT", "Packet received!");
        FMLProxyPacket p = event.packet;
        int length = p.payload()
            .getByte(0);
        ...
        p.payload()
            .readBytes(bytes);
        p.payload()
            .readerIndex(1 + length);
        p.payload()
            .discardReadBytes();
        String discriminator = new String(bytes);
        DataPacket dp = new DataPacket(p.payload());
        NBTTagCompound nbt = dp.getNBT();
        if (handlers.containsKey(discriminator)) handlers.get(discriminator)
            .handleData(nbt);
        else System.err.println("Packet with unknown discriminator " + discriminator + " received!");
    }
    ...
    public boolean registerHandler(String discriminator, IDataPacketHandler handler) {
        if (handlers.containsKey(discriminator)) {
            ...
            throw new RuntimeException(
                "Unable to register packet handler " + discriminator + " - conflict " + handler + ":" + old);
        }
