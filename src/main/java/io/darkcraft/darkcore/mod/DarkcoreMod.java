...
/**
 * When creating a mod using DarkCore, if that mod implements new blocks/items, you should call
 * {@link #registerCreativeTab(String, CreativeTabs)} before instantiating your items
 * 
 * @author dark
 */
@Mod(modid = "darkcore", version = tardis.TardisMod.VERSION)
public class DarkcoreMod implements IConfigHandlerMod {

    @Instance("darkcore")
    public static DarkcoreMod instance;

    @SidedProxy(
        clientSide = "io.darkcraft.darkcore.mod.proxy.ClientProxy",
    ...
    public static boolean repostMessage = false;
    private static String[] repostMessages = {
        "DarkCore: If you have downloaded this mod from anywhere using an ad-wall or other method of gaining money from this mod then that site is breaking this mod's license.",
        "Please download from official sources", "You can disable this message in the DarkCore config file", "" };
    ...
    public static void refreshConfigs() {
        if (config == null) config = configHandler.registerConfigNeeder("TardisCore");
        ...
        repostMessage = config.getBoolean(
            "Display repost message",
            false,
            "Set to false if you have seen the repost message",
            "and are aware the correct download location",
            "is http://minecraft.curseforge.com/mc-mods/230170-tardis-mod",
            "and the forum post is http://www.minecraftforum.net/forums/mapping-and-modding/minecraft-mods/2420176-tardis-mod");
        String bannedSoundsData = config.getString(
            "Banned sounds",
            "",
            "Insert a list of comma separated sounds to be banned",
            "e.g. tardismod:levelup will disable the level up sound for TARDISes");
        chunkLoadCheckTime = config.getInt(
            "Chunk loading check time",
            200,
            "The number of ticks between chunk loaders being checked and unloaded if necessary");
        reloadNullTicket = config.getBoolean(
            "Reload null ticket",
            true,
            "Attempt to reload a chunk loader if it doesn't have a ticket assigned");
        splitTime = config.getBoolean("12h time format", true);
        String[] bannedSoundsBlobs = bannedSoundsData.split(",");
        bannedSounds.clear();
        for (String s : bannedSoundsBlobs) if (!s.isEmpty()) bannedSounds.add(s);
    }
    ...
    public void repostWarning(PlayerLoggedInEvent event) {
        ...
        io.darkcraft.darkcore.mod.logging.TMLLog.debug("DC", "RW");
        EntityPlayer pl = event.player;
        if (repostMessage) for (String s : repostMessages) ServerHelper.sendString(pl, s);
    }
}
