...
public class ConfigHandlerFactory {
    ...
    public static void setConfigDir(File f) {
        ...
        // Store all mod configs under a single TardisMod folder inside the Minecraft config dir
        f = new File(f, "TardisMod");
    ...
    public static ConfigHandler getConfigHandler(IConfigHandlerMod ichm) {
        ...
        // Map certain mod IDs to friendly folder names
        String folderName = modID;
        if ("darkcore".equalsIgnoreCase(modID)) folderName = "TardisCore";
        if (chMap.containsKey(modID)) return chMap.get(modID);
        File f = new File(configDirectory, folderName);
        if ((!f.exists()) || (!f.isDirectory())) f.mkdir();
