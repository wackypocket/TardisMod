 ...
 *
 * Constructor should just contain a call to super with the mod id in or alternatively either a
 * {@link net.minecraft.block.material.Material} or a boolean (true if render,false if no render) followed by the mod
 * id.<br>
 *
...
public abstract class AbstractBlock extends Block implements IRecipeContainer {
    ...
    private String sm;
    public static int[] colorArray = new int[] { 1973019, 11743532, 3887386, 10583369, 2437522, 8073150, 2651799,
    ...
    public final static void refreshConfigs() {
        ...
        coloredUsesDye = config.getBoolean(
            "Colored blocks use up dye",
            false,
            "If true, when dying a colored block, the dye will be used up.",
            "If false, the dye will not be used up");
        maxColorSpread = config.getInt(
            "Max color spread",
            16,
            "How many blocks away from the block you sneak right click should colors spread");
    ...
    public AbstractBlock(boolean render, Material blockMaterial, String sourcemod) {
        ...
        initData();
        renderIcon = render;
        if (this instanceof IColorableBlock) setSubNames(ItemDye.field_150923_a);
     ...
     * 
     * @return this, for the purposes of chaining with the constructor. E.g. AbstractBlock block = new
     *         MyBlock().register()
     */
    public AbstractBlock register() {
        Class<? extends AbstractItemBlock> aib = getIB();
        if (aib != null) GameRegistry.registerBlock(this, aib, getUnlocalizedName());
        else GameRegistry.registerBlock(this, getUnlocalizedName());
        return this;
    }
    ...
    public void getSubBlocks(Item itemID, CreativeTabs tab, List<ItemStack> itemList) {
        int numItems;
        if (this instanceof IColorableBlock) itemList.add(new ItemStack(itemID, 1, 15));
        else {
    ...
    public String getUnlocalizedName(int damage) {
        ...
        if ((numSubnames == 0) || (this instanceof IColorableBlock)) return "tile." + sm + "." + unlocalizedFragment;
        else {
    ...
    public void registerBlockIcons(IIconRegister register) {
        ...
        if ((subIcons != null) && !(this instanceof IColorableBlock)) {
            ...
            if (subNames != null) {
                for (int i = 0; i < subNames.length; i++) {
                    ...
                    if (suffixes == null) {
                        subIcons[i * suffixCount] = register
                            .registerIcon(sm + ":" + getUnlocalizedNameForIcon() + "." + subNames[i]);
                    } else {
                        for (int j = 0; j < suffixes.length; j++) {
                            String iconToReg = sm + ":"
                                + getUnlocalizedNameForIcon()
                                + "."
                                + subNames[i]
                                + "."
                                + suffixes[j];
                            if (DarkcoreMod.debugText) System.out
                                .println("[AB]Registering " + iconToReg + " in slot " + ((i * suffixCount) + j));
        ...
        } else iconBuffer = register.registerIcon(sm + ":" + getUnlocalizedNameForIcon());
    }
    ...
    public IIcon getIcon(int side, int metadata) {
        ...
        if ((subIcons != null) && !(this instanceof IColorableBlock)) {
            ...
            if (suffixes == null) {
                ...
                int metaAdd = 0;
                for (int j = 0; j < suffixes.length; j++) {
                    if (suffixes[j].contains("top") && (side == 1)) return subIcons[metaBase + j];
                    else if (suffixes[j].contains("bottom") && (side == 0)) metaAdd = j;
    ...
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer pl, int s, float i, float j, float k) {
        ...
        if (this instanceof IActivatablePrecise) {
            if (ServerHelper.isClient()) PreciseRightClickHandler.handle(w, x, y, z, pl, s, i, j, k);
            // ((IActivatablePrecise)this).activate(pl, s, x+Math.max(i,0.9999f), y+Math.max(j,0.9999f),
            // z+Math.max(k,0.9999f));
            return true;
        }
        if (this instanceof IColorableBlock) {
            IBlockIteratorCondition ibic = ((IColorableBlock) this)
                .getColoringIterator(new SimpleCoordStore(w, x, y, z));
            ...
            if (item instanceof ItemDye) {
                ...
                if (oreDictIDs.length == 0) return false;
    ...
    public void addInfo(int metadata, NBTTagCompound nbt, List<String> infoList) {};
