...
public class SimulacrumBlock extends AbstractBlock {
    ...
    @Override
    public boolean colorBlock(World w, int x, int y, int z, EntityPlayer pl, IBlockIteratorCondition cond, ItemStack is,
        int color, int depth) {
    ...
    public boolean isOpaqueCube() {
        if (sim != null) return sim.isOpaqueCube();
        return true;
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess w, int s, int x, int y, int z, int ox, int oy, int oz) {
        ...
        if (a == b) {
            ...
            if ((ma == mb) && !a.isOpaqueCube()) return false;
        }
