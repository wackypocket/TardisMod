...
public abstract class AbstractCommandNew extends AbstractCommand {
    ...
    public AbstractCommandNew(AbstractCommandNew... subComs) {
        if (subComs == null) subCommands = new AbstractCommandNew[0];
        else subCommands = subComs;
        allSubArgs = new ArrayList<String>();
        for (AbstractCommandNew acn : subCommands) allSubArgs.addAll(acn.getCommandAliases());
    }
    ...
    public void getCommandUsage(ICommandSender sender, String totalCommand) {
    ...
    public boolean commandBody(ICommandSender sen, String totalCommand, List<String> strs) {
        if (strs.size() > 0) {
            ...
            List<String> nextList = new ArrayList<String>(strs);
            nextList.remove(0);
            comLoop: for (AbstractCommandNew com : subCommands) {
                List<String> aliases = com.getCommandAliases();
                for (String alias : aliases) if (possibleSubName.equalsIgnoreCase(alias)) {
                    if (com.canCommandSenderUseCommand(sen)) {
                        if (com.commandBody(sen, totalCommand + " " + alias, nextList)) return true;
                        else continue comLoop;
                    } else continue comLoop;
                }
            }
        }
        ...
        if (subCommands.length > 0) {
            ...
            for (AbstractCommandNew sc : subCommands) sendString(sen, "    " + sc.getCommandName());
        } else {
            sendString(sen, "------------");
    ...
    public List<String> match(String arg, List<String> toMatch) {
        ...
        while (iter.hasNext()) {
            String lower = iter.next()
                .toLowerCase();
            if (!lower.startsWith(lowerArg)) iter.remove();
        }
        return data;
    }
    ...
    @Override
    public List<String> addTabCompletionOptions(ICommandSender sen, String[] args) {
        if (isUsernameIndex(args, args.length)) return getPlayerList(args[args.length - 1]);
        if (subCommands.length != 0) {
            if (args.length > 0) {
                for (AbstractCommandNew acn : subCommands) for (String alias : acn.getCommandAliases()) {
                    if (alias.equalsIgnoreCase(args[0])) {
                        String[] subArgs = new String[args.length - 1];
                        for (int i = 1; i < args.length; i++) subArgs[i - 1] = args[i];
