...
public abstract class AbstractTileEntity extends TileEntity {
        ...
        updateCounterMax = cf
            .getConfigItem(
                new ConfigItem(
                    ...
                    "Default: 20"))
            .getInt();
        multiBlockInterval = cf
            .getConfigItem(
                new ConfigItem(
                    "multi block interval",
                    CType.INT,
                    80,
    ...
    public void sendUpdate() {
        ...
        if (canSendUpdate()) {
            io.darkcraft.darkcore.mod.logging.TMLLog.debug("ATE", "Sending update " + getClass().getSimpleName());
            updateQueued = false;
            updateCounter++;
            lastUpdateTT = tt;
            ...
            io.darkcraft.darkcore.mod.logging.TMLLog
                .debug("ATE", "Update sending blocked to prevent spam " + getClass().getSimpleName());
            updateQueued = true;
        }
    }
    ...
    public void updateEntity() {
        ...
        if (ServerHelper.isServer() && (this instanceof IChunkLoader)) {
            ...
            if (icl.shouldChunkload()) DarkcoreMod.chunkLoadingHandler.loadMe(icl);
        }
    ...
    public void sendDataPacket() {
        if (ServerHelper.isServer()) {
            io.darkcraft.darkcore.mod.logging.TMLLog.debug("ATE", "Called sendDataPacket");
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        if (!io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "placed")) super.readFromNBT(nbt);
        readTransmittable(nbt);
    }
