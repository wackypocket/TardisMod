...
public abstract class AbstractEffect {
    ...
    public EntityLivingBase getEntityLB() {
        Entity e = getEntity();
        if (!(e instanceof EntityLivingBase)) return null;
    ...
    /**
     * Called when the effect is added to an entity (including when that entity is created (e.g. when a player logs in
     * or the chunk loads))
     */
    ...
    public final void read(NBTTagCompound nbt) {
        tt = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "tt", tt);
        readFromNBT(nbt);
    }
    ...
    /**
     * Save the current state of the effect to NBT (id, tt and duration are all saved automatically in the write
     * function)
     * 
     * @param nbt
     */
