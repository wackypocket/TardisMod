...
public final class StackedEffect extends AbstractEffect {
    ...
    private static int getMaxDuration(AbstractEffect[] in) {
        ...
        for (AbstractEffect e : in) {
            if ((e != null) && ((e.duration - e.tt) > max)) max = e.duration - e.tt;
            if (e.duration == -1) return -1;
        }
        return max;
    }
    ...
    public static AbstractEffect addEffect(StackedEffect old, AbstractEffect newEffect) {
        int count = 0;
        for (AbstractEffect e : old.effects) if (e.tt < e.duration) count++;
        if (count == 0) return newEffect;
        AbstractEffect[] newArr = new AbstractEffect[count + 1];
        int i = 0;
        for (AbstractEffect e : old.effects) if (e.tt < e.duration) newArr[i++] = e;
        newArr[i] = newEffect;
    ...
    public void apply() {
        for (AbstractEffect e : effects) {
            if (e.tt < e.duration) e.update();
            if (e.tt == e.duration) e.effectRemoved();
        }
    }

    @Override
    public void effectAdded() {
        for (AbstractEffect e : effects) e.effectAdded();
    }

    @Override
    public void effectRemoved() {
        for (AbstractEffect e : effects) e.effectRemoved();
    }

    @Override
    public boolean dispellable() {
        for (AbstractEffect e : effects) if ((e.tt < e.duration) && !e.dispellable()) return false;
        return true;
    }

    @Override
    public ResourceLocation getIcon() {
        return effects[0].getIcon();
    }

    @Override
    public UVStore getIconLocation() {
        return effects[0].getIconLocation();
    }
