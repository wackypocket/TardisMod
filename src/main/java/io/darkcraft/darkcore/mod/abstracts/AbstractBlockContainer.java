...
public abstract class AbstractBlockContainer extends AbstractBlock implements ITileEntityProvider {
     ...
     * @param _dropWithData whether Tile Entities contained by this block should keep their NBT data when the block is
     *                      broken
    ...
    public void breakBlock(World w, int x, int y, int z, Block par5, int par6) {
        ...
        if (te instanceof AbstractTileEntity) {
            ...
            for (ItemStack is : items) WorldHelper.dropItemStack(is, pos);
        }
     ...
     * If you want to add behaviour to the TE this block contains, see {@link IActivatable} or
     * {@link IActivatablePrecise}
     */
    @Override
    ...
    protected void dropBlockAsItem(World w, int x, int y, int z, ItemStack is) {
        ...
        if (ServerHelper.isServer() && w.getGameRules()
            .getGameRuleBooleanValue("doTileDrops") && !w.restoringBlockSnapshots) {
            if (dropWithData) {
                TileEntity te = w.getTileEntity(x, y, z);
                if (te != null) {
                    NBTTagCompound nbt = is.stackTagCompound != null ? is.stackTagCompound : new NBTTagCompound();
                    te.writeToNBT(nbt);
            ...
            if (captureDrops.get()) {
                ...
                capturedDrops.get()
                    .add(is);
                TileEntity te = w.getTileEntity(x, y, z);
                if (te instanceof AbstractTileEntity) ((AbstractTileEntity) te).addDrops(items);
                return;
            } else {
                List<ItemStack> items = new ArrayList<ItemStack>();
                items.add(is);
    ...
    /**
     * If you want to add explosion behaviour to this block as a block or the TE this block contains, see
     * {@link IExplodable}
     */
    @Override
