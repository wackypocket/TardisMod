...
public abstract class AbstractBlockRenderer extends TileEntitySpecialRenderer implements IItemRenderer {
    ...
    public void bindTexture(ResourceLocation rl) {
        if (field_147501_a == null) RenderHelper.bindTexture(rl);
        else super.bindTexture(rl);
    }
    ...
    protected void handleItemRenderType(ItemRenderType type) {
        double upSize = 1.2;
        double doSize = 0.85;
        if (type != ItemRenderType.EQUIPPED) GL11.glScaled(doSize, doSize, doSize);
        else GL11.glScaled(upSize, upSize, upSize);
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
            ...
            doSize = 0.75;
            GL11.glScaled(doSize, doSize, doSize);
            if (type != ItemRenderType.INVENTORY) GL11.glTranslated(0, 0.4, 0);
        }
    }
