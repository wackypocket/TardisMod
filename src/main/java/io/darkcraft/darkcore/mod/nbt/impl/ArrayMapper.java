...
public class ArrayMapper<T> extends Mapper<T[]> {
    ...
    public void writeToNBT(NBTTagCompound nbt, T[] t) {
        ...
        for (int i = 0; i < t.length; i++) {
            if (t[i] != null) mapper.writeToNBT(nbt, "i" + i, t[i]);
        }
    }

    @Override
    public T[] fillFromNBT(NBTTagCompound nbt, T[] t) {
        if (t == null) return createFromNBT(nbt);
        if (t.length != io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "s", t.length)) return createFromNBT(nbt);
        for (int i = 0; i < t.length; i++) {
            if (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "i" + i)) {
                if ((t[i] == null) || mapper.shouldCreateNew()) t[i] = mapper.readFromNBT(nbt, "i" + i);
                else t[i] = mapper.fillFromNBT(nbt, "i" + i, t[i]);
            } else t[i] = null;
        }
        return t;
    }
    ...
    public T[] createFromNBT(NBTTagCompound nbt, Object... arguments) {
        T[] tArr = (T[]) Array.newInstance(clazz, io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "s", 0));
