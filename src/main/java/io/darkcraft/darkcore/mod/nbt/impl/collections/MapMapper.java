...
public abstract class MapMapper<T extends Map> extends Mapper<T> {
    ...
    public T fillFromNBT(NBTTagCompound nbt, T t) {
        ...
        t.clear();
        int size = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, SIZE_KEY, 0);
        for (int i = 0; i < size; i++) {
            Object k = stm.createFromNBT(io.darkcraft.darkcore.mod.nbt.NBTUtils.getCompoundOrEmpty(nbt, "k" + i));
            Object v = stm.createFromNBT(io.darkcraft.darkcore.mod.nbt.NBTUtils.getCompoundOrEmpty(nbt, "v" + i));
            t.put(k, v);
        }
        return t;
    }
