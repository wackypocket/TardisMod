...
public class SubTypeMapper<T> extends Mapper<T> {
    ...
    public void writeToNBT(NBTTagCompound nbt, T o) {
        ...
        String clazzName = o.getClass()
            .getName();
        nbt.setString(NBTCLASSNAME, clazzName);
        Mapper<T> mapper = NBTHelper.getMapper(clazz, type);
        if (mapper == null) throw new RuntimeException("No mapper could be found for class " + clazzName);
    ...
    public T fillFromNBT(NBTTagCompound nbt, T o) {
        ...
        Class<T> clazz = (Class<T>) o.getClass();
        Mapper<T> mapper = NBTHelper.getMapper(clazz, type);
        if (mapper == null) throw new RuntimeException("No mapper could be found for class " + clazz.getName());
    ...
    public T createFromNBT(NBTTagCompound nbt, Object... arguments) {
        if (!io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, NBTCLASSNAME)) return null;
        String clazzName = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, NBTCLASSNAME, "");
        try {
            Class<T> clazz = (Class<T>) Class.forName(clazzName);
            Mapper<T> mapper = NBTHelper.getMapper(clazz, type);
            if (mapper == null) throw new RuntimeException("No mapper could be found fod class " + clazz.getName());
