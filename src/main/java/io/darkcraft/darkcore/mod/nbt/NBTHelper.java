...
public class NBTHelper {
    ...
    static {
        mapperTable = Tables.newCustomTable(
            new HashMap<Class<?>, Map<SerialisableType, Mapper<?>>>(),
    ...
    public static <T> void register(Class<T> c, Mapper<T> mapper) {
        for (SerialisableType type : SerialisableType.values()) register(c, type, mapper);
    }
    ...
    public synchronized static <T> Mapper<T> getMapper(Class<T> c, SerialisableType type) {
        if (c.isPrimitive()) c = Primitives.wrap(c);
        Reflection.initialize(c);
        if (unmappableClasses.contains(c)) return null;
        Mapper<T> mapper = (Mapper<T>) mapperTable.get(c, type);
        if (mapper != null) return mapper;
        if (c.isEnum()) mapper = new EnumMapper(c);
        else if (c.isInterface()) mapper = new SubTypeMapper(type);
        else if (c.isArray()) {
            Class<?> arr = c.getComponentType();
            Mapper<?> map = getMapper(arr, type);
            if (map != null) mapper = new ArrayMapper(arr, map);
        } else {
        ...
        if (mapper != null) register(c, type, mapper);
        else unmappableClasses.add(c);
        return mapper;
    }

    public static Object loadFromAnonymousNBT(NBTTagCompound nbt, SerialisableType type, Object... args) {
        String name = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, Mapper.NBTCLASSNAME, "");
        try {
            Class<?> c = Class.forName(name);
            Mapper<?> m = getMapper(c, type);
            if (m == null) throw new RuntimeException("Could not create mapper for " + c.getSimpleName());
