...
public class MessageOverlayRenderer extends Gui {
    ...
    private static class Message {
        ...
        public Message(String message, ResourceLocation icon, int s, long arr, UVStore uv) {
            if ((message == null) || message.isEmpty()) m = "";
            else {
                String[] split = message.split(" ");
                String x = "";
                for (String str : split) x += StatCollector.translateToLocal(str) + " ";
                m = x.trim();
            }
            rl = icon;
            secs = s;
    ...
    private static List<Message> getMessages() {
        ...
        while (mIter.hasNext()) {
            ...
            if ((m.arrivalTime + m.secs) < t) mIter.remove();
        }
        return messageList;
    }
    ...
    private void renderBox(int numLines) {
        ...
        for (int i = 0; i < numLines; i++) face(tess, 0, (mh * i) + th, w, mh, 0, 0.25f, 1f, 0.5f);
