...
public class ShaderHandler {
    ...
    public static int getShader(InputStream str, int shaderType) {
        ...
        try {
            ...
            if (ARBShaderObjects.glGetObjectParameteriARB(s, ARBShaderObjects.GL_OBJECT_COMPILE_STATUS_ARB)
                == GL11.GL_FALSE) throw new RuntimeException(getLogInfo(s));
        } catch (Exception e) {
            if (s != 0) ARBShaderObjects.glDeleteObjectARB(s);
    ...
    public static int getShaderProgram(Pair<InputStream, Integer>... shaders) {
        ...
        for (int i : ids) ARBShaderObjects.glAttachObjectARB(program, i);
        ARBShaderObjects.glLinkProgramARB(program);
        if (ARBShaderObjects.glGetObjectParameteriARB(program, ARBShaderObjects.GL_OBJECT_LINK_STATUS_ARB)
            == GL11.GL_FALSE) throw new RuntimeException(getLogInfo(program));
