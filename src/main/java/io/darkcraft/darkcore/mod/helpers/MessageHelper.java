...
public class MessageHelper {
    ...
    public static void sendMessage(ICommandSender player, ResourceLocation icon, UVStore uv, String message,
        int seconds) {
        if (player instanceof EntityPlayerMP)
            sendMess((EntityPlayerMP) player, MessagePacketHandler.getDataPacket(icon, message, seconds, uv));
        else player.addChatMessage(new ChatComponentText(message));
    }
