...
public class TeleportHelper {
     ...
     */
    public static Entity transferEntityToDimension(Entity ent, int newDimension, double newX, double newY,
        double newZ) {
        if (ent instanceof IBossDisplayData) return ent;
        ...
        if (ent.isRiding()) ent.mountEntity(null);
        if (ent instanceof EntityPlayerMP) {
            ...
            if (entity != null) {
                ...
                if (!didSpawn) System.err.println("Failed to spawn entity");
                else entity.dimension = newDimension;
                ent.isDead = didSpawn;
    ...
    public static void teleportEntity(Entity ent, int worldID, double x, double y, double z, double rot) {
        if (ent instanceof IBossDisplayData) return;
        MinecraftServer serv = MinecraftServer.getServer();
        if (ServerHelper.isServer() && (serv != null) && (ent instanceof EntityLivingBase)) {
            /*
             * WorldServer nW = WorldHelper.getWorldServer(worldID); if(nW.provider instanceof TardisWorldProvider &&
             * ServerHelper.isServer()) { Packet dP = TardisDimensionRegistry.getPacket();
