...
public class MultiBlockHelper {
    ...
    private static ForgeDirection[] rotDirs = new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.EAST,
            ...
            for (int xI = 0; xI < row.length; xI++) {
                ...
                for (int zI = 0; zI < cells.length; zI++) {
                    ...
                    if (cell != null) {
                        ...
                        if (te instanceof IMultiBlockPart) partSet.add((IMultiBlockPart) te);
                    }
                }
            }
        }
        if (core instanceof IMultiBlockCore)
            for (IMultiBlockPart p : partSet) p.setMultiBlockCore((IMultiBlockCore) core);

        return true;
    }

    public static void generateStructure(IMultiBlockStructure structure, World w, SimpleCoordStore pos,
        ForgeDirection dir) {
        int x = pos.x;
        int y = pos.y;
        int z = pos.z;
        ...
        for (int yI = 0; yI < blockStates.length; yI++) {
            ...
            if (row == null) continue;
            for (int xI = 0; xI < row.length; xI++) {
                if (xO != 0) xToCheck = (x + (xO * (structure.getCoreX() - xI)));
                else zToCheck = (z + (zO * (structure.getCoreX() - xI)));
                IBlockState[] cells = row[xI];
                if (cells == null) continue;
                for (int zI = 0; zI < cells.length; zI++) {
                    IBlockState cell = cells[zI];
                    if (cell != null) {
                        if (xO != 0) zToCheck = (z + (xO * (structure.getCoreZ() - zI)));
                        else xToCheck = (x + (zO * (structure.getCoreZ() - zI)));
    ...
    public static void generateAtFloor(IMultiBlockStructure structure, World w, SimpleCoordStore pos,
        ForgeDirection dir) {
        boolean found = false;
        ...
        for (int y = pos.y; (y > 1) && !found; y--) {
            ...
            for (int x = 0; (x < floor.length) && xfound; x++) {
                ...
                for (int z = 0; (z < row.length) && xfound; z++) {
                    ...
                    int xC = pos.x + (dir.offsetX * (x - structure.getCoreX()))
                        + (dir.offsetZ * (z - structure.getCoreZ()));
                    int zC = pos.z + (dir.offsetZ * (x - structure.getCoreX()))
                        + (dir.offsetX * (z - structure.getCoreZ()));
