...
public class WorldHelper {
    ...
    private static HashMap<Integer, String> worldNameMap = new HashMap<Integer, String>();
    ...
    /**
     * Attempts to get the world specified by id. If this is run on the client, it will either return the client side
     * world or null.
     *
     * @return either the world or null
     */
    public static World getWorld(int id) {
        if (ServerHelper.isClient()) return DarkcoreMod.proxy.getWorld(id);
        else return getWorldServer(id);
    }

    /**
     * Will get the {@link WorldServer} specified by the id, will print an error to the console if the world doesn't
     * exist
     * 
     * @param id
     * @return
    ...
    public static WorldServer getWorldServer(int id) {
        ...
        if (DarkcoreMod.debugText) Thread.dumpStack();
        return null;
    }
    ...
    public static int getWorldID(World w) {
        // TODO: Look into this
        if ((w == null) || (w.provider == null)) return 0;
    ...
    public static int getClientWorldID() {
        if (ServerHelper.isClient()) return getWorldID(getCW());
        return 0;
    }
    ...
    public static String getDimensionName(int worldID) {
        if (worldNameMap.containsKey(worldID)) return worldNameMap.get(worldID);
    ...
    public static String getDimensionName(World w) {
        int id = getWorldID(w);
        if (worldNameMap.containsKey(id)) return worldNameMap.get(id);
        if (w != null) {
    ...
    public static void setDimensionName(World w, String newName) {
        ...
        if ((newName == null) || newName.isEmpty() || newName.equals("null")) worldNameMap.remove(id);
        else worldNameMap.put(id, newName);
    }
    ...
    public static ItemStack transferItemStack(ItemStack is, IInventory dest, ForgeDirection side) {
        ...
        for (int i = 0; (i < size) && (remaining.stackSize > 0); i++) {
            boolean canIns = dest instanceof ISidedInventory ? ((ISidedInventory) dest).canInsertItem(i, is, ord)
                : dest.isItemValidForSlot(i, remaining);
            if (canIns) {
    ...
    public static boolean softBlock(World w, int x, int y, int z) {
        ...
        Boolean valid = w.isAirBlock(x, y, z) || b.isFoliage(w, x, y, z)
    ...
    public static boolean removeTE(World w, TileEntity te) {
        if ((w == null) || (te == null)) return false;
    ...
    public static class WorldNameStore extends AbstractWorldDataStore {
        ...
        public WorldNameStore(String s) {
            super(s);
        }

        public void update(World w, String newName) {
            int id = getWorldID(w);
            if ((newName == null) || newName.isEmpty() || newName.equals("null")) nameMap.remove(id);
            else nameMap.put(id, newName);
            markDirty();
        }

        private void copyToWorldNameMap() {
            for (Entry<Integer, String> ent : nameMap.entrySet()) worldNameMap.put(ent.getKey(), ent.getValue());
        }

        @Override
        public void readFromNBT(NBTTagCompound nbt) {
            nameMap.clear();
            for (int i = 0; io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "wi" + i); i++) nameMap.put(
                io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "wi" + i, 0),
                io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "wn" + i, ""));
            copyToWorldNameMap();
        }
