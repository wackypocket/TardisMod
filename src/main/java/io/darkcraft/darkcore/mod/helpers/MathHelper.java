...
public class MathHelper {
    ...
    static {
        ...
        for (int i = 0; i < sinData.length; i++) sinData[i] = Math.sin(Math.toRadians(i / (float) sinNum));
    }
    ...
    public static HashSet<SimpleDoubleCoordStore> removeDuplicateLocations(Set<SimpleDoubleCoordStore> inputSet,
        double tolerance) {
    ...
    private static LoadingCache<Integer, int[]> binomialCache = CacheBuilder.newBuilder()
        ...
        .build(new CacheLoader<Integer, int[]>() {
            ...
            public int[] load(Integer key) throws Exception {
                ...
                newArray[0] = 1;
                int mid = (key + 1) / 2;
                for (int i = 1; i < mid; i++) newArray[i] = previous[i - 1] + previous[i];
                for (int i = mid; i < key; i++) newArray[i] = newArray[key - i - 1];
                return newArray;
            }
    ...
    public static double castHeight(Entity e) {
        if (e instanceof EntityPlayer) return ((EntityPlayer) e).eyeHeight;
        else return e.height / 2;
    }

    public static final ForgeDirection[] horizontal = new ForgeDirection[] { ForgeDirection.NORTH, ForgeDirection.EAST,
