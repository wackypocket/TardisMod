...
public class PlayerHelper {
    ...
    public static EntityPlayerMP getCurrentPlayer(EntityPlayerMP pl) {
        ...
        if ((np == null) || (np.playerNetServerHandler == null) || (np.playerNetServerHandler.netManager == null))
            return null;
        return np;
    }
    ...
    public static boolean validForNetwork(EntityPlayerMP pl) {
        ...
        try {
            pl.playerNetServerHandler.netManager.channel()
    ...
    public static UUID getUUID(EntityPlayer pl) {
        ...
        if (profile != null) return profile.getId();
            ...
            long least = io.darkcraft.darkcore.mod.nbt.NBTUtils.getLong(plNbt, "UUIDLeast", 0L);
            long most = io.darkcraft.darkcore.mod.nbt.NBTUtils.getLong(plNbt, "UUIDMost", 0L);
            if ((least != 0) && (most != 0)) return new UUID(most, least);
        }
        return null;
    }
    ...
    public static boolean isEqual(String oldName, EntityPlayer pl) {
        if (ServerHelper.getUsername(pl)
            .equals(oldName)) {
            getStore().addUser(pl);
            return true;
        }
    ...
    public static class UUIDStore extends AbstractWorldDataStore {
        ...
        public UUID getUUID(String un) {
            ...
            synchronized (uuidMap) {
                for (UUID uuid : uuidMap.keySet()) if (un.equals(uuidMap.get(uuid))) return uuid;
            }
            return null;
        }
        ...
        public void readFromNBT(NBTTagCompound nbt) {
            synchronized (uuidMap) {
                uuidMap.clear();
                int i = 0;
                while (io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "uuid" + i)) {
                    try {
                        String key = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(nbt, "uuid" + (i++), "");
            ...
            swordsEnabled = io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, "swen")
                ? io.darkcraft.darkcore.mod.nbt.NBTUtils.getBoolean(nbt, "swen", true)
                : true;
        }
    ...
    public void playerSpawn(PlayerLoggedInEvent event) {
        ...
        if (!getStore().uuidMap.containsKey(getUUID(player)))
            for (ItemStack is : joinStacks) WorldHelper.giveItemStack(player, is.copy());
