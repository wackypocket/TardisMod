...
public class RaytraceHelper {
    ...
    public static MovingObjectPosition rayIntersectEntity(Entity ent, Vec3 origin, Vec3 end, double expansion) {
        ...
        if (aabb.isVecInside(origin) || aabb.isVecInside(end)) return new MovingObjectPosition(ent, end);
    ...
    public static MovingObjectPosition rayTrace(Entity tracer, Vec3 end, boolean liquids,
        Class<? extends Entity> entClass, boolean doBlocks, boolean doEnts, Entity... skip) {
        ...
        if (doEnts) {
            ...
            aLoop: for (Object o : entityList) {
                for (Entity e : skip) if (o == e) continue aLoop;
                if (!(o instanceof Entity)) continue;
                Entity e = (Entity) o;
                if ((e instanceof IEntityMultiPart) && (entClass != null) && entClass.isInstance(e)) multiparts.add(e);
            }

            oLoop: for (Object o : entityList) {
                ...
                if ((entClass != null) && !entClass.isInstance(e)) {
                    boolean f = false;
                    for (Entity mp : multiparts) if (e.isEntityEqual(mp)) {
                        f = true;
                        break;
                    }
                    if (!f) continue oLoop;
    ...
    public static MovingObjectPosition rayTrace(Entity tracer, Vec3 end, boolean liquids,
        Class<? extends Entity> entClass, boolean blocks, boolean ents) {
        return rayTrace(tracer, end, liquids, entClass, blocks, ents, emptyArr);
    }

    public static MovingObjectPosition rayTrace(Entity tracer, double dist, boolean liquids,
        Class<? extends Entity> entClass, boolean blocks, boolean ents, Entity... skip) {
        Vec3 end = tracer.getLookVec();
        if (dist != 1) {
            double dsq = dist;
    ...
    public static MovingObjectPosition rayTrace(Entity tracer, boolean liquids, Class<? extends Entity> entClass,
        boolean blocks, boolean ents, Entity... skip) {
        Vec3 end = Vec3.createVectorHelper(
            tracer.posX + tracer.motionX,
            tracer.posY + tracer.motionY,
            tracer.posZ + tracer.motionZ);
        return rayTrace(tracer, end, liquids, entClass, blocks, ents, skip);
    }

    public static MovingObjectPosition rayTrace(Entity tracer, boolean liquids, Class<? extends Entity> entClass,
        boolean blocks, boolean ents) {
