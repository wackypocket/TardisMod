...
public class SoundHelper {
    ...
    public static void stopSoundsNear(int dim, int x, int y, int z, double radius) {
        NBTTagCompound data = new NBTTagCompound();
        data.setBoolean("stop", true);
        data.setInteger("x", x);
        data.setInteger("y", y);
        data.setInteger("z", z);
    ...
    public static void stopSoundsNear(TileEntity te, double radius) {
        if (te == null) return;
        stopSoundsNear(WorldHelper.getWorldID(te.getWorldObj()), te.xCoord, te.yCoord, te.zCoord, radius);
    }
}
