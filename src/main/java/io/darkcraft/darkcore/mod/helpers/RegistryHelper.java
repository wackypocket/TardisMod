    ...
    private static void fillIn(Object o, Class<?> c) {
        try {
            for (Field f : c.getDeclaredFields()) {
                if (f.isAnnotationPresent(RegistryIgnore.class)) continue;
                CustomRegistryItem annotation = f.getAnnotation(CustomRegistryItem.class);
                if ((annotation != null) && !Modifier.isPublic(f.getModifiers())) throw new RuntimeException(
                    c.getName() + "." + f.getName() + " is not accessible for RegistryHelper");
                Class<?> fieldClass = f.getType();
                if (!(AbstractBlock.class.isAssignableFrom(fieldClass)
                    || AbstractItem.class.isAssignableFrom(fieldClass))) {
                    io.darkcraft.darkcore.mod.logging.TMLLog
                        .debug("REG", "In class " + c.getName() + " field " + f.getName() + " is not assignable");
                    continue;
                }
                Object instance = generate(annotation, fieldClass);
                if (instance instanceof AbstractBlock) ((AbstractBlock) instance).register();
                else if (instance instanceof AbstractItem) ((AbstractItem) instance).register();
    ...
    private synchronized static Object generate(CustomRegistryItem annot, Class<?> clazz) throws Exception {
        ...
        if (annot == null) {
            ...
            if ((o = constructedMap.get(clazz, annot.value())) == null) {
                o = clazz.getConstructor(String.class)
                    .newInstance(annot.value());
