...
public class ServerHelper {
    ...
    public static boolean isServer(Object o) {
        if (o instanceof World) return !((World) o).isRemote;
        if (o instanceof Entity) return isServer(((Entity) o).worldObj);
        if (o instanceof TileEntity) return isServer(((TileEntity) o).getWorldObj());
        return isServer();
    }
    ...
    public static NBTTagCompound readNBT(InputStream in) {
        try {
            try {
                ...
                System.err.println("[SH]Error reading compressed NBT: " + e.getMessage());
                e.printStackTrace();
            }
        } catch (Throwable t) {
            System.err.println("[SH]Unexpected error reading NBT (possible corrupt file): " + t.getMessage());
            t.printStackTrace();
        }
        return null;
    }
    ...
    public static EntityPlayerMP getPlayer(String username) {
        ...
        for (Object o : playerEnts) {
            if (o instanceof EntityPlayerMP) {
                if (((EntityPlayerMP) o).getCommandSenderName()
                    .equalsIgnoreCase(username)) return (EntityPlayerMP) o;
            }
        }
        return null;
    }
     ...
     * @param color
     */
    public static void sendString(EntityPlayer pl, String string, EnumChatFormatting color, boolean italics,
