...
public class ClientProxy extends CommonProxy {
    ...
    private HashMap<AbstractBlockContainer, AbstractBlockRenderer> clientBlockRendererMap = new HashMap<AbstractBlockContainer, AbstractBlockRenderer>();
    private HashMap<AbstractItem, IItemRenderer> clientItemRenderMap = new HashMap<AbstractItem, IItemRenderer>();
    ...
    private void registerCBR(AbstractBlockContainer b, AbstractBlockRenderer r) {
        ...
        if (b.useRendererForItem()) MinecraftForgeClient.registerItemRenderer(Item.getItemFromBlock(b), r);
    }

    private void registerClientRenderers() {
        {
            Iterator<Entry<AbstractBlockContainer, AbstractBlockRenderer>> iter = clientBlockRendererMap.entrySet()
                .iterator();
            while (iter.hasNext()) {
        ...
        {
            Iterator<Entry<AbstractItem, IItemRenderer>> iter = clientItemRenderMap.entrySet()
                .iterator();
            while (iter.hasNext()) {
    ...
    public void registerClientItem(AbstractItem i) {
        ...
        if (iir == null) return;
        if (DarkcoreMod.inited) registerIR(i, iir);
        else clientItemRenderMap.put(i, iir);
    }

    public void registerClientBlock(AbstractBlockContainer b) {
        AbstractBlockRenderer abr = (AbstractBlockRenderer) b.getRenderer();
        if (abr == null) return;
        io.darkcraft.darkcore.mod.logging.TMLLog.debug("DC", "Registering block renderer - " + b.getUnlocalizedName());
        if (DarkcoreMod.inited) registerCBR(b, abr);
        else clientBlockRendererMap.put(b, abr);
    }
