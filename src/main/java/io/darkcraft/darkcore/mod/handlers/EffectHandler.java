...
public class EffectHandler {
    ...
    private static Set<EntityEffectStore> watchedStores = Collections
        .newSetFromMap(new WeakHashMap<EntityEffectStore, Boolean>());
    ...
    private static AbstractEffect getStackedEffect(Entity ent, NBTTagCompound nbt) {
        int num = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(nbt, "numSubs", 0);
        if (num == 0) return null;
        AbstractEffect[] subs = new AbstractEffect[num];
        for (int i = 0; i < num; i++) {
            NBTTagCompound subNBT = io.darkcraft.darkcore.mod.nbt.NBTUtils.getCompoundOrEmpty(nbt, "sub" + i);
    ...
    public static AbstractEffect getEffect(Entity ent, NBTTagCompound nbt) {
        ...
        if (StackedEffect.stackId.equals(id)) return getStackedEffect(ent, nbt);
    ...
    public void entConstructEvent(EntityConstructing event) {
        ...
        if (ent instanceof EntityLivingBase) ent.registerExtendedProperties("dcEff", new EntityEffectStore(ent));
    }

    @SubscribeEvent
    public void entTickEvent(TickEvent tick) {
        if ((ServerHelper.isClient() && (tick.type != Type.CLIENT))
            || (ServerHelper.isServer() && (tick.type != Type.SERVER))) return;
        ...
        synchronized (watchedStores) {
            ...
            while (iter.hasNext()) {
                ...
                store.tick();
                if (!store.shouldBeWatched()) iter.remove();
            }
        }
    }
}
