...
public class ItemStackContainer {
    ...
    public static ItemStackContainer[] getArray(int length) {
        return getArray(length, defaultCallback);
    }
    ...
    public static ItemStackContainer[] getArray(int length, IEntityItemInitialiser callback) {
        ...
        for (int i = 0; i < length; i++) newArr[i] = getContainer(callback);
        return newArr;
    }
    ...
    public static ItemStackContainer getContainer() {
        return getContainer(defaultCallback);
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        if ((nbt == null) || nbt.hasNoTags()) setIS(null);
        else setIS(ItemStack.loadItemStackFromNBT(nbt));
    }

    public void readFromNBT(NBTTagCompound nbt, String id) {
        if ((nbt == null) || !io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, id)) {
            setIS(null);
            return;
        } else readFromNBT(NBTUtils.getCompoundOrEmpty(nbt, id));
    }
    ...
    public static void readFromNBT(NBTTagCompound nbt, String id, ItemStackContainer[] array) {
        boolean listFull = io.darkcraft.darkcore.mod.nbt.NBTUtils.hasCompound(nbt, id);
        NBTTagList list = null;
        if (listFull) {
            list = nbt.getTagList(id, 10);
            listFull = (list != null) && (list.tagCount() == array.length);
        }
        for (int i = 0; i < array.length; i++) if (listFull) array[i].readFromNBT(list.getCompoundTagAt(i));
        else array[i].setIS(null);
    }

    public static void tick(ItemStackContainer[] array) {
        if (ServerHelper.isServer()) return;
        for (ItemStackContainer isc : array) isc.tick();
    }
