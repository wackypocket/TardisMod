...
public class WeatherWatchingHandler {
    ...
    private Set<EntityLightningBolt> sentEvents = Collections
        .newSetFromMap(new WeakHashMap<EntityLightningBolt, Boolean>());
    ...
    public void handleTick(TickEvent event) {
        if ((event.side == Side.SERVER) && (event.phase == Phase.END)) {
            ...
            while (iter.hasNext()) {
                World w = iter.next();
                if (w != null) if (handleWorld(w)) iter.remove();
            }
        }
    }
    ...
    public void entityConstruct(EntityConstructing event) {
        if (ServerHelper.isServer()) if ((event.entity != null) && (event.entity instanceof EntityLightningBolt))
