...
public class EntityDataStorePacketHandler implements IDataPacketHandler {
    ...
    public static boolean sendUpdate(AbstractEntityDataStore aeds) {
        ...
        if ((ent == null) || ent.isDead) {
            if (DarkcoreMod.debugText)
                System.err.println("DC AEDS - Update cancelled - Noent " + (ent == null) + " - " + aeds.id);
            return false;
        }
        if (ent.ticksExisted < 5) {
            if (DarkcoreMod.debugText && (ent instanceof EntityPlayer))
                System.err.println("DC AEDS - Update delayed - Youngent " + aeds.id);
            return true;
        }
        ...
        if (aeds.notifyArea()) DarkcoreMod.networkChannel
            .sendToAllAround(dp, new TargetPoint(WorldHelper.getWorldID(ent), ent.posX, ent.posY, ent.posZ, 120));
    ...
    public void tickHandler(TickEvent.ServerTickEvent tick) {
        ...
        while ((aeds = queue.poll()) != null) {
            ...
            if (aeds.sendUpdate()) toReadd.add(aeds);
        }
    ...
    public void onClonePlayer(PlayerEvent.Clone e) {
        ...
        synchronized (knownStores) {
            for (String s : knownStores) {
                ...
                io.darkcraft.darkcore.mod.logging.TMLLog.debug("AEDS", "Attempting to clone:" + s);
                if ((p instanceof AbstractEntityDataStore)
                    && (!e.wasDeath || ((AbstractEntityDataStore) p).shouldPersistDeaths())) {
                    if (p2 == null) {
    ...
    public void onRespawn(PlayerRespawnEvent e) {
        ...
        for (Pair<String, AbstractEntityDataStore> store : stores) {
            if (pl.getExtendedProperties(store.a) == null) pl.registerExtendedProperties(store.a, store.b);
        }
    }
    ...
    public void handleData(NBTTagCompound data) {
        if (data == null) return;
        World w = WorldHelper.getWorld(io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(data, "world", 0));
        int eid = io.darkcraft.darkcore.mod.nbt.NBTUtils.getInt(data, "entID", -1);
        if (w == null || eid == -1) return;
        Entity e = w.getEntityByID(eid);
        String aid = io.darkcraft.darkcore.mod.nbt.NBTUtils.getString(data, "aedsID", "");
