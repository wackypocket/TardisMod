...
public class EntityContainerHandler {
    ...
    public static IEntityContainer<?> getContainer(Entity ent) {
        if ((ent instanceof EntityPlayer) && !(ent instanceof FakePlayer)) {
            ...
            if (uuid != null) {
                PlayerContainer pc = getCache(ent.worldObj.isRemote).getUnchecked(uuid);
    ...
    public static IEntityContainer<?> getContainer(NBTTagCompound nbt, String id) {
        NBTTagCompound snbt = io.darkcraft.darkcore.mod.nbt.NBTUtils.getCompoundOrEmpty(nbt, id);
        if (snbt.hasNoTags()) return null;
        byte t = io.darkcraft.darkcore.mod.nbt.NBTUtils.getByte(snbt, "t", (byte) 0);
        if (t == 1) {
            PlayerContainer pc = getCache(ServerHelper.isServer())
                .getUnchecked(NBTHelper.readUUIDFromNBT(snbt, "uuid"));
            return pc;
        } else return EntityContainer.readFromNBT(snbt);
    }
    ...
    private static LoadingCache<UUID, PlayerContainer> getCache(boolean serverSide) {
        if (serverSide) return pcCacheSer;
        else return pcCacheCli;
    }
    ...
    static {
        ...
        pcCacheSer = CacheBuilder.newBuilder()
            .build(loader);
        pcCacheCli = CacheBuilder.newBuilder()
            .build(loader);
    }
